...
ap.add_argument(
    ...
    type=str,
    default=[],
    help="list of layers to add obstacles (all if empty)",
)
args = ap.parse_args()
...
class Rect:
    ...
    def overlaps(self, r, strict=False):
        if strict:
            return (
                self._ll._x < r._ur._x
                and self._ur._x > r._ll._x
                and self._ll._y < r._ur._y
                and self._ur._y > r._ll._y
            )
        else:
            return (
                self._ll._x <= r._ur._x
                                    ...
                                    if layer in layerData:
                                        ...
                                        if "xy" in elem:
                                            for i in range(0, len(elem["xy"]), 2):
                                                xmin = min(
                                                    xmin, elem["xy"][i] * gdsUnits
                                                )
                                                xmax = max(
                                                    xmax, elem["xy"][i] * gdsUnits
                                                )
                                            for i in range(1, len(elem["xy"]), 2):
                                                ymin = min(
                                                    ymin, elem["xy"][i] * gdsUnits
                                                )
                                                ymax = max(
                                                    ymax, elem["xy"][i] * gdsUnits
                                                )
                                            if layer not in layerRects:
                                                layerRects[layer] = list()
                                            layerRects[layer].append(
        ...
        for line in lines:
            ...
            if tmps:
                ...
                if (not inMacro) and (t == "MACRO"):
                    ...
                    r = Rect(
                        float(tmps[1]) * macroUnits,
                        float(tmps[2]) * macroUnits,
                        float(tmps[3]) * macroUnits,
                        float(tmps[4]) * macroUnits,
                    )
