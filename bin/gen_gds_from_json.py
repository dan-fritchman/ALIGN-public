...
ap.add_argument(
    "-p", "--pl_file", type=str, default="", help="<filename.placement_verilog.json>"
)
ap.add_argument(
    "-g", "--gds_dir", type=str, default="", help="<dir with all leaf gds files>"
)
        ...
        for i in self._instances:
            if i._modu:
                ...
                if i._tr._sX < 0:
                    ...
                    refl = i._tr._sY < 0
                print(
                    f"{self._name} creating reference of {i._name} at {(oX,oY)} {refl} {angle})"
                )
                ref = gdspy.CellReference(
                    i._modu._cell, (oX, oY), x_reflection=refl, rotation=angle
                )
                if not self._cell:
                        ...
                        for bstr in bl["bgnstr"]:
                            if "elements" in bstr:
                                ...
                                for el in elements:
                                    ...
                                    if xy and len(xy) == 10:
                                        xmin = (
                                            min([xy[k] for k in range(0, len(xy), 2)])
                                            / args.scale
                                        )
                                        ymin = (
                                            min([xy[k] for k in range(1, len(xy), 2)])
                                            / args.scale
                                        )
                                        xmax = (
                                            max([xy[k] for k in range(0, len(xy), 2)])
                                            / args.scale
                                        )
                                        ymax = (
                                            max([xy[k] for k in range(1, len(xy), 2)])
                                            / args.scale
                                        )
                                        m._cell.add(
                                            gdspy.Rectangle(
                                        ...
                                        pts = [
                                            (xy[k] / args.scale, xy[k + 1] / args.scale)
                                            for k in range(0, len(xy), 2)
                                        ]
                                        m._cell.add(
                                            gdspy.Polygon(
                                                pts, layer=layer, datatype=ldata
                                            )
                                        )
            m._added = True
...
for j, m in modules.items():
    ...
    print(
        f"bounding box of {m._name} : ({round(bbox[0][0],2)},{round(bbox[0][1],2)}) ({round(bbox[1][0],2)},{round(bbox[1][1],2)})"
    )
    gdslib.add(m._cell)
