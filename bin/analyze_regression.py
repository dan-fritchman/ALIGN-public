...
def collect_errors(regression_path):
    ...
    df = pd.DataFrame(
        columns=("name", "failed before pnr", "failed during pnr")
        + tuple(p[0] for p in patterns)
        + ("other", "w", "h", "area", "aspect")
    )
    ...
    for path in regression_path.iterdir():
        ...
        if pnr.is_dir():
            ...
            df = df.append(
                {"name": str(path), "failed before pnr": True}, ignore_index=True
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=".",
        help="Location of regression results, typically $ALIGN_WORK_DIR/FinFET14nm_Mock_PDK",
    )
    parser.add_argument(
        "-o",
        "--csv_output_file",
