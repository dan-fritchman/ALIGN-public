...
def plot_grid_enclosure(temp_matrix, layer):
    enclosure_metal = []
    for i in range(temp_matrix.shape[0]):
        enclosure_metal.append(temp_matrix[i])
    plot_Box(enclosure_metal, "r")
    # plt.savefig(str(layer)+"_Enclosure.png")
    # plt.show()
...
filename_enclosure = sys.argv[1]
filename_metalvia = sys.argv[2]
layer = int(sys.argv[3])
matrix = readfile(filename_enclosure)
temp_matrix = metal_via_data(matrix, layer)
plot_grid_enclosure(temp_matrix, layer)
