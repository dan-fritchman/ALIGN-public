...
class Lee:
    ...
    def astar(self, nm, src, tgt, obstacles):
        def heuristic(v):
            ...
            # k=1 is horizontal
            if v[2] == 1 and delta_i != 0 or v[2] == 0 and delta_j != 0:
                res += self.via_cost
...
class StrongPruning:
                ...
                for x in comb:
                    for obstacles, (cost, order) in dd.items():
                        ...
                        if d is not None:
                            ...
                            reached[new_remaining] = {
                                frozenset(new_obstacles): (new_cost, new_order)
                            }
    ...
    def branch_and_bound(self):
                    ...
                    if min_cost_upper_bound is not None:
                        ...
                        for j in possible:
                            ...
                            if not self.check():
                                ...
                                delta_cost = self.router.path_length(
                                    self.router.paths[new_nm]
                                )
                        ...
                        if ok:
                            ...
                            if min_cost_lower_bound >= min_cost_upper_bound:
                                print(
                                    f'Pruning based on bound "{self.disp()}" {min_cost_lower_bound} {min_cost_upper_bound}'
                                )
                                failed.add(tuple(self.stack))
                                ...
                                while True:
                                    ...
                                    if len(self.stack) > 1:
                                        ...
                                        if min_cost_lower_bound >= min_cost_upper_bound:
                                            print(
                                                f'**More pruning based on bound "{self.disp()}" {min_cost_lower_bound} {min_cost_upper_bound}'
                                            )
                                ...
                                if self.stack:
                                    ...
                                    print(
                                        f"Whole tree pruned {self.disp(most_constraining_e)}"
                                    )
                                    done = True
        ...
        print(
            f'{plural("success", successes, "es")} out of {plural("trial", trial)} and {plural("restart", restarts)}.'
        )
...
def main(n_i, n_j, nets, args):
    ...
    def route(samp):
        ...
        if ok:
            ...
            a.show()
            print(
                f"Routed all {len(nets)} nets. Wire Length = {a.total_wire_length()} Dequeues: {a.sum_of_counts}"
            )
        else:
            ...
...
"""
    ...
    p = re.compile(
        r"^(ten_nets|simple|synthetic|cost|triple|river|random)_(\d+)x(\d+)$"
    )
