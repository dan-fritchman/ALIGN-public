...
def simple(args):
    ...
    print(
        f"n: {n} state_space_size: {state_space_size(n)} count: {count} state_space_size/count: {state_space_size(n)/count} successes: {successes} count/successes: {count/successes}"
    )
...
def disp(s):
    ...
    if not s or max(s) < 52:
        ...
        return " ".join(
            [chr(origin[r] + x0) + chr(origin[0] + x1) for r, x0, x1 in triples]
        )
