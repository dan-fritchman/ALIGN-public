...
def draw(model, instance_map, wires):
    ...
    for name, i in instance_map.items():
        llx, lly = (
            model.var_by_name(f"{name}_llx").x,
            model.var_by_name(f"{name}_lly").x,
        )
        urx, ury = (
            model.var_by_name(f"{name}_urx").x,
            model.var_by_name(f"{name}_ury").x,
        )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            text=n_lst,
            mode="text",
            textfont=dict(color="black", size=196),
        )
    )
...
def test_place_on_boundary():
    ...
    with set_context(constraints):
        constraints.append(constraint_schema.Boundary(max_height=10, max_width=10))
        constraints.append(
            constraint_schema.PlaceOnBoundary(
                northwest="M0", northeast="M1", southwest="M2", southeast="M3"
            )
        )
