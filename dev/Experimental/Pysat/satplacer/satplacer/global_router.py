...
def encode_GR(tech, obj):
    ...
    if isinstance(obj, GR):
        ...
        if obj.layer in tech.verticalMetals:
            ...
            xc = tech.pitchPoly * (
                tech.halfXGRGrid * 2 * obj.rect.llx + tech.halfXGRGrid
            )
            llx = xc - obj.width // 2
            urx = xc + obj.width // 2
            lly = tech.pitchDG * (
                tech.halfYGRGrid * 2 * obj.rect.lly
                + tech.halfYGRGrid
                - (tech.halfYGRGrid - 1)
            )
            ury = tech.pitchDG * (
                tech.halfYGRGrid * 2 * obj.rect.ury
                + tech.halfYGRGrid
                + (tech.halfYGRGrid - 1)
            )
            ...
            ury = yc + obj.width // 2
            llx = tech.pitchPoly * (
                tech.halfXGRGrid * 2 * obj.rect.llx
                + tech.halfXGRGrid
                - (tech.halfXGRGrid - 1)
            )
            urx = tech.pitchPoly * (
                tech.halfXGRGrid * 2 * obj.rect.urx
                + tech.halfXGRGrid
                + (tech.halfXGRGrid - 1)
            )
        else:
            raise RuntimeError(
                repr(obj)
                + (
                    "is not horizontal nor vertical (%d,%d,%d,%d)."
                    % (obj.rect.llx, obj.rect.lly, obj.rect.urx, obj.rect.ury)
                )
            )
...
class Grid:
    ...
    def addTerminal(self, net_nm, x, y):
        if net_nm not in self.nets:
            self.nets[net_nm] = set()
    ...
    def cleanAntennas(self):
        ...
        print(
            "Phase 3: cleanAntennas: one by one, check if a site can be made empty, then force it to remain empty."
        )
        for (x, y, k, ly, bv) in self.allRasterPoints():
    ...
    def genMaxCapacityConstraints(self, max_capacity):
        ...
        for ly in self.layers:
            ...
            for x in range(self.nx):
                for y in range(self.ny):
                    outs_bv = tally.BitVec(
                        self.s, "cap_%s_%d_%d" % (ly, x, y), max_capacity + 1
                    )
                    ...
                    inps = [
                        self.per_net_grid[k][ly].var(self.idx(x, y))
    ...
    def genDifferentNetMaxCapacityConstraints(self, max_capacity):
        for x in range(self.nx):
            for y in range(self.ny):
                for (l, ll) in (
                    (l, ll) for l in self.layers for ll in self.layers if l != ll
                ):
                    for k in self.per_net_grid.keys():
                        inps = [self.per_net_grid[k][l].var(self.idx(x, y))] + [
                            self.per_net_grid[kk][ll].var(self.idx(x, y))
                            for kk in self.per_net_grid.keys()
                            if k != kk
                        ]
                        outs_bv = tally.BitVec(
                            self.s,
                            "cap2_%s_%s_%s_%d_%d" % (l, ll, k, x, y),
                            max_capacity + 1,
                        )
                        outs = [outs_bv.var(i) for i in range(max_capacity + 1)]
                        self.s.emit_tally(inps, outs)
    ...
    def genSymmetricRoutes(self, n0, n1):
        ...
        def allStepX(k, v, kk, vv):
            ...
            for x in range(x0, x1 + 1):
                ...
                if x != x0:
                    self.emitWire(k, r, hly, x0, y0, x, y0)
                self.emitWire(k, r, vly, x, y0, x, y1)
                if x != x1:
                    self.emitWire(k, r, hly, x, y1, x1, y1)

                if x != x0:
                    self.emitWire(kk, r, hly, xdist - x, y0, xdist - x0, y0)
                self.emitWire(kk, r, vly, xdist - x, y0, xdist - x, y1)
                if x != x1:
                    self.emitWire(kk, r, hly, xdist - x1, y1, xdist - x, y1)
        ...
        def allStepY(k, v, kk, vv):
            ...
            for y in range(y0, y1 + 1):
                ...
                if y != y0:
                    self.emitWire(k, r, vly, x0, y0, x0, y)
                self.emitWire(k, r, hly, x0, y, x1, y)
                if y != y1:
                    self.emitWire(k, r, vly, x1, y, x1, y1)

                if y != y0:
                    self.emitWire(kk, r, vly, xdist - x0, y0, xdist - x0, y)
                self.emitWire(kk, r, hly, xdist - x1, y, xdist - x0, y)
                if y != y1:
                    self.emitWire(kk, r, vly, xdist - x1, y, xdist - x1, y1)
    ...
    def semantic(self, max_capacity=1, different_net_max_capacity=None):
        ...
        for k in list(self.nets.keys()) + ["!kor"]:
            ...
            for ly in self.layers:
                self.per_net_grid[k][ly] = tally.BitVec(
                    self.s, k + "_" + ly, self.nx * self.ny
                )
    ...
    def emitWire(self, k, r, ly, x0, y0, x1, y1):
        ...
        if x0 != x1:
            ...
            for x in range(x0, x1 + 1):
                print(k, x, y0, y1)
                self.s.emit_implies(
                    r.var(), self.per_net_grid[k][ly].var(self.idx(x, y0))
                )
        ...
        if y0 != y1:
            ...
            for y in range(y0, y1 + 1):
                self.s.emit_implies(
                    r.var(), self.per_net_grid[k][ly].var(self.idx(x0, y))
                )
            ...
            for (ly, bv) in v.items():
                ...
                for y in range(self.ny - 1, -1, -1):
                    print(
                        "".join(
                            [
                                ("1" if bv.val(self.idx(x, y)) else "0")
                                for x in range(self.nx)
                            ]
                        )
                    )
                    ...
                    for y in range(self.ny):
                        ...
                        for x in range(self.nx):
                            ...
                            if (
                                ...
                                if ly not in self.wires[k]:
                                    self.wires[k][ly] = []
                                self.wires[k][ly].append(
                                    GR(k, ly, 400, Rect(x0, y, x1, y))
                                )
                                x0, x1 = None, None
    ...
    def write_globalrouting_json(self, fp, tech, placer_results=None):
        ...
        if placer_results is not None:
            globalScale = Transformation(
                0,
                0,
                hack * tech.halfXADTGrid * tech.pitchPoly,
                hack * tech.halfYADTGrid * tech.pitchDG,
            )
            b = globalScale.hitRect(Rect(*placer_results["bbox"])).canonical()
            terminals.append(
                {
                    "netName": placer_results["nm"],
                    "layer": "diearea",
                    "rect": b.toList(),
                }
            )
            ...
            leaves_map = {
                leaf["template_name"]: leaf for leaf in placer_results["leaves"]
            }
            ...
            for inst in placer_results["instances"]:
                ...
                terminals.append(
                    {"netName": nm, "layer": "cellarea", "rect": r.toList()}
                )
            ...
            for term in placer_results["terminals"]:
                ...
                b.ury -= 360
                terminals.append(
                    {"netName": nm, "layer": term["layer"], "rect": b.toList()}
                )
        ...
        globalScale = Transformation(
            0,
            0,
            hack * tech.halfXGRGrid * tech.pitchPoly,
            hack * tech.halfYGRGrid * tech.pitchDG,
        )
        ...
        data = {
            "bbox": [self.bbox.llx, self.bbox.lly, self.bbox.urx, self.bbox.ury],
            "globalRoutes": grs,
        ...
        fp.write(
            json.dumps(data, indent=2, default=lambda x: encode_GR(tech, x)) + "\n"
        )
...
def aux_from_json(tag, gridFactor=4):
    ...
    #
    g = Grid(
        (nx + gridFactor - 1) // gridFactor,
        (ny + gridFactor - 1) // gridFactor,
        gridFactor,
    )
    ...
    for (ly, v) in g.max_capacity_constraints.items():
        ...
        for ((x, y), vv) in v.items():
            count += sum(
                (1 if vv.val(i) is True else 0) for i in range(max_capacity + 1)
            )
