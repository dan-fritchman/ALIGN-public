...
def encode_T(tech, obj):
    globalTrans = Transformation(
        0,
        0,
        tech.pitchPoly * tech.halfXADTGrid * 2,
        tech.pitchDG * tech.halfYADTGrid * 2,
    )
    ...
    if isinstance(obj, Rect):
        ...
        if obj.layer == "metal1":
            ...
            raise TypeError(
                repr(obj)
                + (" is not JSON serializable. Unknown terminal layer %s" % obj.layer)
            )
    else:
...
class CellTemplate:
    ...
    def dumpJson(self, fp, tech):
        ...
        for ci in self.instances.values():
            if ci.template.nm not in collect_templates:
                collect_templates[ci.template.nm] = []
        ...
        for (k, v) in collect_templates.items():
            ...
            for (net_nm, term_lst) in v[0].terminals.items():
                for term in term_lst:
                    terminals.append(
                        {"net_name": net_nm, "layer": "metal1", "rect": term.toList()}
                    )
            leaves.append(
                {"template_name": k, "bbox": v[0].bbox.toList(), "terminals": terminals}
            )
        ...
        for (k, ci) in self.instances.items():
            for (net_nm, term_lst) in ci.template.terminals.items():
                for term in term_lst:
                    ...
                    assert self.bbox.llx <= r.llx and r.urx <= self.bbox.urx, (
                        self.bbox,
                        k,
                        net_nm,
                        r,
                    ...
                    assert self.bbox.lly <= r.lly and r.ury <= self.bbox.ury, (
                        self.bbox,
                        k,
                        net_nm,
                        r,
    ...
    def buildDataForAnimation(self):
        ...
        p = re.compile(r"^DP_(.+)_(\d+)$")
        fills = {"a": "#e0ffe0", "b": "#e0e0ff", "s": "#ffe0e0"}

        p2 = re.compile(r"^CM_(.+)_(\d+)$")
        fills2 = {
            "4": "#e0ffe0",
            "2": "#e0e0ff",
            "1a": "#ffe0e0",
    ...
    def write_globalrouting_json(self, fp, tech):
        ...
        for inst in self.instances.values():
            ...
            terminals.append(
                {"netName": nm, "layer": "cellarea", "rect": r.canonical()}
            )
        ...
        for inst in self.instances.values():
            ...
            for (k, v) in inst.template.terminals.items():
                for term in v:
                    ...
                    terminals.append(
                        Terminal(
                            nm, "metal1", inst.transformation.hitRect(term).canonical()
                        )
                    )
...
class CellHier(CellTemplate):
    ...
    def updateBbox(self):
        ...
        for inst in self.instances.values():
            ...
            if self.bbox.llx is None or self.bbox.llx > r.llx:
                self.bbox.llx = r.llx
            if self.bbox.lly is None or self.bbox.lly > r.lly:
                self.bbox.lly = r.lly
            if self.bbox.urx is None or self.bbox.urx < r.urx:
                self.bbox.urx = r.urx
            if self.bbox.ury is None or self.bbox.ury < r.ury:
                self.bbox.ury = r.ury
    ...
    def tGen(self, plusOneIfMirrored=False):
        for x in range(self.r.nx):
            for y in range(self.r.ny):
                ...
                pairs = [
                    (self.anchor, Transformation(x, y, 1, 1)),
            ...
            for xx in range(bbox.llx, bbox.urx):
                for yy in range(bbox.lly, bbox.ury):
                    ...
                    if 0 <= xxx and xxx < self.r.nx and 0 <= yyy and yyy < self.r.ny:
                        self.r.s.emit_implies(
                            anchor, self.filled.var(self.r.idx(xxx, yyy))
                        )
                    else:
...
class Raster:
    ...
    def addTerminalOverlapConstraints(self):
        ...
        for k in self.nets.keys():
            ...
            self.net_bvs[k] = tally.BitVec(
                self.s, ("net_terminal_%s" % k), (self.nx + 1) * self.ny
            )
                    ...
                    for term in v:
                        for yy in range(term.lly, term.ury):
                            ...
                            if (
                                ...
                                self.s.emit_implies(
                                    anchor, self.net_bvs[a].var(self.idx(r.llx, r.lly))
                                )
    ...
    def addXNetLengthConstraints(self, net_nm):
        ...
        for x in range(nx + 1):
            self.s.emit_or(
                [bv.var(self.idx(x, y)) for y in range(self.ny)], col_or.var(x)
            )
        ...
        for x in range(nx + 1):
            if x == 0:
                ...
                self.s.emit_or(
                    [col_or.var(nx - x), rscan.var(nx - (x - 1))], rscan.var(nx - x)
                )
    ...
    def addYNetLengthConstraints(self, net_nm):
        ...
        for y in range(ny + 1):
            self.s.emit_or(
                [bv.var(self.idx(x, y)) for x in range(self.nx)], col_or.var(y)
            )
        ...
        for y in range(ny + 1):
            if y == 0:
                ...
                self.s.emit_or(
                    [col_or.var(ny - y), rscan.var(ny - (y - 1))], rscan.var(ny - y)
                )
    ...
    def semantic(self, skipTerminals=False):
        self.ris = [
            RasterInstance(self, inst) for inst in self.template.instances.values()
        ]

        for x in range(self.nx):
            for y in range(self.ny):
                self.s.emit_at_most_one(
                    [ri.filled.var(self.idx(x, y)) for ri in self.ris]
                )
    ...
    def print_rasters(self, bv, nx=None):
        ...
        for y in range(self.ny - 1, -1, -1):
            print(
                "".join([("1" if bv.val(self.idx(x, y)) else "0") for x in range(nx)])
            )
    ...
    def dump_for_animation(self, fp):
        ...
        for placement in self.placements_for_animation:
            if last_placement is None or not equal_placements(
        ...
        leaf_tbl = {
            ci.template.nm: ci.template for (k, ci) in self.template.instances.items()
        }
    ...
    def optimizeNets(self, priority_nets):
        ...
        def findSmallest(net_nm, n, extents, tag, lst=[], strict=False):
            ...
            for lim in range(n, -1, -1):
                ...
                if self.s.state == "SAT":
                    ...
                    print(
                        "Didn't wait to place with %s < %d %s extent"
                        % (net_nm, lim, tag)
                    )
                    return lim + 1
                else:
        ...
        def findSmallestBisection(net_nm, n, extents, tag, lst=[], strict=False):
            ...
            def aux(lo, hi):
                ...
                if lo == hi:
                    print(
                        "Base case: %s %d %d => returns %d" % (net_nm, lo, hi, hi + 1)
                    )
                    return hi + 1
                else:
                    mid = (lo + hi + 1) // 2
                    ...
                    if self.s.state == "SAT":
                        ...
                        print(
                            "Fails to place with %s < %d %s extent" % (net_nm, mid, tag)
                        )
                        return aux(mid, hi)
        ...
        def findSmallestX(net_nm, lst=[], strict=False):
            ...
            return findSmallestBisection(
                net_nm, self.nx, self.xExtents[net_nm][1], "x", lst, strict
            )

        def findSmallestY(net_nm, lst=[], strict=False):
            #        return findSmallest( net_nm, self.ny, self.yExtents[net_nm][1], "y", lst, strict)
            return findSmallestBisection(
                net_nm, self.ny, self.yExtents[net_nm][1], "y", lst, strict
            )
        ...
        def optimizeNetLength(tag, nets):
            ...
            for net_nm in nets:
                ...
                for y in range(self.ny):
                    ...
                    if self.yExtents[net_nm][0].val(y) is True:
                        count += 1
            ...
            result = findSmallestBisection(
                tag, count - 1, netsOut, "total xy", [], True
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Global router placer for collection of designs"
    )
