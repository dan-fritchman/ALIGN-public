...
def translatePlacerResults(
    placer_results, *, x_shift=None, y_shift=None, ux_ratio=None, uy_ratio=None
):
    #
    ...
    def scaleBBox(bbox, offsetX=0, offsetY=0):
        return [
            bbox[0] * ux_ratio + offsetX,
            bbox[1] * uy_ratio + offsetY,
            bbox[2] * ux_ratio + offsetX,
            bbox[3] * uy_ratio + offsetY,
        ]
...
if __name__ == "__main__":
    ...
    with open(block + "_placer_out_scaled.json", "wt") as fp:
        fp.write(
            json.dumps(
                translatePlacerResults(
                    placer_results,
                    x_shift=args.x_shift,
                    y_shift=args.y_shift,
                    ux_ratio=args.ux_ratio,
                    uy_ratio=args.uy_ratio,
                ),
                indent=2,
            )
            + "\n"
