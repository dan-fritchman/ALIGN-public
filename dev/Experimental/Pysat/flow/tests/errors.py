...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print(
            '<testsuites name="End-to-end tests" tests="%d" failures="%d">'
            % (num_tests, fatals + errors)
        )
        print(
            '\t<testsuite name="%s" errors="%d" failures="%d" skipped="%d" tests="%d">'
            % (testsuite, errors, fatals, num_tests - completed, num_tests)
        )
        for i in range(1, len(sys.argv)):
            test = sys.argv[i]
            res = rollup_test(test)
            timing = res[5]
            print(
                '\t\t<testcase classname="%s" name="%s" time="%f"> </testcase>'
                % (test, test, timing)
            )
        print("\t</testsuite>")
        print("</testsuites>")
