...
class SeqPair:
    ...
    def gen_assumptions(self, pvec, nvec):
        return [self.pos[x].var(i) for i, x in enumerate(pvec)] + [
            self.neg[x].var(i) for i, x in enumerate(nvec)
        ]
    ...
    def symmetric_pairs_ordered(self, lst_of_lst, axis="V"):
        ...
        for u, v in pairs:
            ...
            for x in singles:
                self.s.emit_iff(
                    self.order_expr(u, x, axis=oa), self.order_expr(x, v, axis=oa)
                )
        ...
        for (u0, v0), (u1, v1) in combinations(pairs, 2):
            ...
            self.s.emit_implies(
                self.order_expr(u0, u1, axis=oa), self.order_expr(u1, v1, axis=oa)
            )
            self.s.emit_implies(
                self.order_expr(u0, u1, axis=oa), self.order_expr(v1, v0, axis=oa)
            )

            # u1   u0  ccc   v0 v1
            # (u1<u0) => u0 < v0 and v0 < v1
            self.s.emit_implies(
                self.order_expr(u1, u0, axis=oa), self.order_expr(u0, v0, axis=oa)
            )
            self.s.emit_implies(
                self.order_expr(u1, u0, axis=oa), self.order_expr(v0, v1, axis=oa)
            )
    ...
    def symmetric(self, lst_of_lst, axis="V"):
        ...
        def aux0(l, u, x, v):
            self.s.add_clause(
                [-l, -self.order_expr(u, x, axis=oa), self.order_expr(x, v, axis=oa)]
            )
            self.s.add_clause(
                [-l, -self.order_expr(x, v, axis=oa), self.order_expr(u, x, axis=oa)]
            )
        ...
        def aux1(l0, l1, u0, v0, u1, v1):
            self.s.add_clause(
                [
                    -l0,
                    -l1,
                    -self.order_expr(u0, u1, axis=oa),
                    self.order_expr(u1, v1, axis=oa),
                ]
            )
            self.s.add_clause(
                [
            ...
            self.s.add_clause(
                [
                    -l0,
                    -l1,
                    -self.order_expr(u1, u0, axis=oa),
                    self.order_expr(u0, v0, axis=oa),
                ]
            )
            self.s.add_clause(
                [
    ...
    def gen_solutions(self, max_solutions=100):
        ...
        for i in range(max_solutions):
            ...
            self.s.add_clause(
                [-control] + [-x for x in self.gen_assumptions(p_res, n_res)]
            )
...
def test_order_mixed():
    ...
    assert {((0, 1), (0, 1)), ((0, 1), (1, 0))} == set(
        sp.gen_solutions(max_solutions=100)
    )
...
def satisfy_constraints(
    ...
    for constraint in constraints:
        if constraint["constraint"] == "Order":
            ...
            sp.order_array(
                [m[iname] for iname in constraint["instances"]], axis=axis, abut=abut
            )

        elif constraint["constraint"] == "SymmetricBlocks":
            axis = constraint["direction"]
            sp.symmetric(
                [[m[iname] for iname in lst] for lst in constraint["pairs"]], axis=axis
            )
    ...
    if pos_solution is not None and neg_solution is not None:
        ...
        assump = sp.gen_assumptions(
            [m[c] for c in pos_solution], [m[c] for c in neg_solution]
        )
    ...
    p_res, n_res = [invm[x] for x in SeqPair.perm2vec(sp.pos)], [
        invm[x] for x in SeqPair.perm2vec(sp.neg)
    ]
...
def test_soner():
    ...
    satisfy_constraints(
        constraints, pos_s, neg_s, single_character=True, max_solutions=100
    )
