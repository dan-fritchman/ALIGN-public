...
def test_mirrors():
    ...
    places_common_centroid = [
        (tag, nnx - 1 - x, nny - 1 - y) for (tag, x, y) in places if y != 1
    ]
    ...
    for (tag, v) in od.items():
        for (idx, (tag, x, y)) in enumerate(v):
            s.emit_always(
                ri_tbl["CM_%s_%i" % (tag, idx)].anchor.var(r.idx(x * ux, y * uy))
            )
...
def test_diffpairs1x():
    ...
    for (tag, v) in od.items():
        for (idx, (tag, x, y)) in enumerate(v):
            s.emit_always(
                ri_tbl["DP_%s_%i" % (tag, idx)].anchor.var(r.idx(x * ux, y * uy))
            )
...
def test_diffpairs2x():
    ...
    if True:
        ...
        places_common_centroid = [
            (tag, nnx - 1 - x, nny - 1 - y) for (tag, x, y) in places
        ]
    ...
    groups = [out_nets, in_nets, s_nets, ctrl_nets] + [
        list(tup) for tup in chunk(remaining_nets, 6)
    ]
...
def test_vga(optimize=True):
    ...
    groups = [drv_nets, out_nets, in_nets, ctrl_nets] + [
        list(tup) for tup in chunk(remaining_nets, 6)
    ]
