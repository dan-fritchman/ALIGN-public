...
class SatBasedEulerPathsRow:
    ...
    def sat_based_euler_paths_aux(self, G):
        ...
        self.unflipped = [
            self.p.mgr.add_var(
                tally.PossiblyUnknownEnumVar(
                    self.p.s, self.row_nm + ("==>[%s]" % idx), edge_values
                )
            )
        ...
        self.diffs = [
            self.p.mgr.add_var(
                tally.PossiblyUnknownEnumVar(
                    self.p.s, self.row_nm + ("dif[%s]" % idx), self.node_values
                )
            )
        ...
        self.polys = [
            self.p.mgr.add_var(
                tally.PossiblyUnknownEnumVar(
                    self.p.s, self.row_nm + ("ply[%s]" % idx), self.p.gate_values
                )
            )
            for idx in range(0, m)
        ]
        ...
        for ev in edge_values:
            self.p.s.emit_exactly_one(
                [t.var(ev) for t in self.unflipped + self.flipped]
            )
        ...
        for e in G.edges(keys=True, data=True):
            ...
            for idx in range(0, m):
                self.p.s.emit_implies(
                    self.unflipped[idx].var(k), self.diffs[idx].var(n0)
                )
                self.p.s.emit_implies(
                    self.unflipped[idx].var(k), self.diffs[idx + 1].var(n1)
                )
                self.p.s.emit_implies(
                    self.unflipped[idx].var(k), self.polys[idx].var(g)
                )
                ...
                self.p.s.emit_implies(
                    self.flipped[idx].var(k), self.diffs[idx + 1].var(n0)
                )
        ...
        self.different_diffs_and_unknown_tran = [
            self.p.s.add_var() for idx in range(0, m)
        ]
        ...
        for idx in range(0, m):
            ...
            self.p.s.emit_or(
                self.unflipped[idx].vars + self.flipped[idx].vars,
                self.tran_defined[idx],
            )
            ...
            self.p.s.emit_and(
                [tally.Tally.neg(self.tran_defined[idx]), q1],
                self.different_diffs_and_unknown_tran[idx],
            )
...
class SatBasedEulerPaths:
    ...
    def emit_channel_constraints(self):
        ...
        self.all_net_values = list(
            set(self.gate_values + self.row_p.node_values + self.row_n.node_values)
        )
        ...
        left_scan = [
            self.mgr.add_var(
                tally.PowerSetEnumVar(self.s, "ls[%02d]" % idx, self.all_net_values)
            )
            for idx in range(len(self.row_p.polys + self.row_p.diffs))
        ]
        ...
        right_scan = [
            self.mgr.add_var(
                tally.PowerSetEnumVar(self.s, "rs[%02d]" % idx, self.all_net_values)
            )
            for idx in range(len(self.row_p.polys + self.row_p.diffs))
        ]
        right_scan.reverse()
        ...
        and_of_scans = [
            self.mgr.add_var(
                tally.PowerSetEnumVar(self.s, "as[%02d]" % idx, self.all_net_values)
            )
            for idx in range(len(self.row_p.polys + self.row_p.diffs))
        ]
        ...
        for q in and_of_scans:
            ...
            self.s.emit_tally(
                [vr for (vl, vr) in zip(q.vals, q.vars) if vl not in ["vss", "vcc"]],
                outs,
            )
...
class SatPartitionAux:
    def __init__(self, dad, ty, G):
        ...
        self.vars = [
            self.dad.mgr.add_var(tally.BitVar(self.dad.s, ("%s_edge_%d" % (ty, idx))))
    ...
    def semantic(self):
        ...
        if n % 4 == 2:
            ...
            limit = n // 2 + 1
            print(
                "Limiting number of devices in %s master to %d of %d"
                % (self.ty, limit, n)
            )
...
class SatPartition:
    ...
    def semantic(self):
        ...
        self.net_vars = [
            self.mgr.add_var(tally.BitVar(self.s, ("net_%s" % k)))
            for (k, v) in self.lst
        ]
