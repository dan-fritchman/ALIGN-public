...
class Block:
        ...
        if self.children:
            ...
            for (blk, local_tr) in zip(self.children, self.transformations):
                ...
                llx, lly, urx, ury = tuple(
                    local_tr.hitRect(Rect(ll=(blk.llx, blk.lly), ur=(blk.urx, blk.ury)))
                    .canonical()
                    .toList()
                )
                if self.llx is None or llx < self.llx:
                    self.llx = llx
                if self.lly is None or lly < self.lly:
                    self.lly = lly
                if self.urx is None or urx > self.urx:
                    self.urx = urx
                if self.ury is None or ury > self.ury:
                    self.ury = ury
    ...
    def gen_rects(self, global_tr):
        if True or not self.children:
            yield self.nm, tuple(
                global_tr.hitRect(
                    Rect(ll=(self.llx, self.lly), ur=(self.urx, self.ury))
                )
                .canonical()
                .toList()
            )
