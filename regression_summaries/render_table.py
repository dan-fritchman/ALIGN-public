...
parser = argparse.ArgumentParser(
    description="Analyze Regression Results and Build CSV Table"
)
parser.add_argument(
    "-0", "--csv_input_file0", type=str, help="CSV input file 0 (reference)"
)
parser.add_argument(
    "-1", "--csv_input_file1", type=str, help="CSV input file 1 (current)"
)
parser.add_argument(
    "-d0",
    "--regression_directory0",
    type=str,
    help="Regression directory 0 (reference)",
)
parser.add_argument(
    "-d1", "--regression_directory1", type=str, help="Regression directory 1 (current)"
)
...
for id in df.columns:
    if id == "name":
        s = {"if": {"column_id": id}, "minWidth": "200px", "width": "200px"}
    ...
    if m.groups()[1] in "xy" and m.groups()[0] not in ["w", "h", "area", "aspect"]:
        s = {
            "if": {"column_id": id, "filter_query": f"{{{id}}} > 0"},
            "color": "tomato",
            "fontWeight": "bold",
        }
        style_data_conditional.append(s)
...
translate = {
    "x": f"File0 or Directory0: {short_name0}",
...
@app.callback(Output("container", "children"), Input("table", "active_cell"))
