...
def align_manifest_filter(cmake_manifest):
    ...
    return list(
        filter(
            lambda name: "test_" in name or ".so" in name or ".py" in name,
            cmake_manifest,
        )
    )


version = get_version(os.path.join(os.path.abspath(os.path.dirname(__file__)), "align"))
...
setup(
    ...
    package_data={"align": ["config/*", "pdk/finfet/*.json", "pdk/finfet/*.sp"]},
    ...
    extras_require={
        "test": ["pytest", "pytest-cov", "pytest-xdist", "pytest-timeout", "pytest-cpp"]
    },
    python_requires=">=3.7",
    classifiers=[
