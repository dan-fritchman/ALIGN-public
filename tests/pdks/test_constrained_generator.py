...
def check_shorts(pdk, cmdlist, constraints):
    spec = importlib.util.spec_from_file_location(
        "Align_primitives", pdk / "Align_primitives.py"
    )
    ...
    if constraints[0]["parameters"].get("pattern", None) == "cc":
        assert (
            uc.rd.subinsts.keys() == common_centroid
        ), f"common centroid should be ABBA"
    elif constraints[0]["parameters"].get("pattern", None) == "ncc":
        assert uc.rd.subinsts.keys() == set(
            [
                "M1_X0_Y0",
                "M1_X1_Y0",
                "M2_X2_Y0",
        ...
        ), f"non common centroid should be AABB"
        ...
        ), f"inter digitated pattern should be ABAB"
    elif constraints[0]["parameters"].get("shared_diff", None) == True:
        assert (
            uc.rd.subinsts.keys() == common_centroid
        ), f"common centroid should be ABBA"
        assert uc.rd.canvas.bbox.toList() == [
            0,
            0,
            1120,
            3528,
        ], "shared device should have smaller area"
...
supported_const = [
    {"constraint": "Generator", "name": "MOS", "parameters": {"pattern": "cc"}},
