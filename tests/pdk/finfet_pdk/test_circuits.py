...
def test_cmp_vanilla():
    ...
    counter = len(
        [
            fname.name
            for fname in (run_dir / "2_primitives").iterdir()
            if fname.name.startswith("DP_NMOS") and fname.name.endswith(".lef")
        ]
    )
    assert (
        counter == 6
    ), f"Diff pair in comparator should have 6 variants. Found {counter}."
    if CLEANUP:
...
@pytest.mark.skip(
    reason="This test is failing. Enable in a future PR after refactoring"
)
def test_cmp_noconst():
    ...
    with (run_dir / "1_topology" / f"{name.upper()}.verilog.json").open("rt") as fp:
        ...
        for module in modules.values():
            assert (
                len(module["constraints"]) == 1
            ), "Constraints generated despise AutoConstraint"
    if CLEANUP:
        shutil.rmtree(run_dir)
        shutil.rmtree(ckt_dir)
...
def test_cmp_order():
    ...
    ckt_dir, run_dir = run_example(
        example,
        cleanup=False,
        additional_args=["--flow_stop", "3_pnr:prep"],
        log_level=LOG_LEVEL,
    )
    ...
    with (run_dir / "1_topology" / f"{name}.verilog.json").open("rt") as fp:
        ...
        assert (
            "X_MP7" in instances and "X_MP8" in instances
        ), f"MP7 or MP8 not found in {instances}"
...
def test_ota_six():
    ...
    constraints = [
        ...
        },
        {
            ...
            "template_name": "diffpair",
            "constraints": [{"constraint": "Boundary", "halo_vertical": 0.63}],
        },
        {
            "constraint": "GroupBlocks",
            "instances": ["mp5", "mp6"],
        ...
        name: [
            {
                "constraint": "Order",
                "direction": "left_to_right",
                "instances": [f"xi{k}" for k in range(5)],
            },
        ],
    }
    example = build_example(name, netlist, constraints)
...
def test_two_stage_ota():
    ...
    constraints = [
        ...
        },
        {
            "constraint": "Order",
            "direction": "top_to_bottom",
            "instances": ["xsc2", "xdp2", "xscp", "xdp", "xscn"],
            "abut": True,
        },
        {
            "constraint": "SymmetricBlocks",
            "direction": "V",
            "pairs": [["xsc2"], ["xdp2"], ["xscp"], ["xdp"], ["xscn"]],
        },
    ]
    example = build_example(name, netlist, constraints)
    run_example(example, cleanup=CLEANUP, log_level=LOG_LEVEL)
...
def test_charge_pump_switch():
    ...
    constraints = {
        ...
        "switch": [{"constraint": "DoNotIdentify", "instances": ["qp0", "qn0"]}],
    }
    example = build_example(name, netlist, constraints)
    ckt_dir, run_dir = run_example(
        example,
        ...
        cleanup=False,
        log_level=LOG_LEVEL,
        additional_args=["--flow_stop", "3_pnr:prep", "--router_mode", "bottom_up"],
    )
    name = name.upper()
    with (run_dir / "1_topology" / f"{name}.verilog.json").open("rt") as fp:
        ...
        same_template = [
            c for c in module["constraints"] if c["constraint"] == "SameTemplate"
        ]
        assert len(same_template) == 2, "Duplicate SameTemplate constraints"
        align_in_order = [
            c for c in module["constraints"] if c["constraint"] == "AlignInOrder"
        ]
    ...
    with (run_dir / "3_pnr" / "inputs" / f"{name}.pnr.const.json").open("rt") as fp:
        ...
        same_template = [
            c
            for c in charge_pump_const["constraints"]
            if c["const_name"] == "SameTemplate"
        ]
...
def test_charge_pump_switch_small():
    ...
    ckt_dir, run_dir = run_example(
        ...
        cleanup=False,
        log_level=LOG_LEVEL,
        additional_args=["--flow_stop", "1_Topology", "--router_mode", "bottom_up"],
    )
    name = name.upper()
    with (run_dir / "1_topology" / f"{name}.verilog.json").open("rt") as fp:
        hierarchy = json.load(fp)
...
def test_niwc_opamp_split():
    ...
    constraints = [
        ...
        {"constraint": "GroundPorts", "ports": ["vssx"]},
        {
            "constraint": "GroupBlocks",
            "instances": ["mtail"],
            "instance_name": "xmtail0",
            "template_name": "mtail0",
            "generator": {
                "name": "MOS",
                "parameters": {
                    "PARTIAL_ROUTING": True,
                    "single_device_connect_m1": False,
                    "legal_sizes": [{"y": 8}],
                },
            },
        },
        {
            ...
            "generator": {
                "name": "MOS",
                "parameters": {
                    "exact_patterns": [["AbBa", "BaAb", "BaAb", "AbBa"]],
                    "PARTIAL_ROUTING": True,
                },
            },
        },
        {
            "constraint": "GroupBlocks",
            "instances": ["m7a", "m8a"],
            "instance_name": "xnraila",
            "template_name": "nraila",
            "generator": {
                "name": "MOS",
                "parameters": {
                    "pattern_template": ["AbBa", "BaAb"],
                    "PARTIAL_ROUTING": True,
                    "legal_sizes": [{"y": 8}],
                },
            },
        },
        {
            "constraint": "GroupBlocks",
            "instances": ["m7b", "m8b"],
            "instance_name": "xnrailb",
            "template_name": "nrailb",
            "generator": {
                "name": "MOS",
                "parameters": {
                    "pattern_template": ["AbBa", "BaAb"],
        ...
        },
        {
            ...
            "instance_name": "xlsa",
            "template_name": "lsa",
            "generator": {"name": "MOS", "parameters": {"legal_sizes": [{"y": 4}]}},
        },
        {
            "constraint": "GroupBlocks",
            "instances": ["m3b", "m4b"],
        ...
        },
        {
            "constraint": "GroupBlocks",
            "instances": ["m5a", "m6a"],
            "instance_name": "xostagea",
            "template_name": "ostagea",
            "generator": {"name": "MOS", "parameters": {"legal_sizes": [{"y": 4}]}},
        },
        {
            "constraint": "GroupBlocks",
            "instances": ["m5b", "m6b"],
            "instance_name": "xostageb",
            "template_name": "ostageb",
            "generator": {"name": "MOS", "parameters": {"legal_sizes": [{"y": 4}]}},
        },
    ...
    ckt_dir, run_dir = run_example(
        ...
        cleanup=False,
        log_level=LOG_LEVEL,
        additional_args=["--flow_stop", "3_pnr:place"],
    )
...
def test_niwc_opamp_split_collect_pins():
    ...
    ckt_dir, run_dir = run_example(
        ...
        cleanup=False,
        log_level=LOG_LEVEL,
        additional_args=["--router_mode", "collect_pins"],
    )
...
def test_niwc_opamp_split_reuse():
    ...
    constraints = [
        ...
        {
            "constraint": "GroupBlocks",
            "instance_name": "X_M7A_M8A",
            "instances": ["M7A", "M8A"],
            "template_name": "CMC_NMOS",
            "generator": {"name": "mos", "parameters": None},
        },
        {
            "constraint": "GroupBlocks",
            "instance_name": "X_M7B_M8B",
            "instances": ["M7B", "M8B"],
            "template_name": "CMC_NMOS",
            "generator": {"name": "mos", "parameters": None},
        },
        ...
        {
            "constraint": "GroupBlocks",
            "instance_name": "X_MTAIL",
            "instances": ["MTAIL"],
            "template_name": "NMOS_3T",
            "generator": {"name": "mos", "parameters": None},
        },
        {
            "constraint": "GroupBlocks",
            "instance_name": "X_M3A_M4A",
            "instances": ["M3A", "M4A"],
            "generator": {"name": "mos", "parameters": None},
        },
        {
            "constraint": "GroupBlocks",
            "instance_name": "X_M3B_M4B",
            "instances": ["M3B", "M4B"],
            "generator": {"name": "mos", "parameters": None},
        },
        {
            "constraint": "SymmetricBlocks",
            "direction": "V",
            "pairs": [["X_M5A_M6A", "X_M5B_M6B"]],
        },
        {
            "constraint": "SymmetricBlocks",
            "direction": "V",
            "pairs": [["X_M7A_M8A", "X_M7B_M8B"]],
        },
    ]
...
def test_opamp_poor():
    ...
    constraints = [
        ...
        },
        {
            ...
            "instances": ["iloadl<0>", "iloadl<1>", "iloadr<0>", "iloadr<1>"],
        },
        {
            "constraint": "SameTemplate",
            "instances": ["idiffl<0>", "idiffl<1>", "idiffr<0>", "idiffr<1>"],
        },
        {
            "constraint": "SameTemplate",
            "instances": [
...
def test_comparator_analog():
    ...
    constraints = [
        ...
        {
            "constraint": "GroupBlocks",
            "instances": ["invp1", "invp1"],
            "instance_name": "xinv0",
        },
        {
            "constraint": "GroupBlocks",
            "instances": ["invp2", "invp2"],
            "instance_name": "xinv1",
        },
        {
        ...
        },
        {
            ...
            "order": True,
            "symmetrize": True,
            "regions": [["decap0"], ["ptail"], ["xdp"], ["xcm"], ["decap1"]],
        },
    ]
    example = build_example(name, netlist, constraints)
    run_example(
...
def test_folded_cascode():
    ...
    constraints = [
        ...
        },
        {
            ...
            "generator": {
                "name": "MOS",
                "parameters": {"add_tap": False, "legal_sizes": [{"y": 2}]},
            },
        },
        {
            "constraint": "GroupBlocks",
...
def test_binary():
    ...
    _, run_dir = run_example(
        ...
        cleanup=False,
        log_level="DEBUG",
        additional_args=["--placer_sa_iterations", "100"],
    )
    ...
    with (run_dir / "3_pnr" / f"{name.upper()}_0.json").open("rt") as fp:
        data = json.load(fp)
        terminals = set(
            [term["netName"] for term in data["terminals"] if term["netName"]]
        )
