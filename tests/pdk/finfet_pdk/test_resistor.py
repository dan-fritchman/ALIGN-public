...
def test_res_hier():
    ...
    run_example(
        example, cleanup=CLEANUP, n=1, additional_args=["--flow_stop", "2_primitives"]
    )
...
def test_res_flat():
    ...
    with (run_dir / "1_topology" / "__primitives_library__.json").open("rt") as f1:
        ...
        with (run_dir / "1_topology" / f"{name}.verilog.json").open("rt") as f2:
            ...
            assert (
                instances["XR3"]["abstract_template_name"]
                == instances["XR4"]["abstract_template_name"]
            )

            for k, v in instances.items():
                assert (
                    v["abstract_template_name"] in atn
                ), f"Abstract not found: {v['abstract_template_name']}"
    if CLEANUP:
...
def test_res_one():
    ...
    example = build_example(name, netlist, constraints)
    ckt_dir, run_dir = run_example(
        example, cleanup=False, n=1, additional_args=["--flow_stop", "1_topology"]
    )
