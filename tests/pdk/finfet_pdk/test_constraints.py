...
def test_cmp_reuse():
    ...
    ckt_dir_1, run_dir_1 = run_example(
        example_1,
        n=1,
        cleanup=False,
        log_level=LOG_LEVEL,
        additional_args=["--flow_stop", "1_topology"],
    )
    ...
    ckt_dir_2, run_dir_2 = run_example(
        example_2,
        n=1,
        cleanup=False,
        log_level=LOG_LEVEL,
...
def test_same_template_group_block():
    ...
    constraints = [
        {
            ...
            "instances": ["mn0", "mn1"],
            "instance_name": "xdiff_left",
        },
        {
            "constraint": "GroupBlocks",
            "instances": ["mn2", "mn3"],
            "instance_name": "xdiff_right",
        },
    ...
    with (run_dir / "1_topology" / f"{name.upper()}.verilog.json").open("rt") as fp:
        ...
        atn_l = [
            ...
            if inst["instance_name"] == "XDIFF_LEFT"
        ][0]
        atn_r = [
            inst["abstract_template_name"]
            for inst in instances
            if inst["instance_name"] == "XDIFF_RIGHT"
        ][0]
