...
def test_dig_primitive():
    ...
    ckt_dir, run_dir = run_example(
        example, cleanup=False, additional_args=["--flow_stop", "1_topology"]
    )
...
def test_dig_5(engine, params):
    ...
    example = build_example(name, netlist, constraints)
    run_example(
        example, n=1, cleanup=CLEANUP, log_level="DEBUG", additional_args=params
    )
