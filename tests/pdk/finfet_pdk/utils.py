...
def export_to_viewer(fn, c):
    if ALIGN_HOME:
        ...
        with open(
            pathlib.Path(ALIGN_HOME) / "Viewer" / "INPUT" / f"{fn}.json", "wt"
        ) as fp:
...
def verify_abstract_names(name, run_dir):
    ...
    with (run_dir / "1_topology" / f"{name}.verilog.json").open("rt") as fp:
        ...
        for module in verilog_json["modules"]:
            abstract_names_used += [
                i["abstract_template_name"] for i in module["instances"]
            ]
        abstract_names_used = set(abstract_names_used)
    assert abstract_names.issubset(
        abstract_names_used
    ...
    if json_file.exists():
        with json_file.open("rt") as fp:
            ...
            if area is not None and area > 0:
                ...
                print(
                    f"Target area: {area} Current area: {area_0} Current/Target: {area_0/area}"
                )
...
def plot_sa_seq(name):
    ...
    if len(data["pos_pair"]) == len(data["selected"]):
        for i in range(len(data["pos_pair"])):
            data["pos_pair"][i] = float(
                f"{int(data['pos_pair'][i])}.{int(data['selected'][i])}"
            )
            data["neg_pair"][i] = float(
                f"{int(data['neg_pair'][i])}.{int(data['selected'][i])}"
            )

    max_val = max(max(data["neg_pair"]), max(data["pos_pair"]))

    im0 = ax[0].scatter(
        data["pos_pair"], data["neg_pair"], c=data["temp"], cmap=cm, marker="."
    )
