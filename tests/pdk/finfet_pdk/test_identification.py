...
def test_collisions():
    ...
    *** case 3: DP2
    ...
    ckt_dir, run_dir = run_example(
        example, cleanup=False, n=1, additional_args=["--flow_stop", "2_primitives"]
    )
    ...
    with (filename).open("rt") as fp:
        ...
        inst = [
            key for key in instances.keys() if any([p in key for p in ["MN3", "MN4"]])
        ]
...
def test_disable_fix_merge():
    ...
    with (filename).open("rt") as fp:
        ...
        _ = dump_simplified_json(modules, filename)
        assert all(
            [k in instances for k in ["X_MN01", "X_MN02", "X_MN03", "X_MN04"]]
