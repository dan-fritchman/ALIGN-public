...
def test_check_metadata(place_on_grid_h):
    ...
    for filename in prim_dir.glob("*.json"):
        if str(filename).endswith(".gds.json") or str(filename.stem).startswith(
            "__primitives"
        ):
            continue
        with (filename).open("rt") as fp:
            primitive = json.load(fp)
            assert (
                primitive["metadata"]["constraints"][0]["constraint"] == "PlaceOnGrid"
            ), filename.stem
            assert (
                primitive["metadata"]["constraints"][0]["direction"] == "H"
            ), filename.stem
    shutil.rmtree(run_dir)
    shutil.rmtree(ckt_dir)
...
def test_cmp_on_grid(place_on_grid_h):
    ...
    example = build_example(name, netlist, constraints)
    run_example(
        example, cleanup=CLEANUP, additional_args=["--placer_sa_iterations", "10"]
    )
...
def test_cmp_on_grid_ilp(place_on_grid_h):
    ...
    run_example(
        example,
        cleanup=CLEANUP,
        additional_args=["--place_using_ILP", "--placer_sa_iterations", "10"],
    )
...
def test_bias_generator(disable_tap):
    ...
    constraints = {
        "nbias_gen": [
            ...
            {"constraint": "PowerPorts", "ports": ["vcca"], "propagate": True},
            {"constraint": "GroundPorts", "ports": ["vssx"], "propagate": True},
            {"constraint": "DoNotRoute", "nets": ["vssx", "vcca"]},
            {
                "constraint": "GroupBlocks",
                "instances": ["mp0"],
                "instance_name": "xmp0",
                "generator": {"name": "MOS", "parameters": {"legal_sizes": [{"y": 2}]}},
            },
            {
                "constraint": "GroupBlocks",
                "instances": ["qp1"],
        ...
        name: [
            ...
            {"constraint": "PowerPorts", "ports": ["vccd", "vcca"], "propagate": True},
            {"constraint": "GroundPorts", "ports": ["vssx"], "propagate": True},
            {"constraint": "DoNotRoute", "nets": ["vssx", "vccd", "vcca"]},
            {
                "constraint": "GroupBlocks",
                "instances": ["nand0", "nand1", "inv09"],
                "instance_name": "xdig",
            },
            {
                "constraint": "GroupBlocks",
                "instances": ["R0", "i12", "i21", "i13", "i20", "qn3"],
                "instance_name": "xoutp",
            },
            {
                "constraint": "GroupBlocks",
                "instances": ["R6", "i15", "i35", "i36", "i22", "i16"],
                "instance_name": "xoutn",
            },
            {
                "constraint": "GroupBlocks",
                "instances": ["i25", "i32"],
                "instance_name": "xbiasp",
            },
            {
                "constraint": "GroupBlocks",
                "instances": ["i24", "i27"],
                "instance_name": "xbiasn",
            },
            {
                "constraint": "Floorplan",
                "order": True,
