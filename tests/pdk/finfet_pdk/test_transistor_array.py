...
def test_uno_drc(n_row, n_col, nf, device_type, ports):
    ...
    if c.drc.num_errors > 0 or len(c.rd.opens) > 0 or len(c.rd.shorts) > 0:
        ...
        assert (
            False
        ), f"{get_test_id()} DRC ports:{ports} type:{device_type} nf:{nf}, n_col:{n_col} n_row:{n_row}"
...
def test_unit_interleave_pattern():
    ...
    assert [["A", "B"], ["B", "A"]] == MOSGenerator.interleave_pattern(2, 2)
    assert [["A", "B", "A"], ["B", "A", "B"]] == MOSGenerator.interleave_pattern(2, 3)
    ...
    assert [
        ["A", "b", "A", "b"],
        ["B", "a", "B", "a"],
    ] == MOSGenerator.interleave_pattern(2, 4, pattern_template=["Ab", "Ba"])

    assert [
        ["A", "b", "B", "a"],
        ["B", "a", "A", "b"],
    ] == MOSGenerator.interleave_pattern(2, 4, pattern_template=["AbBa", "BaAb"])
