...
def test_aspect_ratio_low():
    ...
    name = name.upper()
    with (
        run_dir / "3_pnr" / "Results" / f"{name}_0.scaled_placement_verilog.json"
    ).open("rt") as fp:
        verilog_json = json.load(fp)
        modules = {
            module["concrete_name"]: module for module in verilog_json["modules"]
        }
        cn = f"{name}_0"
...
def test_boundary_halo():
    ...
    netlist = circuits.cascode_amplifier(name)
    constraints = [
        {"constraint": "Boundary", "halo_vertical": 0.63, "halo_horizontal": 0.216}
    ]
    example = build_example(name, netlist, constraints)
    run_example(example)
...
def test_placecloser():
    ...
    with (
        ...
        for inst in modules[cn]["instances"]:
            ...
            if "xi2" in inst["instance_name"].lower():
                xi2_ox = inst["transformation"]["oX"]
        assert (xi0_ox < xi1_ox and xi0_ox < xi2_ox) or (
            xi0_ox > xi1_ox and xi0_ox > xi2_ox
...
def test_group_blocks():
    ...
    with (run_dir / "1_topology" / f"{name.upper()}.verilog.json").open("rt") as fp:
        ...
        module = [
            ...
            if module["name"].startswith("MYGROUP")
        ][0]
    ...
    with (
        ...
        for inst in modules[cn]["instances"]:
            ...
            if "xi2" in inst["instance_name"].lower():
                xi2_ox = inst["transformation"]["oX"]
        assert (
            xi0_ox < xi1_ox and xi2_ox < xi1_ox
...
def test_enumerate():
    ...
    assert (
        num_seq_pairs == 4
    ), f"4 seq pairs expected but only {num_seq_pairs} seq pairs generated"
    shutil.rmtree(run_dir)
    shutil.rmtree(ckt_dir)
...
def test_enumerate_2():
    ...
    assert (
        num_seq_pairs == 576
    ), f"576 seq pairs expected but only {num_seq_pairs} seq pairs generated"
    shutil.rmtree(run_dir)
    shutil.rmtree(ckt_dir)
...
def test_power_propagation():
    ...
    constraints = {
        "ckt_a": [{"constraint": "PowerPorts", "ports": ["vcca"], "propagate": False}],
        name: [{"constraint": "PowerPorts", "ports": ["vccx"], "propagate": True}],
    }
    example = build_example(name, netlist, constraints)
    ckt_dir, run_dir = run_example(
        example, cleanup=False, additional_args=["--flow_stop", "3_pnr:prep"]
    )
    name = name.upper()
