...
def test_sizing2(ckt):
    assert {ele.name for ele in ckt.elements} == {
        "X_MN3_MN4",
        "X_XI2",
        "X_MN1",
        "X_XI0",
...
def test_sizing3(ckt):
    ...
    assert ckt.parent.find(
        "DP_NMOS_B_77995687"
    ), f"Check hash is consistent? {[subckt.name for subckt in ckt.parent if 'DP_NMOS_B' in subckt.name]}"
    assert (
        ckt.parent.find("DP_NMOS_B_77995687").elements[0].parameters["W"] == "3.6E-07"
    )
        ...
        len(
            {
                v.name
                for v in ckt.parent
                if isinstance(v, SubCircuit) and "INV_B" in v.name
            }
        )
        == 2
    )
        ...
        len(
            {
                v.name
                for v in ckt.parent
                if isinstance(v, SubCircuit) and "CMB_PMOS" in v.name
            }
        )
        == 1
    )
