...
def test_array_gen_ro_flat():
    ...
    assert ckt_lib.find(
        "ARRAY_TEMPLATE"
    ), f"{set([inst.name for inst in ckt_lib.find('ARRAY_TEMPLATE').elements])}"
    assert set([inst.name for inst in ckt_lib.find("ARRAY_TEMPLATE").elements]) == {
        "X_MP0",
        "X_MN0",
    }
    array_insts = [
    ...
    assert [
        inst.name for inst in ckt_lib.find("ARRAY_HIER_VCCX").elements
    ] == array_insts
    clean_data(name)
...
def test_array_limit():
    ...
    constraints = [
        {
            "constraint": "DoNotUseLib",
            "libraries": ["STAGE2_INV", "INV", "DP_NMOS", "DP_NMOS_B"],
        }
    ]
    example = build_example(name, netlist, constraints)
    ...
    all_modules = {
        module.name
        for module in cktlib
        if isinstance(module, SubCircuit) and len(module.elements) > 1
    }
    assert all_modules == {name.upper()}
    align_const = cktlib.find(name).constraints.dict()["__root__"][-1]
    assert set(align_const["instances"]) == {
        "X_MN1",
        "X_MN2",
        "X_MN3",
        "X_MN4",
...
def test_mismatch():
    ...
    constraints = [{"constraint": "DoNotUseLib", "libraries": ["DP_NMOS", "DP_NMOS_B"]}]
    example = build_example(name, netlist, constraints)
    ckt_lib, prim_lib = compiler_input(example, name, pdk_path, config_path)
    annotate_library(ckt_lib, prim_lib)
    ckt = ckt_lib.find(name)
...
def test_end_search_at_net_convergence():
    ...
    assert set([inst.name for inst in ckt_lib.find("ARRAY_TEMPLATE1").elements]) == {
        "X_MN2A"
    }
    assert set([inst.name for inst in ckt_lib.find("ARRAY_TEMPLATE2").elements]) == {
        "X_MN3A"
    }
...
def test_end_search_at_inst_convergence():
    ...
    array_cl = process_arrays(ckt, dict())
    array1 = array_cl.find_array("S", ["VSSX"])
    assert array1 == [["XI0", "X_MN1"], ["XI0", "X_MN2"], ["XI0", "X_MN3"]]
    array_cl.add_align_block_const()
    array_cl.add_new_array_hier()
    assert ckt.get_element("X_ARRAY_HIER_S")
    assert ckt_lib.find("ARRAY_HIER_S")
