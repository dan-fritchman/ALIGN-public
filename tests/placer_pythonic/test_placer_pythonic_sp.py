...
from align.pnr.placer_pythonic_sp import (
    enumerate_sequence_pairs,
    enumerate_block_variants,
    place_sequence_pair,
)
        ...
        for name, instance_bbox in wires.items():
            ...
            for instance, bbox in instance_bbox:
                ...
                for (tag, axis), offset in zip(
                    ...
                    bb[tag + axis] = (
                        model.var_by_name(f"{instance}_ll{axis}").x
                        + offset
                        + (size[axis] - 2 * offset)
                        * model.var_by_name(f"{instance}_f{axis}").x
                    )
...
def test_place_sequence_pair_1():
    ...
    wires = {"net1": [("M0", (1, 2, 2, 2.25)), ("M1", (3, 1, 4, 1.25))]}
...
def test_place_sequence_pair_boundary():
    ...
    assert not place_sequence_pair(
        constraints, instance_map, instance_sizes, sequence_pair
    )
