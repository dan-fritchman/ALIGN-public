...
def test_ru_zero():
    ...
    cv.bbox = transformation.Rect(
        *[0, 0, 10 * cv.pdk["M1"]["Pitch"], 10 * cv.pdk["M2"]["Pitch"]]
    )
...
def test_ru_1():
    ...
    cv.bbox = transformation.Rect(
        *[0, 0, 6 * cv.pdk["M1"]["Pitch"], 20 * cv.pdk["M2"]["Pitch"]]
    )
    run_postamble(name, cv, max_errors=0)
...
def test_ru_3():
    ...
    assert (
        len(center_lines) == 2
    ), f"Two M3 tracks suffice to complete routing {len(center_lines)}"
    assert (
        segment_count == 2
    ), f"Two M3 segments suffice to complete routing {segment_count}"
...
def test_ru_exclude_m1():
    ...
    cv.bbox = transformation.Rect(
        *[0, 0, 8 * cv.pdk["M1"]["Pitch"], 10 * cv.pdk["M2"]["Pitch"]]
    )
...
def test_ru_exclude_per_net():
    ...
    cv.bbox = transformation.Rect(
        *[0, 0, 16 * cv.pdk["M1"]["Pitch"], 10 * cv.pdk["M2"]["Pitch"]]
    )
        ...
        constraints=[
            {
                ...
                "max_layer": "M3",
                "customize": [
                    {"nets": ["A"], "min_layer": "M2", "max_layer": "M3"},
...
def test_ru_allow_ports_on_excluded_layers():
    ...
    cv.bbox = transformation.Rect(
        *[0, 0, 10 * cv.pdk["M1"]["Pitch"], 16 * cv.pdk["M2"]["Pitch"]]
    )
    ...
    run_postamble(
        ...
        cv,
        max_errors=0,
        constraints=[{"constraint": "Route", "min_layer": "M2", "max_layer": "M2"}],
    )
...
def test_ru_staggered_m1():
    ...
    cv.bbox = transformation.Rect(
        *[0, 0, 8 * cv.pdk["M1"]["Pitch"], 16 * cv.pdk["M2"]["Pitch"]]
    )
...
def test_ru_no_extra_routing_on_m1():
    ...
    constraints = [{"constraint": "Route", "min_layer": "M2", "max_layer": "M3"}]
