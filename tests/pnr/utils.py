...
def set_bbox(cv):
    ...
    cv.bbox.urx = (cv.bbox.urx // cv.pdk["M1"]["Pitch"]) * cv.pdk["M1"][
        "Pitch"
    ] + cv.pdk["M1"]["Pitch"]
...
def run_postamble(nm, cv, max_errors=0, constraints=None):
    ...
    _ = cv.gen_data(
        run_drc=True, run_pex=False, nets_allowed_to_be_open=nets_allowed_to_be_open
    )
    assert (
        cv.drc.num_errors == 0
    ), f"Unit test has design rule violations: {cv.drc.errors}"
    assert len(cv.rd.shorts) == 0, f"Unit test has shorts: {cv.rd.shorts}"

    bbox = cv.bbox.toList()
    terminals = cv.removeDuplicates(silence_errors=True)
    terminals.insert(
        0, {"layer": "Boundary", "netName": None, "rect": bbox, "netType": "drawing"}
    )
    ...
    with open(
        pathlib.Path(ALIGN_HOME) / "Viewer" / "INPUT" / f"{nm.upper()}.json", "wt"
    ) as fp:
    ...
    gen_lef.json_lef(
        run_dir / "2_primitives" / f"{ctn}.json", ctn, bodyswitch=1, blockM=0, p=cv.pdk
    )
    ...
    *,
    ...
    for result in results:
        ...
        for (k, v) in variants.items():
            ...
            assert (
                v["errors"] <= max_errors
            ), f"{example.name} ({k}):Number of DRC errors: {str(v['errors'])}"
