...
def aux(design, subdesign=None):
    ...
    for cblk in hN.Blocks:
        ...
        tr2 = transformation.Transformation(
            oX=blk.placedBox.UR.x - blk.originBox.LL.x,
...
def results_directory_missing(design):
    ...
    """
    if "ALIGN_WORK_DIR" not in os.environ:
        return True
...
@pytest.mark.skipif(
    results_directory_missing("five_transistor_ota"),
...
@pytest.mark.skipif(
    results_directory_missing("switched_capacitor_filter"),
    reason="Necessary test collateral has not been built",
)
def test_B():
...
@pytest.mark.skipif(
    results_directory_missing("telescopic_ota"),
    reason="Necessary test collateral has not been built",
)
def test_B2():
...
@pytest.mark.skipif(
    results_directory_missing("cascode_current_mirror_ota"),
    reason="Necessary test collateral has not been built",
)
def test_C():
