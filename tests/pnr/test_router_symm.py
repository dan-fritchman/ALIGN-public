...
def check_symmetry(cv, net_a, net_b, direction):
    ...
    if terminals_a or terminals_b:
        ...
        for term_a in terminals_a:
            ...
            for term_b in terminals_b:
                ...
                if term_a["layer"] != term_b["layer"]:
                    continue
                if (
                    rect_a[lla] == rect_b[lla]
                    and rect_a[ura] == rect_b[ura]
                    and rect_a[uro] - rect_a[llo] == rect_b[uro] - rect_b[llo]
                ):
        ...
        assert (
            len(set(symmetry_lines)) == 1
        ), f"Symmetry lines do not match {symmetry_lines}"
...
def test_ru_symm_1():
    ...
    cv.bbox = transformation.Rect(
        *[0, 0, 10 * cv.pdk["M1"]["Pitch"], 8 * cv.pdk["M2"]["Pitch"]]
    )
