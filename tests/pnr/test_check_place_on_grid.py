...
def test_flatten_leaves():
    placement = {
        "leaves": [
            {"abstract_name": "INV", "bbox": [0, 0, 3000, 5000], "concrete_name": "INV"}
        ],
        "modules": [
            {
    ...
    flat_leaves_gold = [
        ...
        },
        {
            "concrete_name": "INV",
            "name": "XI2/XI0",
            "transformation": {"oX": 0, "oY": 10000, "sX": 1, "sY": -1},
        },
    ]
...
def test_check_place_on_grid():
    ...
    constraints = [
        PlaceOnGrid(
            direction="H",
            pitch=100,
            ored_terms=[OffsetsScalings(offsets=[50], scalings=[1])],
        ).dict(),
        PlaceOnGrid(
            direction="V",
            pitch=20,
            ored_terms=[OffsetsScalings(offsets=[0, 10], scalings=[1])],
        ).dict(),
    ]
    ...
    flat_leaf = {
        "name": "u0",
        "concrete_name": "CELL_0",
        "transformation": {"oX": 40, "oY": 150, "sX": 1, "sY": 1},
    }
    ...
    flat_leaf = {
        "name": "u0",
        "concrete_name": "CELL_0",
        "transformation": {"oX": 50, "oY": 150, "sX": 1, "sY": 1},
    }
    _check_place_on_grid(flat_leaf, constraints)
    ...
    with pytest.raises(AssertionError) as exe:
        flat_leaf = {
            "name": "u0",
            "concrete_name": "CELL_0",
            "transformation": {"oX": 0, "oY": 50, "sX": 1, "sY": -1},
        }
        _check_place_on_grid(flat_leaf, constraints)
    assert "does not satisfy" in str(exe.value)
    ...
    with pytest.raises(AssertionError) as exe:
        flat_leaf = {
            "name": "u0",
            "concrete_name": "CELL_0",
            "transformation": {"oX": 7, "oY": 50, "sX": 1, "sY": 1},
        }
        _check_place_on_grid(flat_leaf, constraints)
    assert "does not satisfy" in str(exe.value)
