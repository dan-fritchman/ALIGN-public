...
def run_postamble(nm, ctn, verilog_d, bbox, terminals, max_errors):
    ...
    terminals.append(
        {"layer": "Nwell", "netName": None, "rect": bbox, "netType": "drawing"}
    )
    ...
    gen_lef.json_lef(
        run_dir / "2_primitives" / f"{ctn}.json",
        ctn,
        bodyswitch=1,
        blockM=0,
        p=p,
    ...
    gen_lef.json_lef(
        run_dir / "2_primitives" / f"{ctn}.json", ctn, bodyswitch=1, blockM=0, p=p
    )

    os.chdir(run_dir)

    args = [
        "dummy_input_directory_can_be_anything",
        "-s",
        nm,
        "--flow_start",
        "3_pnr",
    ...
    for result in results:
        ...
        for (k, v) in variants.items():
            ...
            assert (
                v["errors"] <= max_errors
            ), f"{nm} ({k}):Number of DRC errors: {str(v['errors'])}"
...
def run_horizontal_wire(nm, pins, setup, max_errors, extra_y=0, add_blockage=False):
    ...
    if add_blockage:
        for i in range(1, ny):
            c.addWire(
                m2, None, i, (nx // 2 - 2, 1), (nx // 2 + 2, -1), netType="blockage"
            )
    ...
    instance = {
        ...
        "fa_map": [
            {"formal": actual + str(i), "actual": actual}
            for actual in pins
            for i in range(2)
        ],
    }
...
def test_four_horizontal_wires_extend(setup):
    run_horizontal_wire(
        get_test_id(), ["A", "B", "C", "D"], setup, max_errors=0, extra_y=1
    )
...
def test_two_horizontal_wires_with_obstacles(setup):
    run_horizontal_wire(
        get_test_id(), ["A", "B"], setup, max_errors=0, add_blockage=True
    )


def test_three_horizontal_wires_with_obstacles(setup):
    run_horizontal_wire(
        get_test_id(), ["A", "B", "C"], setup, max_errors=0, add_blockage=True
    )


def test_four_horizontal_wires_with_obstacles(setup):
    run_horizontal_wire(
        get_test_id(), ["A", "B", "C", "D"], setup, max_errors=1, add_blockage=True
    )
...
def run_vertical_wire(nm, pins, setup, max_errors, extra_x=0, add_blockage=False):
    ...
    if add_blockage:
        for i in range(1, nx):
            c.addWire(
                m1, None, i, (ny // 2 - 2, 1), (ny // 2 + 2, -1), netType="blockage"
            )
...
def test_four_vertical_wires_with_obstacles(setup):
    run_vertical_wire(
        get_test_id(), ["A", "B", "C", "D"], setup, max_errors=0, add_blockage=True
    )
...
def run_diagonal_wire(nm, pins, setup, max_errors, nx=20, ny=20, add_blockage=False):
    ...
    for i, actual in enumerate(pins):
        for j, xoff in enumerate([1, nx - len(pins)]):
            ...
            if add_blockage:
                c.addWire(
                    m1, None, x, (ny // 2 - 2, 1), (ny // 2 + 2, -1), netType="blockage"
                )
...
def test_four_diagonal_wires_20x5(setup):
    run_diagonal_wire(
        get_test_id(), ["A", "B", "C", "D"], setup, max_errors=0, nx=20, ny=5
    )
...
def test_ten_diagonal_wires_with_obstacles(setup):
    run_diagonal_wire(
        ...
        ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J"],
        setup,
        max_errors=3 if bypass_errors else 0,
        add_blockage=True,
    )  # should route
