...
def mock_circuit():
    ...
    with types.set_context(library):
        ...
        dummy_sub = SubCircuit(name="dummy", pins=["D", "G", "S", "B"])
...
@pytest.mark.parametrize(
    "results_file",
    (pathlib.Path(__file__).parent.parent / "files" / "test_results").glob(
        "*.pnr.const.json"
    ),
...
def test_group_block_hsc(results_file, mock_circuit):
    ...
    with open(tmp_file, "w") as outfile:
        json.dump(
            PnRConstraintWriter().map_valid_const(
                mock_circuit.constraints, mock_circuit
            ),
            outfile,
            indent=4,
        )
