...
def test_m1_pex(setup):
    c = setup
    c.terminals = [
        {"layer": "M1", "netName": "x", "rect": [0, 0, 100, 300], "netType": "drawing"}
    ]
    c.gen_data()
...
def test_m2_pex(setup):
    c = setup
    c.terminals = [
        {"layer": "M2", "netName": "x", "rect": [0, 0, 300, 100], "netType": "drawing"}
    ]
    c.gen_data()
    assert len(c.pex.netCells) == math.ceil(300 / c.pdk["Poly"]["Pitch"])
...
def test_via_pex2(setup):
    c = setup
    for (i, nm) in product([5, 7, 9], ["a"]):
        c.addWire(c.m1, nm, i, (0, -1), (0, 1))
    for (i, nm) in product([1, 3, 5, 7, 9], ["a"]):
