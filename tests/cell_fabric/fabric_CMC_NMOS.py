...
class CanvasNMOS(Canvas):
    def __init__(self, gate_u, fin_u, fin_u1):
        ...
        self.m1Width = 32
        self.m2Pitch = (
            84  ### Can be directly used from DRM (usually twice of the fin pitch)
        )
        self.m2Width = 32
        ...
        self.v_enclosure = 20
        self.fin_enclosure = (
            self.finPitch - self.finWidth
        ) // 2  ### Fin enclosure by active
        ...
        self.gate = int(
            round(gate_u + 2 * self.gateDummy)
        )  #### Total number of gates per unit cell
        self.extension_x = (
        ...
        self.m0 = self.addGen(
            Wire(
                ...
                clg=UncoloredCenterLineGrid(
                    pitch=self.m0Pitch, width=self.m0Width, offset=self.m0Pitch // 2
                ),
                spg=EnclosureGrid(
                    pitch=self.activePitch,
        ...
        self.m1 = self.addGen(
            Wire(
                ...
                spg=EnclosureGrid(
                    ...
                    stoppoint=self.m2Width // 2 + self.v_enclosure,
                    check=True,
                ),
            )
        )
        ...
        self.m2 = self.addGen(
            Wire(
                ...
                spg=EnclosureGrid(
                    pitch=self.m1Pitch,
                    offset=self.m1Pitch // 2,
                    stoppoint=self.m1Width // 2 + self.v_enclosure,
                    check=True,
                ),
            )
        )
        ...
        self.pl = self.addGen(
            Wire(
                ...
                "v",
                clg=UncoloredCenterLineGrid(
                    pitch=self.plPitch, width=self.plWidth, offset=self.plOffset
                ),
        ...
        self.fin = self.addGen(
            Wire(
                ...
                "h",
                clg=UncoloredCenterLineGrid(
                    pitch=self.finPitch, width=self.finWidth, offset=self.finOffset
                ),
        ...
        self.RVT = self.addGen(
            Wire(
                ...
                "h",
                clg=UncoloredCenterLineGrid(
                    pitch=self.RVTPitch, width=self.RVTWidth, offset=self.RVTOffset
                ),
                spg=SingleGrid(pitch=self.plPitch),
            )
        )
        ...
        self.v0 = self.addGen(
            Via(
                "v0",
                "via0",
                h_clg=UncoloredCenterLineGrid(
                    pitch=self.v0Pitch, width=self.v0Width, offset=v0x_offset
                ),
                v_clg=self.m1.clg,
            )
        )
...
class UnitCell(CanvasNMOS):
    def unit(self, x, y, x_cells, y_cells, fin_u, gate_u):
        ...
        #####   Derived parameters   #####
        fin = int(
            round(fin_u + 2 * self.finDummy)
        )  #### Total number of fins per unit cell
        ...
        m1Length = (
            self.m2Width / 2 + self.v_enclosure + self.m2Pitch * ((fin_u + 2) // 2)
        )
        m1PCLength = (
            self.m2Width + 2 * self.v_enclosure + self.m2Pitch * ((fin_u + 3) // 2)
        )
        ...
        for i in range(self.gate):
            ...
            self.addWire(
                ...
                xx,
                (grid_y0, -1),
                (grid_y1, 1 if y == y_cells - 1 else -1),
            )
        ...
        for i in [self.gateDummy - 1, self.gateDummy + gate_u - 1]:
            ...
            for j in range(cont_no):
                ...
                grid_yy = (
                    j
                    + y * ((fin_u + 2 * self.finDummy) * self.finPitch) // self.v0Pitch
                )
        ...
        if x_cells - 1 == x:
            ...
            for (net, i, y_offset) in itertools.chain(
                *[[(net, p, q) for p in P] for (net, P, q) in triples]
            ):
