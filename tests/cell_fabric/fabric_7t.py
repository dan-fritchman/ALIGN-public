...
class Canvas(AbstractCanvas):
    def __init__(self):
        ...
        self.nd = self.addGen(
            Region("nd", "ndiff", h_grid=SingleGrid(pitch=ndPitch), v_grid=self.pl.clg)
        )

        self.pd = self.addGen(
            Region("pd", "pdiff", h_grid=SingleGrid(pitch=pdPitch), v_grid=self.pl.clg)
        )
        ...
        self.m2 = self.addGen(
            Wire(
                ...
                clg=UncoloredCenterLineGrid(
                    width=m2Width, pitch=m2Pitch, repeat=self.m2PerUnitCell
                ),
        ...
        self.m3 = self.addGen(
            Wire(
                ...
                spg=EnclosureGrid(
                    ...
                    stoppoint=self.unitCellHeight // 2 - m2Pitch,
                ),
            )
        )
    ...
    def nunit(self):
        ...
        self.addWire(
            ...
            ds0,
            (0, 0),
            (0 * self.m2PerUnitCell + 1, -1),
            (1 * self.m2PerUnitCell - 1, 1),
        )
        self.addWire(
            self.m1,
            "g",
