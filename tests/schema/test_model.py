...
def test_base_model_case_insensitivity(library):
    ...
    with set_context(library):
        MyDevice = Model(
            name="MyDevice",
            pins=["d", "S"],
            parameters={"PARAM1": "nf*4", "param2": "2"},
        )
...
def test_derived_model_case_insensitivity(testmos):
    with set_context(testmos.parent):
        DerivedMOS = Model(
            name="DerivedMOS", base="testmos", parameters={"param1": "nf*4"}
        )
...
def test_derived_model_new_parameters(testmos):
    with set_context(testmos.parent):
        DerivedMOS = Model(
            name="DERIVEDMOS",
            base="testmos",
            parameters={"PARAM1": "NF*6", "PARAM3": "NF*4"},
        )
    assert DerivedMOS.parameters == {"PARAM1": "NF*6", "PARAM2": "2", "PARAM3": "NF*4"}
...
def test_model_str_casting(library):
    ...
    with set_context(library):
        MyDevice = Model(
            name="MyDevice", pins=["D", "S"], parameters={"PARAM1": 1.0, "PARAM2": 2}
        )
