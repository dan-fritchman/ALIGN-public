...
def test_subckt_definition(library, test_ckt):
    ...
    with set_context(test_ckt.elements):
        ...
        inst = Instance(
            name="X1", model="subckt", pins={"PIN1": "NET10", "PIN2": "NET12"}
        )
    assert inst.name == "X1"
    ...
    with set_context(test_ckt.elements):
        ...
        inst = Instance(
            ...
            model="subckt",
            pins={"PIN1": "NET10", "PIN2": "NET12"},
            parameters={"param1": "2", "param3": "1e-16"},
        )
...
def test_subckt_instantiation(library, test_ckt):
    ...
    with set_context(subckt.elements):
        ...
        X2 = Instance(
            name="X2", model="TwoTerminalDevice", pins={"A": "NET2", "B": "NET3"}
        )
