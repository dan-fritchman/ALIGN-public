...
"""
    ...
    assert all(
        tok1.type == tok2.type and tok1.value == tok2.value
        for tok1, tok2 in zip(tokens, SpiceParser._generate_tokens(setup_basic))
    ), tokens
...
def test_parser_basic(setup_basic, parser):
    with set_context(parser.library):
        parser.library.append(
            SubCircuit(
                name="TESTDEV", pins=["+", "-"], parameters={"X": "1F", "Y": "0.1"}
            )
        )
...
def test_parser_realistic(setup_realistic, parser):
    ...
    assert [x.name for x in parser.circuit.elements] == [
        "R1",
        "R2",
        "M1",
        "M2",
...
def test_parser_annotation(setup_annotation, parser):
    ...
    assert [x.name for x in diffamp.elements] == [
        "R1",
        "R2",
        "M1",
        "M2",
...
def test_model(parser):
    ...
    assert list(nmos_rvt.parameters.keys()) == [
        "W",
        "L",
        "NFIN",
        "NF",
...
def test_ota_sp_parsing(parser):
    with open(
        (pathlib.Path(__file__).parent.parent / "files" / "ota.sp").resolve()
    ) as fp:
        parser.parse(fp.read())
    ota = parser.library.find("OTA")
    assert ota is not None
