    ...
    with set_context(circuit.elements):
        ...
        circuit.elements.append(
            Instance(
                name="X4",
                model="CustomDevice",
                pins={"A": "NET3", "B": "NET1", "C": "NET2"},
            )
        )
        circuit.elements.append(
            Instance(
                name="X5", model="TwoTerminalDevice", pins={"A": "net01", "B": "net00"}
            )
        )
        circuit.elements.append(
            Instance(
                name="X6", model="TwoTerminalDevice", pins={"A": "net02", "B": "net00"}
            )
        )
        circuit.elements.append(
            Instance(
                name="X7", model="TwoTerminalDevice", pins={"A": "NET3", "B": "net03"}
            )
        )
    return circuit
...
def ota():
    parser = SpiceParser()
    with open(
        (pathlib.Path(__file__).parent.parent / "files" / "ota.cir").resolve()
    ) as fp:
...
def primitives():
    ...
    parser = SpiceParser()
    with open(
        (pathlib.Path(__file__).parent.parent / "files" / "basic_template.sp").resolve()
    ) as fp:
        parser.parse(fp.read())
...
def test_netlist(circuit):
    with set_context(circuit.elements):
        X1 = circuit.elements.append(
            Instance(
                name="X1", model="TwoTerminalDevice", pins={"A": "NET1", "B": "NET2"}
            )
        )
        X2 = circuit.elements.append(
            Instance(
                name="X2",
                model="ThreeTerminalDevice",
                pins={"A": "NET1", "B": "NET2", "C": "NET3"},
            )
        )
...
def test_find_subgraph_matches(simple_circuit, matching_subckt):
    ...
    with set_context(simple_circuit.parent):
        subckt2 = SubCircuit(
            name="test_subckt2", pins=["PIN1", "PIN2", "PIN3", "PIN4", "PIN5"]
        )
    with set_context(subckt2.elements):
        subckt2.elements.append(
            Instance(
                name="X1",
                model="ThreeTerminalDevice",
                pins={"A": "PIN1", "B": "PIN3", "C": "PIN4"},
            )
        )
        subckt2.elements.append(
            Instance(
    ...
    with set_context(subckt3.elements):
        subckt3.elements.append(
            Instance(
                name="X1", model="TwoTerminalDevice", pins={"A": "PIN1", "B": "PIN2"}
            )
        )
        subckt3.elements.append(
            Instance(
                name="X2", model="TwoTerminalDevice", pins={"A": "PIN3", "B": "PIN4"}
            )
        )
...
def test_replace_matching_subgraph(simple_circuit, matching_subckt):
    ...
    assert all(x in netlist.edges.data("pin") for x in new_edges), netlist.edges.data(
        "pin"
    )
...
def test_replace_matching_subckts(ota, primitives):
    ...
    primitives.sort(
        key=lambda x: len(x.elements) * 10000 - 100 * len(x.pins) + len(x.nets),
        reverse=True,
    )
...
def test_flatten(heirarchical_ckt):
    ...
    assert all(
        element.parameters["MYPARAMETER"] == myparametermap[element.name]
