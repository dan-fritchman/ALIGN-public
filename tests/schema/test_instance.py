...
def test_instance_model(circuit):
    with set_context(circuit.elements):
        ...
        with pytest.raises(Exception):
            M1 = Instance(
                name="M1", pins={"D": "NET01", "G": "NET02", "S": "NET03", "B": "NET04"}
            )
        with pytest.raises(Exception):
            M1 = Instance(
                name="M1",
...
def test_instance_json(circuit):
    ...
    assert (
        M1.json()
        == '{"model": "TESTMOS", "name": "M1", "pins": {"D": "NET01", "G": "NET02", "S": "NET03", "B": "NET04"}, "parameters": {"PARAM1": "NF*4", "PARAM2": "2"}, "abstract_name": null}'
    )
