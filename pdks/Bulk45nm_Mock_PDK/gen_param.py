...
def limit_pairs(pairs):
    ...
    if len(pairs) > 12:
        ...
        for l in log10_aspect_ratios:
            best_pair = min(
                (abs(log10(newy) - log10(newx) - l), (newx, newy))
...
def add_primitive(primitives, block_name, block_args, generator_constraint):
    ...
    if block_name in primitives:
        ...
        if not primitives[block_name] == block_args:
            logger.warning(
                f"Distinct devices mapped to the same primitive {block_name}: \
        ...
        if "x_cells" in block_args and "y_cells" in block_args:
            ...
            if generator_constraint is not None:
                ...
                if generator_parameters is not None:
                    ...
                    if exact_patterns is not None:
                        legal_size_set = set(
                            construct_sizes_from_exact_patterns(exact_patterns).keys()
                        )

            if legal_size_set is None and legal_sizes is None:
                pairs = limit_pairs(
                    (pairs)
                )  # call limit_pairs if there aren't constraints
            ...
            for newx, newy in pairs:
                ...
                if legal_sizes is not None or legal_size_set is not None:
                    if ok:
                        logger.debug(
                            f"Adding matching primitive of size {newx} {newy} {generator_constraint}"
                        )
                    else:
                        logger.debug(
                            f"Not adding primitive of size {newx} {newy} because it doesn't match {generator_constraint}"
                        )
...
def gen_param(subckt, primitives, pdk_dir):
    ...
    if generator_name == "CAP":
        ...
        assert (
            size <= design_config["max_size_cap"]
        ), f"caps larger than {design_config['max_size_cap']}fF are not supported"
        ...
        block_args = {"primitive": generator_name, "value": [int(length), int(width)]}
        add_primitive(primitives, block_name, block_args, generator_constraint)

    elif generator_name == "RES":
        assert float(values["VALUE"]) or float(
            values["R"]
        ), f"unidentified size {values['VALUE']} for {block_name}"
        if "R" in values:
        ...
        block_args = {"primitive": generator_name, "value": (height, float(size))}
        add_primitive(primitives, block_name, block_args, generator_constraint)
        ...
        if "vt_type" in design_config:
            vt = [
                vt.upper()
                for vt in design_config["vt_type"]
                if vt.upper() in subckt.elements[0].model
            ]
        mvalues = {}
        for ele in subckt.elements:
            mvalues[ele.name] = ele.parameters
        ...
        for key in mvalues:
            assert (
                mvalues[key]["W"] != str
            ), f"unrecognized size of device {key}:{mvalues[key]['W']} in {block_name}"
            width = int(float(mvalues[key]["W"]) * 1e9 / design_config["Fin_pitch"])
        ...
        if "NF" in mvalues[device_name].keys():
            for key in mvalues:
                assert int(
                    mvalues[key]["NF"]
                ), f"unrecognized NF of device {key}:{mvalues[key]['NF']} in {block_name}"
                assert (
                    int(mvalues[key]["NF"]) % 2 == 0
                ), f"NF must be even for device {key}:{mvalues[key]['NF']} in {block_name}"
        ...
        if "M" in mvalues[device_name].keys():
            for key in mvalues:
                assert int(
                    mvalues[key]["M"]
                ), f"unrecognized M of device {key}:{mvalues[key]['M']} in {block_name}"
                if (
                    "PARALLEL" in mvalues[key].keys()
                    and int(mvalues[key]["PARALLEL"]) > 1
                ):
                    mvalues[key]["PARALLEL"] = int(mvalues[key]["PARALLEL"])
                    mvalues[key]["M"] = int(mvalues[key]["M"]) * int(
                        mvalues[key]["PARALLEL"]
                    )
        ...
        if isinstance(size, int):
            for key in mvalues:
                assert int(float(mvalues[device_name]["W"]) * 1e9) == int(
                    float(mvalues[key]["W"]) * 1e9
                ), f"W should be same for all devices in {block_name} {mvalues}"
                size_device = int(mvalues[key]["NF"]) * int(mvalues[key]["M"])
                size = size + size_device
            no_units = ceil(
                size / (2 * len(mvalues))
            )  # Factor 2 is due to NF=2 in each unit cell; needs to be generalized
        ...
        def total_device_size(v):
            return int(float(v["W"]) * 1e9) * int(v["NF"]) * int(v["M"])

        unequal_devices = None
        if len(device_name_all) == 2:
            unequal_devices = total_device_size(
                mvalues[device_name_all[0]]
        ...
        if generator_constraint is not None:
            ...
            if generator_parameters is not None:
                ...
                if exact_patterns is not None:
                    ...
                    xval = (
                        len(exact_patterns[0][0])
                        if len(device_name_all) != 2
                        else len(exact_patterns[0][0]) // 2
                    )
        ...
        if (
            "STACK" in mvalues[device_name].keys()
            and int(mvalues[device_name]["STACK"]) > 1
        ):
