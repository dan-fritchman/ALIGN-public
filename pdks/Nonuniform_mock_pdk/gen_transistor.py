...
def mos(tx: Transistor):
    ...
    params = " ".join(
        [
            f'("dev_type" "string" "{tx.device_type}")',
            f'("vt_type" "string" "{tx.model_name}")',
            f'("nfin" "string" "{tx.nfin}")' f'("nf" "string" "{tx.nf}")',
        ]
    )
    instance = (
        {
...
def test_one():
    tx = Transistor(device_type="stack", nf=4, nfin=4, model_name="n")
...
def test_two():
    ...
    tx = Transistor(device_type="parallel", nf=4, nfin=4, model_name="p")
    data = mos(tx)
    data["globalRoutes"] = []
    data["globalRouteGrid"] = []
    print(data)
