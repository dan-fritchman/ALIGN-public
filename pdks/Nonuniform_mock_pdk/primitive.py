...
class MOSGenerator(CanvasPDK):
    ...
    def addNMOSArray(self, x_cells, y_cells, pattern, vt_type, ports, **parameters):
        self.mos_array_temporary_wrapper(
            x_cells, y_cells, pattern, vt_type, ports, **parameters
        )
    ...
    def mos_array_temporary_wrapper(
        self, x_cells, y_cells, pattern, vt_type, ports, **parameters
    ):
        ...
        assert (
            "nf" or "stack" in parameters
        ), f"Missing transistor parameter nf or stack"
        ...
        transistor_array = TransistorArray(
            unit_transistor=unit_transistor, m=m, ports=p, n_rows=x_cells
        )
    ...
    def mos_array(self, transistor_array: TransistorArray, **parameters):
        ...
        assert (
            len(transistor_array.m) <= 2
        ), f"Arrays of more than 2 devices not supported yet"
        ...
        for y in range(n_row):
            ...
            for x in range(n_col):
                pin_map = transistor_array.ports.get(
                    interleave[cnt], transistor_array.ports[1]
                )
                flip_x = 1
    ...
    def stamp_cell(self, template, instance_name, pin_map, x_offset, y_offset, flip_x):
        ...
        self.instantiated_cells.append(
            [instance_name, (x_offset, y_offset, flip_x, 1), template["instance"]]
        )
...
def test_one():
    ...
    with open(fn, "wt") as fp:
        mg.writeJSON(
            fp, draw_grid=False, run_drc=False, run_pex=False, postprocess=True
        )
