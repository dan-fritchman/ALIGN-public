...
class MOSGenerator(DefaultCanvas):
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            self.finDummy >= 8
        ), f"number of fins/width {fin} in the transistor must be less than unit cell height {self.finsPerUnitCell -2*8}"
        ...
        self.active_diff = self.addGen(
            Wire(
                ...
                spg=SingleGrid(
                    pitch=self.pdk["Poly"]["Pitch"],
                    offset=(self.gateDummy - 1) * self.pdk["Poly"]["Pitch"]
                    + self.pdk["Poly"]["Pitch"] // 2,
                ),
            )
        )
        ...
        offset_active_body = (
            (self.lFin // 2) * self.pdk["Fin"]["Pitch"]
            + self.unitCellHeight
            - self.pdk["Fin"]["Pitch"] // 2
        )
            ...
            for pin in pins:
                ...
                for j in range(self.minvias):
                    ...
                    self.addWireAndViaSet(
                        net, self.m2_updated, self.v1_x, current_track, contacts
                    )
    ...
    def _addBodyContact(self, x, y, x_cells, yloc=None, name="M1"):
        ...
        if self.shared_diff == 0:
            ...
            self.addWire(
                self.pb_diff, None, (y + 1) * h + body_v0_track, (x, 1), (x + 1, -1)
            )
        self.addWire(
            self.m1_updated,
            None,
