...
class ResGenerator(DefaultCanvas):
    def __init__(self, pdk, fin, finDummy):
        ...
        self.m1res = self.addGen(
            Wire(
                ...
                clg=ColoredCenterLineGrid(
                    colors=["c1", "c2"],
                    pitch=self.pdk["Cap"]["m1Pitch"],
                    width=self.pdk["Cap"]["m1Width"],
                ),
                spg=EnclosureGrid(
                    pitch=self.pdk["M2"]["Pitch"],
                    stoppoint=self.pdk["V1"]["VencA_L"]
                    + self.pdk["Cap"]["m2Width"] // 2,
                    check=True,
                ),
            )
        )

        self.m1res2 = self.addGen(
            Wire(
                "m1res2",
                "M1",
                "h",
                ...
                spg=EnclosureGrid(
                    pitch=self.pdk["Cap"]["m1Pitch"],
                    stoppoint=self.pdk["Cap"]["m1Width"] // 2,
                    check=False,
                ),
            )
        )

        self.m2res = self.addGen(
            Wire(
                "m2res",
                "M2",
                "h",
                clg=ColoredCenterLineGrid(
                    colors=["c1", "c2"],
                    pitch=self.pdk["M2"]["Pitch"],
                    width=self.pdk["Cap"]["m2Width"],
                ),
                spg=EnclosureGrid(
                    pitch=self.pdk["Cap"]["m1Pitch"],
                    stoppoint=self.pdk["V1"]["VencA_H"]
                    + self.pdk["Cap"]["m1Width"] // 2,
                    check=False,
                ),
            )
        )

        self.m2res2 = self.addGen(
            Wire(
                "m2res2",
                "M2",
                "h",
        ...
        self.m3res = self.addGen(
            Wire(
                ...
                clg=ColoredCenterLineGrid(
                    colors=["c1", "c2"],
                    pitch=self.pdk["Cap"]["m3Pitch"],
                    width=self.pdk["Cap"]["m3Width"],
                ),
                spg=EnclosureGrid(
                    pitch=self.pdk["M2"]["Pitch"],
                    stoppoint=self.pdk["V2"]["VencA_H"]
                    + self.pdk["Cap"]["m2Width"] // 2,
                    check=True,
                ),
            )
        ...
        self.v1res = self.addGen(
            Via(
                "v1res",
                "V1",
                h_clg=self.m2res.clg,
                v_clg=self.m1res.clg,
                WidthX=self.v1.WidthX,
                WidthY=self.v1.WidthY,
                h_ext=self.v1.h_ext,
                v_ext=self.v1.v_ext,
            )
        )
        self.v2res = self.addGen(
            Via(
                "v2res",
                "V2",
                h_clg=self.m2res.clg,
                v_clg=self.m3res.clg,
                WidthX=self.v2.WidthX,
                WidthY=self.v2.WidthY,
                h_ext=self.v2.h_ext,
                v_ext=self.v2.v_ext,
            )
        )

        self.Rboundary = self.addGen(
            Region("Rboundary", "Rboundary", h_grid=self.m2.clg, v_grid=self.m1.clg)
        )
    ...
    def addResArray(self, x_cells, y_cells, height, unit_res):
        ...
        for x in range(x_cells):
            for y in range(y_cells):
                self._addRes(
                    x, y, height, unit_res, (x == x_cells - 1) and (y == y_cells - 1)
                )
    ...
    def _addRes(self, x, y, height, unit_res, draw_boundary=True):
        ...
        y_number = int(
            ...
            * round(
                (
                    (y_length + self.pdk["Cap"]["m2Pitch"] - self.pdk["Cap"]["m2Width"])
                    / (2.0 * self.pdk["Cap"]["m2Pitch"])
                )
            )
        )

        last_y1_track = (
            (y_number - 1) * self.pdk["Cap"]["m2Pitch"] + self.pdk["M2"]["Pitch"] - 1
        ) // self.pdk["M2"]["Pitch"]
        ...
        if (y_number - 1) % 2 != last_y1_track % 2:
            last_y1_track += (
                1  # so the last color is compatible with the external view of the cell
            )
        ...
        m2n = Wire(
            self.m2res2.nm,
            self.m2res2.layer,
            self.m2res2.direction,
            clg=self.m2res2.clg.copyShift(
                self.m2res.clg.value(grid_cell_y_pitch * y)[0]
            ),
            spg=self.m2res2.spg,
        )
