...
class CapGenerator(DefaultCanvas):
        ...
        self.m2n = self.addGen(
            Wire(
                ...
                spg=EnclosureGrid(
                    ...
                    stoppoint=self.pdk["V1"]["VencA_H"] + self.pdk["M1"]["Width"] // 2,
                    check=False,
                ),
            )
        )
    ...
    def addCap(self, length, width):
        ...
        self.addRegion(
            self.boundary, "Boundary", -4, -2, last_x1_track + 4, last_y1_track + 2
        )

        self.addRegion(self.Cboundary, "Cboundary", 0, 1, last_x1_track, last_y1_track)

        logger.debug(
            f"Computed Boundary: {self.terminals[-1]} {self.terminals[-1]['rect'][2]} {self.terminals[-1]['rect'][2]%80}"
        )
