...
class MOSGenerator(DefaultCanvas):
        ...
        **kwargs,
        ...
        self.pl = self.addGen(
            Wire(
                ...
                spg=SingleGrid(
                    offset=self.pdk["M2"]["Offset"], pitch=self.unitCellHeight
                ),
            )
        )
        ...
        self.LISD = self.addGen(
            Wire(
                ...
                clg=UncoloredCenterLineGrid(
                    ...
                    width=self.pdk["Lisd"]["LisdWidth"],
                    offset=self.pdk["M1"]["Offset"],
                ),
                spg=EnclosureGrid(
                    pitch=self.unitCellHeight, offset=0, stoppoint=stoppoint, check=True
                ),
            )
        )
        ...
        self.LISDb = self.addGen(
            Wire(
                ...
                spg=EnclosureGrid(
                    pitch=self.pdk["M2"]["Pitch"],
                    offset=0,
                    stoppoint=self.pdk["M2"]["Width"] // 2 + self.pdk["V1"]["VencA_L"],
                    check=True,
                ),
            )
        )
    ...
    def _addMOS(self, x, y, x_cells, vt_type, name="M1", reflect=False, **parameters):
        ...
        for i in range(self.gatesPerUnitCell):
            self.addWire(
                self.pl,
                None,
                self.gatesPerUnitCell * x + self.gateDummy * self.shared_diff + i,
                (y, 0),
                (y, 1),
            )
        ...
        if self.shared_diff == 1:
            for i in range(1, self.finsPerUnitCell):
                self.addWire(
                    self.fin_diff,
                    None,
                    self.finsPerUnitCell * y + i,
                    0,
                    (self.gate * x_cells + 2 * self.gateDummy),
                )
        else:
            for i in range(1, self.finsPerUnitCell):
            ...
            for pin in pins:
                ...
                for j in range(self.minvias):
                    ...
                    self.addWireAndViaSet(
                        net, self.m2, self.v1, current_track, contacts
                    )
    ...
    def _connectNets(self, x_cells, y_cells):
        ...
        m3start = (
            self.shared_diff
            + (x_cells * self.gatesPerUnitCell - len(self._nets) * self.minvias) // 2
        )
