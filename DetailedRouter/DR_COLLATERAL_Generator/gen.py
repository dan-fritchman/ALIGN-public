...
"""
        ...
        v, l, u, x1, y1, x2, y2, width1, width2, cutWidth, cutHeight, l_width, u_width
    )
        ...
        for (v, l, u) in triples:
            ...
            for l_width in widths[l]:
                for u_width in widths[u]:
                    fp.write(
                        generateVia(tech, v, l, u, l_width, u_width, l_space, u_space)
                    )

    with open("arch.txt", "wt") as fp:
        fp.write(
            """Option name=gr_region_width_in_poly_pitches value={0}
Option name=gr_region_height_in_diff_pitches value={1}
""".format(
                tech["halfXGRGrid"] * 2, tech["halfYGRGrid"] * 2
            )
        )
    ...
    with open("layers.txt", "wt") as fp:
        emitLayer(
            fp, "diffusion", 0, types=["diffusion"], pgd="hor", pitch=tech["pitchDG"]
        )
        emitLayer(
            fp,
            "wirepoly",
    ...
    with open("design_rules.txt", "wt") as fp:
        ...
        for m in metals:
            ...
            fp.write(
                "Rule name={0}_{1} type={1} value={2} layer={0}\n".format(
                    m, "minete", minete
                )
            )
        fp.write("\n")
