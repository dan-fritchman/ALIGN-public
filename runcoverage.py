...
def main():
    exit_status = 0
    assert (
        pathlib.Path.cwd().resolve() == pathlib.Path(__file__).parent.resolve()
    ...
    if shutil.which("lcov") is None:
        print(
            "WARNING: `lcov` not found. Generating coverage for python components only."
        )
    elif not CMAKE_BINARY_DIR or not CMAKE_SOURCE_DIR:
        print(
            "WARNING: CPP Source / Binary information not found. Generating coverage for python components only."
        )
        print(
            "         Run `pip install -e .[test] --no-build-isolation --install-option='-DCODE_COVERAGE=ON' --install-option='-DBUILD_TESTING=ON'` to instrument cpp code."
        )
    elif next(pathlib.Path(CMAKE_BINARY_DIR).glob("**/*.gcno"), None) is None:
        print(
            "WARNING: Could not find any .gcno files. Generating coverage for python components only."
        )
        print(
            "         Run `pip install -e .[test] --no-build-isolation --install-option='-DCODE_COVERAGE=ON' --install-option='-DBUILD_TESTING=ON'` to instrument cpp code."
        )
        ...
        print(
            "INFO: Code coverage for cpp extension has been enabled. Please see coverage-reports/cpp."
        )
        GCOV_ENABLED = True
    ...
    if GCOV_ENABLED:
        ret = subprocess.run(
            " ".join(["lcov", "--directory", CMAKE_BINARY_DIR, "--zerocounters"]),
            shell=True,
        )
        if not exit_status:
    ...
    ret = subprocess.run(
        " ".join(
            [
                ...
                "-n",
                MAX_JOBS,  # pytest-xdist options
                "--cov-report",
                f"html:{output_dir}/python",
                "--cov=align",  # pytest-cov options
                *argv,
            ]
        ),
        shell=True,
    ...
    ret = subprocess.run(
        " ".join(
            [
                ...
                "--runnightly",
                "-k",
                "telescopic_ota_guard_ring or switched_capacitor_filter",
                "-n",
                MAX_JOBS,  # pytest-xdist options
                "--cov-report",
                f"html:{output_dir}/python",
