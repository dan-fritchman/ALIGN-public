...
class DefaultCanvas(Canvas):
    def __init__(self, pdk, *, check=True):
        ...
        self.boundary = self.addGen(
            Region("boundary", "Boundary", h_grid=self.m2.clg, v_grid=self.m1.clg)
        )
    ...
    def _get_metal_width(self, layer):
        return (
            self.pdk[layer]["Width"][0]
            if isinstance(self.pdk[layer]["Width"], list)
            else self.pdk[layer]["Width"]
        )

    def _get_metal_pitch(self, layer):
        return (
            self.pdk[layer]["Pitch"][0]
            if isinstance(self.pdk[layer]["Pitch"], list)
            else self.pdk[layer]["Pitch"]
        )

    def _get_metal_offset(self, layer):
        return (
            self.pdk[layer]["Offset"][0]
            if isinstance(self.pdk[layer]["Offset"], list)
            else self.pdk[layer]["Offset"]
        )

    def _get_via_ext(self, metal, via):
        viaenc = (
            self.pdk[via][f"VencA_L"]
            if self.pdk[via]["Stack"][0] == metal
            else self.pdk[via][f"VencA_H"]
        )
        viawidth = (
            self.pdk[via]["WidthX"]
            if self.pdk[metal]["Direction"] == "h"
            else self.pdk[via]["WidthY"]
        )
    ...
    def _create_metal(self, layer, info, *, check=True):
        if "Color" in info and len(info["Color"]) > 0:
            ...
            self.postprocessor.register(
                layer, ColorClosure(info=info, errors=self.postprocessor.errors)
            )
        ...
        if isinstance(info["Width"], list):
            ...
            for i in range(0, 1):
                self._create_metal(
                    layer,
                    {
                        k: v[i] if k in ["Pitch", "Width", "MinL", "MaxL"] else v
            ...
            if nm is None:
                ...
                spg_pitch, spg_stop, spg_offset = (
                    self._get_metal_pitch(pm),
                ...
                spg_pitch, spg_stop, spg_offset = (
                    self._get_metal_pitch(nm),
                    self._get_via_ext(base_layer, nv),
                    self._get_metal_offset(nm),
                )
            else:
                pm_pitch, nm_pitch = self._get_metal_pitch(pm), self._get_metal_pitch(
                    nm
                )
    ...
    def _create_via(self, layer, info):
        if any(x is None for x in info["Stack"]):
            logger.debug(
                f"Cannot create {layer} via automatically. One or more metal layers are None."
            )
            return

        if self.pdk[info["Stack"][0]]["Direction"] == "h":
            assert (
                self.pdk[info["Stack"][1]]["Direction"] == "v"
            ), f"{info['Stack']} both appear to be horizontal"
            ...
            assert (
                self.pdk[info["Stack"][1]]["Direction"] == "h"
            ), f"{info['Stack']} both appear to be vertical"
    ...
    def _find_adjoining_layers(self, layer):
        ...
        for (v, (m0, m1)) in self.pdk.get_via_stack():
            if layer == m0:
                ...
                pm = m0
        assert (
            nm is not None or pm is not None
        ), f"Could not trace any connections for {layer}"
        return (pm, pv, nv, nm)
