...
def generate_MOS_primitive(
    ...
    shared_diff = (
        0
        if (
            len(primitive.elements) == 2
            and primitive.elements[0].pins["S"] != primitive.elements[1].pins["S"]
        )
        else 1
    )
    gen_const = [
    ...
    uc = generator(
        ...
        primitive_constraints=primitive.constraints,
    )
    ...
    if not input_pattern:
        if len(primitive.elements) == 1:
            ...
            ele.parameters == primitive.elements[0].parameters
    ...
    if len(primitive.elements) == 2:
        x_cells = 2 * x_cells
        pattern = (
            2 if x_cells % 4 != 0 else pattern
        )  # CC is not possible; default is interdigitated
...
def generate_primitives(primitive_lib, pdk_dir, primitive_dir, netlist_dir):
    ...
    for block_name, block_args in primitives.items():
        if (
            block_args["primitive"] != "generic"
            and block_args["primitive"] != "guard_ring"
        ):
            primitive_def = primitive_lib.find(block_args["abstract_template_name"])
            assert (
                primitive_def is not None
            ), f"unavailable primitive definition {block_name} of type {block_args['abstract_template_name']}"
        else:
            primitive_def = block_args["primitive"]
        block_args.pop("primitive", None)
        uc = generate_primitive(
            block_name,
            primitive_def,
            **block_args,
            pdkdir=pdk_dir,
            outputdir=primitive_dir,
...
def generate_primitive_param(
    subckt: SubCircuit, primitives: list, pdk_dir: pathlib.Path, uniform_height=False
):
    """Return commands to generate parameterized lef"""
    assert isinstance(
        subckt, SubCircuit
    ), f"invalid input for primitive generator {subckt}"
...
def generate_primitive(
    ...
    pinswitch=0,
    bodyswitch=1,
    pdkdir=pathlib.Path.cwd(),
    outputdir=pathlib.Path.cwd(),
    netlistdir=pathlib.Path.cwd(),
    abstract_template_name=None,
    concrete_template_name=None,
):
    assert pdkdir.exists() and pdkdir.is_dir(), "PDK directory does not exist"
    assert (
        isinstance(primitive, SubCircuit)
        or primitive == "generic"
        or "ring" in primitive
    ), f"{block_name} definition: {primitive}"
    ...
    if primitive == "generic":
        ...
        uc, _ = generate_Res(
            pdkdir, block_name, height, x_cells, y_cells, value[0], value[1]
        )
        uc.setBboxFromBoundary()
    else:
    ...
    with open(outputdir / (block_name + ".json"), "rt") as fp0, open(
        outputdir / (block_name + ".gds.json"), "wt"
    ) as fp1:
