...
class Scanline:
    ...
    def merge_slr(self, base_slr, new_slr):
        base_slr.rect[self.dIndex + 2] = max(
            base_slr.rect[self.dIndex + 2], new_slr.rect[self.dIndex + 2]
        )
    ...
    def build_centerline_tbl(self):
        ...
        for d in self.canvas.terminals:
            ...
            if isPorted:
                assert (
                    netName != None
                ), f"netName for pin rectange {rect} on layer {layer} is None"
            if layer in self.skip_layers:
                continue
            if layer in self.layers:
                twice_center = sum(
                    rect[index] for index in self.indicesTbl[self.layers[layer]][0]
                )
                ...
                for (rect, _, _, _) in v[1:]:
                    if not all(rect[i] == rect0[i] for i in indices):
                        ...
                        if layer not in skip_layers_for_different_widths:
                            ...
                            tup = (
                                f"Rectangles on layer {layer} with the same 2x centerline {twice_center} but different widths {widths}:",
                                (indices, v),
                            )
                            # logger.warning( f"{tup}")
                            self.different_widths.append(tup)

                sl = self.store_scan_lines[layer][twice_center] = Scanline(
                    indices, dIndex
                )

                current_slr = None
                for (rect, netName, netType, isPorted) in sorted(
                    v, key=lambda p: p[0][dIndex]
                ):
                    potential_slr = sl.new_slr_no_add(
                        rect, netName, netType, isPorted=isPorted
                    )
                    if (
                        not sl.isEmpty()
                        and rect[dIndex] <= current_slr.rect[dIndex + 2]
                        and all(rect[i] == current_slr.rect[i] for i in indices)
                    ):  # continuation
                        if self.connectPair(layer, current_slr, potential_slr):
                            if potential_slr.netType not in [
                                "blockage"
                            ] and current_slr.netType not in ["blockage"]:
                ...
                for (twice_center, via_scan_line) in self.store_scan_lines[via].items():
                    if mv is not None:
                        if twice_center not in self.store_scan_lines[mv]:
                            logger.warning(
                                f"{twice_center} not in self.store_scan_lines[{mv}]. Skipping..."
                            )
                            continue
                        metal_scan_line_vertical = self.store_scan_lines[mv][
                            twice_center
                        ]
                        for via_rect in via_scan_line.rects:
                            metal_rect_v = metal_scan_line_vertical.find_touching(
                                via_rect
                            )
                    ...
                    if mh is not None:
                        for via_rect in via_scan_line.rects:
                            ...
                            if twice_center_y not in self.store_scan_lines[mh]:
                                logger.warning(
                                    f"{twice_center_y} not in self.store_scan_lines[{mh}]. Skipping..."
                                )
                                continue
                            metal_scan_line_horizontal = self.store_scan_lines[mh][
                                twice_center_y
                            ]
                            metal_rect_h = metal_scan_line_horizontal.find_touching(
                                via_rect
                            )
    ...
    def check_shorts_induced_by_terminals(self):
        for instance, v in self.subinsts.items():
            for pin, slrs in v.pins.items():
                ...
                if len(names) > 1:
                    self.shorts.append(
                        (names, f"THROUGH TERMINAL {instance}:{pin}", slrs)
                    )
    ...
    def remove_duplicates(self, silence_errors=False):
        ...
        if True:
            if self.shorts or self.opens or self.different_widths:
                if silence_errors:
                    logger.debug(
                        f"Found errors: SHORT: {len(self.shorts)} OPEN: {len(self.opens)} DIFFERENT WIDTH: {len(self.different_widths)}"
                    )
                else:
                    logger.error(
                        f"Found errors: SHORT: {len(self.shorts)} OPEN: {len(self.opens)} DIFFERENT WIDTH: {len(self.different_widths)}"
