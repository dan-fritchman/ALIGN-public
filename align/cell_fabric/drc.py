...
class DesignRuleCheck:
    ...
    def _check_via_rules(self, layer, vv):
                    ...
                    if idx + 1 < len(sl0.rects):
                        ...
                        if slr0.rect[3] < slr1.rect[1]:
                            if slr0.rect[3] + space_y > slr1.rect[1]:
                                self.errors.append(
                                    f"Vertical space violation on {layer}: {slr0} {slr1} {space_y}"
                                )
                    ...
                    if idx + 1 < len(bins):
                        ...
                        if slr0[2] < slr1[0]:
                            if slr0[2] + space_x > slr1[0]:
                                self.errors.append(
                                    f"Horizontal space violation on {layer}: {slr0} {slr1} {space_x}"
                                )
        ...
        if max_adjacent_y is not None:
            ...
            for (_, sl) in vv.items():
                idx_prev = None
                for slr in sl.rects:
                    idx = via.h_clg.inverseBounds((slr.rect[1] + slr.rect[3]) // 2)[0][
                        0
                    ]
                    if idx_prev is None:
                        count = 1
                    ...
                    if count > max_adjacent_y:
                        self.errors.append(
                            f"Vertical max adjacent via violation on {layer}: {slr}"
                        )
            ...
            for (_, sl) in vv_h.items():
                ...
                for r in sl:
                    ...
                    if count > max_adjacent_x:
                        self.errors.append(
                            f"Horizontal max adjacent via violation on {layer}: {slr}"
                        )
    ...
    def _check_via_enclosure_rules(self, layer, vv):
        ...
        def check_single_metal(r, ly, metal_dir, enclosure_value):
            ...
            if metal_r is None:
                self.errors.append(
                    f"Enclosure violation on {ly}-{layer} for {net_name}: No metal found surrounding {r.rect}, {enclosure_value}"
                )
            elif (
                metal_r[o + 0] > r.rect[o + 0] - enclosure_value
                or metal_r[o + 2] < r.rect[o + 2] + enclosure_value
            ):
                self.errors.append(
                    ...
                    for slr0 in v0.rects:
                        for slr1 in v1.rects:
                            if 0 < slr1.rect[o + 0] - slr0.rect[o + 2] <= dist:
                                self.errors.append(
                                    f"Adjacent metal attacker {layer}: {slr0.rect} too close to {slr1.rect} dist: {dist}"
                                )
        ...
        for slr in slrects:
            ...
            if rect[end] - rect[start] < min_length:
                ...
                if self.r_regions.contained_in(rect):
                    logger.debug(
                        f"Skipping: MinLength violation on {layer}: {root.netName}{rect}"
                    )
                else:
                    self.errors.append(
    ...
    def _check_min_spacing(self, layer, slrects, dIndex):
        ...
        for slr in slrects:
            if (
                prev_slr is not None
                and 0 < slr.rect[start] - prev_slr.rect[end] < min_space
            ):
