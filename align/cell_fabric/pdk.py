...
class Pdk(object):
    ...
    def load(self, filename):
        ...
        for layer in j["Abstraction"]:
            assert (
                layer["Layer"] not in self.pdk
            ), f"Cannot have multiple {layer['Layer']} layers with same name"
            assert layer["Layer"][
                0
            ].isupper(), (
                f"Layer name {layer['Layer']} must start with capitalized letter"
            )
    ...
    def _check(parameters, optional_parameters, **kwargs):
        assert all(
            x in kwargs for x in parameters
        ), f"Entry {kwargs} missing one or more of {parameters}"

        assert all(
            (x in parameters) or (x in optional_parameters) for x in kwargs.keys()
        ), f"Entry {kwargs} has one or more spurious entries (Needs only {parameters})"

    def _add(self, parameters, **kwargs):
        # Guarantee one is to one mapping between parameters & kwargs
        layername = kwargs.pop("Layer")
        self.pdk[layername] = {
            key: None if value == "NA" else value for key, value in kwargs.items()
        }
    ...
    def addMetal(self, **kwargs):
        ...
        assert all(
            all(isinstance(y, int) for y in kwargs[x] if y is not None)
            if isinstance(kwargs[x], list)
            else isinstance(kwargs[x], int)
            for x in params[5:10]
            if kwargs[x] is not None
        ), f"One or more of {params[5:10]} not an integer in {kwargs}"
        assert all(
            all(y is not None and y % 2 == 0 for y in kwargs[x])
            if isinstance(kwargs[x], list)
            else kwargs[x] is not None and kwargs[x] % 2 == 0
            for x in params[5:6]
            if kwargs[x] is not None
        ...
        assert kwargs["Direction"] in (
            "v",
            "h",
        ), f"Invalid Direction {kwargs['Direction']} in {kwargs}"
    ...
    def addVia(self, **kwargs):
        ...
        # 0. Dimensions
        assert all(
            isinstance(kwargs[x], int) for x in params[4:7]
        ), f"One or more of {params[3:7]} not an integer in {kwargs}"
        assert all(
            kwargs[x] % 2 == 0 for x in params[4:7]
        ), f"One or more of {params[4:7]} in {kwargs} not a multiple of two"
        # 1. Metal Stack
        assert (
            isinstance(kwargs["Stack"], list) and len(kwargs["Stack"]) == 2
        ), f"Parameter 'Stack': {kwargs['Stack']} must be a list of size 2"
        assert all(
            x is None or x in self.pdk for x in kwargs["Stack"]
        ), f"One or more of metals {kwargs['Stack']} not yet defined."
        # 2. DesignRules
    ...
    def add(self, **kwargs):
        assert (
            "Layer" in kwargs
        ), '"Layer" is required parameter for all layers in PDK abstraction'
    ...
    def get_gds_map(self):
        return {
            x: self.pdk[x]["LayerNo"]
            for x in self.pdk.keys()
            if "LayerNo" in self.pdk[x]
        }
