...
class PostProcessor:
    ...
    def register(self, layer, func):
        assert (
            layer not in self.postprocessors
        ), "Please specify only one postprocessor per layer"
        self.postprocessors[layer] = func

    @staticmethod
    def _check_valid_rect(rect):
        assert len(rect) == 4 and all(
            isinstance(x, int) for x in rect
        ), f"Invalid generator output {rect}"
    ...
    def run(self, terminals):
        ...
        for term in old_terminals:
            if term["layer"] in self.postprocessors:
                ...
                if isinstance(postprocessor_output, types.GeneratorType) or isinstance(
                    postprocessor_output, list
                ):
