class Wire:
    ...
    def segment(
        self, netName, center, bIdx, eIdx, *, bS=None, eS=None, netType="drawing"
    ):
        if bS is None:
            bS = self.spg
        if eS is None:
        ...
        if netType in ["drawing", "pin", "blockage"]:
            ...
            assert (
                "Invalid net type, valid net types include drawing, pin, and blockage"
            )
        return data
...
class Region:
    ...
    def segment(self, netName, grid_x0, grid_y0, grid_x1, grid_y1, netType="drawing"):
        ...
        rect = [
            self.physical_x(grid_x0),
            self.physical_y(grid_y0),
            self.physical_x(grid_x1),
            self.physical_y(grid_y1),
        ]
