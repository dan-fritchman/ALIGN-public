...
class MetalTemplate:
    def __init__(self, m):
        ...
        tuples = [
            (idx, pair)
            for (idx, pair) in enumerate(m.spg.grid)
            if idx in m.spg.legalIndices
        ]
    ...
    def write(self, fp):
        ...
        fp.write(
            f"MetalTemplate layer={m.layer} name={m.nm} widths={widths} spaces={spaces}{colors_str} stops={stops} stop_offset={self.stop_offset}\n"
        )
...
def gen(c, dirname):
    ...
    metal_templates = dict(
        {(k, v) for (k, v) in c.generators.items() if k.startswith("m")}
    )
