...
def lef_from_layout_d(
    ...
    *,
    ...
    assert (
        bbox[3] - bbox[1]
    ) % m2pitch == 0, f"Cell height not a multiple of the grid {bbox}"
    assert (
        bbox[2] - bbox[0]
    ) % m3pitch == 0, f"Cell width not a multiple of the grid {bbox}"
    ...
    pins = [
        term["netName"] for term in layout_d["terminals"] if term["netType"] == "pin"
    ]
    ...
    for obj in layout_d["terminals"]:
        ...
        if (
            (obj["netType"] != "pin" or cond)
            and blockM == 0
            and obj["layer"] not in exclude_layers
        ):
            fp.write("    LAYER %s ;\n" % obj["layer"])
            fp.write("      RECT %s %s %s %s ;\n" % tuple(obj["rect"]))
