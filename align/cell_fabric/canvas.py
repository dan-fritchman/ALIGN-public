...
class Canvas:
    ...
    def addWire(
        self, wire, netName, c, bIdx, eIdx, *, bS=None, eS=None, netType="drawing"
    ):
        self.transform_and_add(
            wire.segment(netName, c, bIdx, eIdx, bS=bS, eS=eS, netType=netType)
        )

    def addRegion(self, region, netName, grid_x0, grid_y0, grid_x1, grid_y1):
        self.transform_and_add(
            region.segment(
                netName, grid_x0, grid_y0, grid_x1, grid_y1, netType="drawing"
            )
        )
    ...
    def addWireAndMultiViaSet(
        self, netName, wire, c, listOfPairs, *, bIdx=None, eIdx=None, netType="drawing"
    ):
        ...
        dim_tuples = [
            (via.v_clg if wire.direction == "h" else via.h_clg).value(idx, check=False)[
                0
            ]
        ...
        via_list = [
            via for (via, listOfIndices) in listOfPairs for idx in listOfIndices
        ]
        ...
        for (cl, sl) in m_lines.items():
            ...
            for (idx, slr) in enumerate(sl.rects):
                ...
                if idx + 1 < len(sl.rects):
                    ...
                    if (
                        slr.netName == next_slr.netName
                        and w == next_w
                        and new_length <= max_l
                        and slr.netType == next_slr.netType
                    ):
                ...
                for (mv_cl, mv_sl) in mv_lines.items():
                    ...
                    for (_, mv_slr) in enumerate(mv_sl.rects):
                        ...
                        self._drop_via_if_dr_clean(
                            via, via_matrix, mh, mh_cl, mh_slr, mv, mv_cl, mv_slr
                        )
    ...
    def _drop_via_if_dr_clean(
        self, via, via_matrix, mh, mh_cl, mh_slr, mv, mv_cl, mv_slr
    ):
        ...
        if mh_cl_m1 in via_matrix:
            for v_cl, r in via_matrix[mh_cl_m1].items():
                if v_cl == mv_cl:
                    if (r[3] < via_rect[1]) and (
                        r[3] > via_rect[1] - via_def["SpaceY"]
                    ):
                        return
        ...
        if mh_cl_p1 in via_matrix:
            for v_cl, r in via_matrix[mh_cl_p1].items():
                if v_cl == mv_cl:
                    if (r[1] > via_rect[3]) and (
                        r[1] < via_rect[3] + via_def["SpaceY"]
                    ):
                        return
    ...
    def asciiStickDiagram(self, v1, m2, v2, m3, matrix, *, xpitch=4, ypitch=2):
        ...
        for x in range((ncols - 1) // xpitch + 1):
            col = (
                "".join([paddedA[nrows - 1 - y][x * xpitch] for y in range(nrows)])
                + " "
            )
            started = False
            nm = None
    ...
    def _initialize_layer_stack(self):
        ...
        self.layer_stack = [
            (l, (pl, nl)) if self.pdk[nl]["Direction"] == "h" else (l, (nl, pl))
            for l, (pl, nl) in self.pdk.get_via_stack()
            if l.startswith("V")
        ]
    ...
    def __init__(self, pdk=None, gds_layer_map=None):
        ...
        self.subinsts = collections.defaultdict(
            lambda: LayoutDevice(
                collections.defaultdict(None), collections.defaultdict(set)
            )
        )
        self.terminals = []
    ...
    def removeDuplicates(
        self, *, nets_allowed_to_be_open=None, allow_opens=False, silence_errors=False
    ):
        self.rd = RemoveDuplicates(
            self,
            nets_allowed_to_be_open=nets_allowed_to_be_open,
    ...
    def writeJSON(
        self, fp, *, draw_grid=False, run_drc=True, run_pex=True, postprocess=False
    ):
        data = self.gen_data(
            draw_grid=draw_grid,
            run_drc=run_drc,
        ...
        if ly in self.pdk:
            ...
            for ix in range((width + pitch - 1) // pitch):
                ...
                data["terminals"].append(
                    {"netName": ly + "_grid", "layer": ly, "rect": r}
                )
    ...
    def writeGDS(self, fp1, timestamp=None):
        ...
        with io.StringIO(contents) as fp0, io.StringIO() as fp_tmp:
            # TODO: Update this
            raise NotImplementedError(
                "Translate api has changed but canvas hasn't been updated"
            )
    ...
    def checkDesignRules(self):
        ...
        assert (
            self.rd is not None
        ), "removeDuplicates() must be run before checkDesignRules()"
    ...
    def extractParasitics(self):
        ...
        assert (
            self.rd is not None
        ), "removeDuplicates() must be run before extractParasitics()"
