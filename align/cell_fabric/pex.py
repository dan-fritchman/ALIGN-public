...
class ParasiticExtraction:
    def __init__(self, canvas):
        self.canvas = canvas
        self._terms = collections.defaultdict(
            lambda: collections.defaultdict(list)
        )  # layer: {scanline: [p1...pn]}
        self._c_count = 0
        self._r_count = 0
    ...
    def run(self):
        ...
        for (layer, vv) in self.canvas.rd.store_scan_lines.items():
            if (
                not (layer.startswith("V") or layer.startswith("M"))
                or layer not in self.canvas.pdk
            ):
                continue
            if self.canvas.rd.layers[layer] == "*":
        ...
        for tup in self.netCells.items():
            ...
            if ly.startswith("M"):
                dist = (
                    self.compute_dist(rect[0], rect[2])
                    if self.canvas.pdk[ly]["Direction"] == "h"
                    else self.compute_dist(rect[1], rect[3])
                )
                (self.pi if mode == "Pi" else self.tee)(
                    t0,
                    t1,
                    self.canvas.pdk[ly]["UnitR"]["Mean"] * dist,
                    self.canvas.pdk[ly]["UnitC"]["Mean"] * dist,
                )
            elif ly.startswith("V"):
                self.components.append(
                    (self.resistor(), t0, t1, self.canvas.pdk[ly]["R"]["Mean"])
                )
            else:
                assert False, ly
    ...
    def _extract_via_parasitics(self, layer, vv):
        for v in vv.values():
            for slr in v.rects:
                self._create_via_netcells(
                    slr.root().netName, slr.terminal, layer, slr.rect
                )
    ...
    def _create_via_netcells(self, net, terminal, layer, rect):
        ...
        if terminal is None:
            node1 = self._gen_netcell_node_name(
                net, self.canvas.pdk[layer]["Stack"][0], x, y
            )
            node2 = self._gen_netcell_node_name(
                net, self.canvas.pdk[layer]["Stack"][1], x, y
            )
    ...
    def _extract_metal_scanline(self, layer, slrects, dIndex, twice_center):
        for slr in slrects:
            self._extract_metal_rectangle(
                slr.root().netName, layer, twice_center, slr.rect, dIndex
            )
    ...
    def _stamp_netcells(self, net, layer, twice_center, starti, endi, rect, dIndex):
        ...
        for i in range(numcells):
            ...
            if self.canvas.pdk[layer]["Direction"] == "v":
                node1 = self._gen_netcell_node_name(
                    net, layer, twice_center // 2, cellstart
                )
                node2 = self._gen_netcell_node_name(
                    net, layer, twice_center // 2, cellend
                )
            else:
                node1 = self._gen_netcell_node_name(
                    net, layer, cellstart, twice_center // 2
                )
                node2 = self._gen_netcell_node_name(
                    net, layer, cellend, twice_center // 2
                )
            cell_rect = rect.copy()
    ...
    def _extract_metal_rectangle(self, net, layer, twice_center, rect, dIndex):
        ...
        for port in self._terms[layer][twice_center]:
            if prev_port is None:
                if port > starti:
                    prev_port = self._stamp_netcells(
                        net, layer, twice_center, starti, port, rect, dIndex
                    )
            elif port > endi:
                break
            else:
                prev_port = self._stamp_netcells(
                    net, layer, twice_center, prev_port, port, rect, dIndex
                )
        if prev_port is None:
            prev_port = starti
    ...
    def writePex(self, fp):
        ...
        for inst, v in self.canvas.rd.subinsts.items():
            ...
            fp.write(
                f"{inst}_0 net_{inst}_D net_{inst}_G net_{inst}_diff {'vdd!' if 'PMOS' in model.upper() else 'gnd!'} {model} {paramstring}\n"
            )
            fp.write(
                f"{inst}_1 net_{inst}_diff net_{inst}_G net_{inst}_S {'vdd!' if 'PMOS' in model.upper() else 'gnd!'} {model} {paramstring}\n"
            )
