...
from .. import PnR
from .render_placement import (
    gen_placement_verilog,
    scale_placement_verilog,
    gen_boxes_and_hovertext,
    standalone_overlap_checker,
    scalar_rational_scaling,
    round_to_angstroms,
)
...
def place(
    *,
    ...
    modules_d=None,
    ilp_solver,
    place_on_grid_constraints_json,
    ...
    hyper.ALPHA = math.exp(
        math.log(hyper.T_MIN / hyper.T_INT) / max(hyper.SA_MAX_ITER, 10000)
    )
    ...
    curr_plc = PnR.PlacerIfc(
        current_node, numLayout, opath, effort, DB.getDrc_info(), hyper
    )
    ...
    if actualNumLayout != numLayout:
        logger.debug(
            f"Placer did not provide numLayout ({numLayout} > {actualNumLayout}) layouts for {DB.hierTree[idx].name}"
        )
...
def scale_and_check_placement(
    *,
    ...
    scaled_placement_verilog_d = scale_placement_verilog(
        placement_verilog_d, scale_factor
    )
    (pathlib.Path(opath) / f"{concrete_name}.scaled_placement_verilog.json").write_text(
        scaled_placement_verilog_d.json(indent=2, sort_keys=True)
    )
...
def per_placement(
    ...
    *,
    ...
    concrete_names = {
        ...
        if m["abstract_name"] == abstract_name
    }
    assert len(concrete_names) == 1, concrete_names
    concrete_name = next(iter(concrete_names))

    scale_and_check_placement(
        placement_verilog_d=placement_verilog_d,
    ...
    hpwl_alt = calculate_HPWL_from_placement_verilog_d(
        placement_verilog_d, concrete_name, nets_d, skip_globals=True
    )
    ...
    if hpwl_alt != hN.HPWL_extend:
        ...
        logger.debug(
            f"hpwl: locally computed from netlist {hpwl_alt}, placer computed {hN.HPWL_extend} are equal for {concrete_name}!"
        )
    ...
    for atn, gds_lst in DB.gdsData2.items():
        ...
        for ctn in ctns:
            if ctn in DB.lefData:
                ...
                p = scalar_rational_scaling(
                    lef.width, mul=0.001, div=2
                ), scalar_rational_scaling(lef.height, mul=0.001, div=2)
...
def startup_gui(
    *, top_level, leaf_map, placement_verilog_alternatives, lambda_coeff, metrics
):
    ...
    for concrete_name, placement_verilog_d in placement_verilog_alternatives.items():
        ...
        gui_scaled_placement_verilog_d = scale_placement_verilog(
            placement_verilog_d, 0.001
        )

        modules = {
            x["concrete_name"]: x for x in gui_scaled_placement_verilog_d["modules"]
        }
        ...
        tagged_bboxes[abstract_name][concrete_name] = (
            metrics[concrete_name],
            list(
                gen_boxes_and_hovertext(
                    gui_scaled_placement_verilog_d, concrete_name, nets_d
                )
            ),
            nets_d,
        )
    ...
    print(
        f"Press Ctrl-C to end the GUI interaction. If current selection is a toplevel placement, the routing engine will be called on that placement. If the current selection is not toplevel (an intermediate hierarchy or a leaf), the router call will be skipped."
    )

    selected_concrete_name = run_gui(
        tagged_bboxes=tagged_bboxes, module_name=top_level, lambda_coeff=lambda_coeff
    )
...
def update_grid_constraints(
    grid_constraints, DB, idx, verilog_d, primitives, scale_factor
):
    ...
    for sel in range(DB.hierTree[idx].numPlacement):
        ...
        for leaf in scaled_placement_verilog_d["leaves"]:
            ...
            if "metadata" in primitive and "constraints" in primitive["metadata"]:
                ...
                leaf["constraints"].extend(
                    constraint for constraint in primitive["metadata"]["constraints"]
                )
        ...
        top_module = next(
            iter(
                [
                    ...
                    if module["concrete_name"] == top_name
                ]
            )
        )
        ...
        frontier[top_name] = [
            constraint.dict()
            for constraint in top_module["constraints"]
...
def hierarchical_place(
    *,
    ...
    logger.debug(
        f'Calling hierarchical_place with {"existing placement" if placement_verilog_d is not None else "no placement"}'
    )

    if placement_verilog_d is not None:
        hack_placement_verilog_d = scale_placement_verilog(
            placement_verilog_d, scale_factor, invert=True
        )
    ...
    for idx in DB.TraverseHierTree():
        ...
        json_str = json.dumps(
            [
                {"concrete_name": k, "constraints": v}
                for k, v in grid_constraints.items()
            ],
            indent=2,
        )
        ...
        place(
            ...
            select_in_ILP=select_in_ILP,
            place_using_ILP=place_using_ILP,
            seed=seed,
            use_analytical_placer=use_analytical_placer,
            modules_d=modules_d,
            ilp_solver=ilp_solver,
            place_on_grid_constraints_json=json_str,
    ...
    top_level, leaf_map, placement_verilog_alternatives, metrics = process_placements(
        DB=DB,
        verilog_d=verilog_d,
        lambda_coeff=lambda_coeff,
        scale_factor=scale_factor,
...
def placer_driver(
    *,
    ...
    DB, verilog_d, new_fpath, opath, numLayout, effort = gen_DB_verilog_d(
        toplevel_args_d=toplevel_args_d,
    ...
    top_level, leaf_map, placement_verilog_alternatives, metrics = hierarchical_place(
        DB=DB,
        opath=opath,
        fpath=fpath,
        numLayout=numLayout,
