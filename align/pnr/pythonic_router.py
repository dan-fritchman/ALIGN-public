...
def route_by_stretch(flat_tbl, Canvas):
    ...
    for net, terminals in flat_tbl.items():
        for (layer, rect) in terminals:
            cv.terminals.append(
                {"netName": net, "layer": layer, "rect": rect, "netType": "drawing"}
            )
    ...
    for net, terminals in flat_tbl.items():
        logger.info(
            f"Number of wires for {net} before {len(terminals)} after {len(new_flat_tbl[net])}"
        )
...
def draw_routing_problems_grid(module, grid_tbl):
    colorscale = px.colors.qualitative.Alphabet

    _, _, width, height = module["bbox"]
    for idx, (k, v) in enumerate(grid_tbl.items()):
        # color = colorscale[idx % len(colorscale)]
        ...
        for instance in module["instances"]:
            ...
            for fa in instance["fa_map"]:
                ...
                for layer, rect in pin_tbl[ctn][formal]:
                    newRect = (
                        tr.hitRect(transformation.Rect(*rect)).canonical().toList()
                    )

                    grid_tbl[actual].append(
                        (layer, rect_to_grid(canvas, layer, newRect))
                    )
