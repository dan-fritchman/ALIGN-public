...
def trim_placement_data(placement_data, top_level):
    ...
    top_concrete_names = [
        ...
        if module["abstract_name"] == top_level
    ]
    all_modules_leaves = set()
    for concrete_name in top_concrete_names:
        _, found_modules, found_leaves = compute_topoorder(
            modules, concrete_name, key="concrete_template_name"
        )
    ...
    for key in ["leaves", "modules"]:
        new_placement_data[key] = [
            x for x in placement_data[key] if x["concrete_name"] in all_modules_leaves
        ]
    ...
    for instance in modules[module_name]["instances"]:
        ...
        if sub_module_name in modules:
            ...
            for formal_actual in instance["fa_map"]:
                ...
                if actual in global_signals and formal not in modules[
                    sub_module_name
                ].get(GS, set()):
                    signals_to_propagate.add(formal)
            if signals_to_propagate:
                propagate_down_global_signals(
                    modules, sub_module_name, signals_to_propagate
                )
