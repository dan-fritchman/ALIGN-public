...
def _ReadVerilogJson(DB, j, add_placement_info=False):
    ...
    for module in j["modules"]:
        ...
        for parameter in module["parameters"]:
            ...
            temp_terminal.type = (
                "input"  # All nets are inputs, we don't use this for anything do we?
            )
        ...
        for instance in module["instances"]:
            ...
            if "template_name" in instance:
                ...
                assert (
                    False
                ), f"Missing template_name (abstract or otherwise) in instance {instance}"
    ...
    for global_signal in j["global_signals"]:
        global_signals.append(
            (global_signal["prefix"], global_signal["formal"], global_signal["actual"])
        )
...
def _attach_constraint_files(DB, fpath):
    ...
    for curr_node in DB.hierTree:
        ...
        if fp.exists():
            ...
            logger.debug(
                f"Finished reading contraint json file {curr_node.name}.pnr.const.json"
            )
        else:
    ...
    for name, instances in DB.lefData.items():
        ...
        if fp.exists():
            ...
            logger.warning(
                f"No primitive json file for primitive {name}. Okay if a CC capacitor."
            )
...
def gen_DB_verilog_d(
    toplevel_args_d, results_dir, *, verilog_d_in=None, map_d_in=None, lef_s_in=None
):
