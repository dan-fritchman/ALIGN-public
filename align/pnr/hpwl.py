...
def gen_netlist(placement_verilog_d, concrete_name):
    ...
    modules = {
        module["concrete_name"]: module for module in placement_verilog_d["modules"]
    }
        ...
        for inst in module["instances"]:
            ...
            def gen_pair():
                for fa in inst["fa_map"]:
                    ...
                    new_a = (
                        (a,)
                        if a in global_actuals
                        else translate_d.get(a, prefix_path + (a,))
                    )
                    yield f, new_a
...
def calculate_HPWL_from_placement_verilog_d_top_down(
    placement_verilog_d, concrete_name, nets_d, *, skip_globals=False
):
    instances = {
        (module["concrete_name"], instance["instance_name"]): instance
    ...
    for hnet, hpins in nets_d.items():
        if skip_globals and len(hnet) == 1 and hnet[0] in global_actuals:
            continue
        sp = SemiPerimeter()
        for hpin in hpins:
            ctn = concrete_name
...
def calculate_HPWL_from_placement_verilog_d_bottom_up(
    placement_verilog_d, concrete_name, *, skip_globals=False
):
        ...
        if cn in found_modules:
            ...
            for instance in module["instances"]:
                ...
                for a in global_actuals:
                    if (ctn, a) not in net_bboxes:
                        continue
                    net_bboxes[(cn, a)].update(net_bboxes[(ctn, a)], tr)

            for a in local_a.difference(
                set(module["parameters"]).union(global_actuals)
            ):
    ...
    for a in set(module["parameters"]).union(global_actuals):
        if skip_globals and a in global_actuals:
            continue
        net_hpwl = net_bboxes[(cn, a)].dist()
...
def calculate_HPWL_from_placement_verilog_d(
    placement_verilog_d, concrete_name, nets_d, *, skip_globals=False
):
    hpwl_top_down = calculate_HPWL_from_placement_verilog_d_top_down(
        placement_verilog_d, concrete_name, nets_d, skip_globals=skip_globals
    )
    hpwl_bottom_up = calculate_HPWL_from_placement_verilog_d_bottom_up(
        placement_verilog_d, concrete_name, skip_globals=skip_globals
    )

    if hpwl_top_down != hpwl_bottom_up:
        logger.warning(
            f"HPWL calculated in different ways differ: top_down: {hpwl_top_down} bottom_up: {hpwl_bottom_up}"
        )
