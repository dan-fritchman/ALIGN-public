...
def check_floating_pins(verilog_d):
    ...
    for module in verilog_d["modules"]:
        all_nets = set(
            ...
            for fa_map in inst["fa_map"]
        )
        assert set(module["parameters"]).issubset(
            all_nets
        ), f"floating port found in module {module['name']}"
    ...
    for mod in verilog_d["modules"]:
        for inst in mod["instances"]:
            ...
            if atn in all_module_pins:
                assert all(
                    fm["formal"] in all_module_pins[atn] for fm in inst["fa_map"]
...
def remove_pg_pins(verilog_d: dict, subckt: str, pg_connections: dict):
    ...
    modules_dict = {
        module["name"]: module["parameters"] for module in verilog_d["modules"]
    }
    ...
    for inst in module["instances"]:
        if inst["abstract_template_name"] in modules_dict:
            ...
            hier_pg_connections = {
                conn["formal"]: pg_connections[conn["actual"]]
                for conn in inst["fa_map"]
                if conn["actual"] in pg_connections
            }
            if len(hier_pg_connections) > 0:
                logger.debug(
                    f"Removing instance {inst['instance_name']} pins connected to global nets {hier_pg_connections}"
                )
                inst["fa_map"] = [
                    conn
                    for conn in inst["fa_map"]
                    if conn["formal"] not in hier_pg_connections
                ]
                # Creates a copy of module with reduced pins
                new_name = modify_pg_conn_subckt(
                    verilog_d, inst["abstract_template_name"], hier_pg_connections
                )
            ...
            inst["fa_map"] = [
                {
                    "formal": conn["formal"],
                    "actual": pg_connections.get(conn["actual"], conn["actual"]),
                }
                for conn in inst["fa_map"]
            ]
            logging.debug(
                f"leaf level node {inst['instance_name']} {inst['abstract_template_name']}"
            )
...
def modify_pg_conn_subckt(verilog_d, subckt, pp):
    ...
    nm_dict = [module for module in verilog_d["modules"] if module["name"] == subckt][
        0
    ].dict()
...
def add_cap_dummy_connections(verilog_d, cap_map):
    ...
    global_gnds = [
        entry["actual"]
        for entry in verilog_d["global_signals"]
        if entry["formal"] == "supply0"
    ]
            ...
            if atn in cap_abstract_names:
                ...
                for pin in ["dummy_gnd_PLUS", "dummy_gnd_MINUS"]:
                    if entry[pin]:
                        ...
                        inst["fa_map"].append(
                            FormalActualMap(formal=pin, actual=global_gnds[0])
                        )
...
def manipulate_hierarchy(verilog_d, subckt):
    ...
    remove_pg_pins(
        verilog_d,
        subckt,
        {p["actual"]: p["actual"] for p in verilog_d["global_signals"]},
    )
        ...
        for module in scaled_placement_verilog_d["modules"]:
            for instance in module["instances"]:
                ...
                if ctn in primitives:
                    ...
                    if (
                        "metadata" in primitive
                        and "partially_routed_pins" in primitive["metadata"]
                    ):
                        ...
                        for fa in instance["fa_map"]:
                            ...
                            for enity_name in by_net.get(f, [f]):
                                new_fa_map.append(
                                    FormalActualMap(formal=enity_name, actual=a)
                                )
