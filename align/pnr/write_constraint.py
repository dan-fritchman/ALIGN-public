...
class PnRConstraintWriter:
    ...
    def merge_sametemplate_const(self, all_const, pnr_const):
        ...
        for insts in same_template:
            pnr_const.append({"const_name": "SameTemplate", "blocks": list(insts)})
    ...
    def map_valid_const(self, all_const, module):
        ...
        for input_const in all_const:
            if input_const.constraint in ["PowerPorts", "GroundPorts", "ClockPorts"]:
                for k in input_const.ports:
                    pwrgndclkports.add(k)
        maxrmsc = -1.0
        ...
        for input_const in constraint.expand_user_constraints(all_const):
            ...
            const = input_const.dict(
                exclude={"constraint", "_instance_attribute"}, exclude_unset=False
            )
            const["const_name"] = input_const.__class__.__name__
            ...
            if const["const_name"] == "Order":
                ...
                if const["direction"] in ("left_to_right", "horizontal"):
                    ...
                    raise NotImplementedError(
                        f'PnR does not support direction {const["direction"]} yet'
                    )
                ...
                if const["line"] not in [
                    ...
                    raise NotImplementedError(
                        f'PnR does not support edge {const["line"]} yet'
                    )
                ...
                const["const_name"] == "ChargeFlow"
                and "pin_current" in const
                and "time" in const
            ):
                time = const["time"]
                nets = dict()
                if "instances" in module:
                    for inst in module["instances"]:
                        instname = (
                            inst["instance_name"] if "instance_name" in inst else ""
                        )
                        if "fa_map" in inst:
                            for fa in inst["fa_map"]:
                                net = fa["actual"]
                                if net in pwrgndclkports:
                                    continue
                                if net not in nets:
                                    nets[net] = set()
                                nets[net].add(instname + "/" + fa["formal"])
                if "scaled_rms_charge_flow" not in const:
                    const["scaled_rms_charge_flow"] = dict()
                for net, pins in nets.items():
                    ...
                    if len(pc):
                        ...
                        for i in range(len(time)):
                            ...
                            for snk in snkcurr:
                                for src in srccurr:
                                    if (src, snk) not in ppcurrents and (
                                        snk,
                                        src,
                                    ) not in ppcurrents:
                                        ppcurrents[(src, snk)] = [
                                            0 for i in range(len(time))
                                        ]
                                    if (src, snk) in ppcurrents:
                                        ppcurrents[(src, snk)][i] = (
                                            abs(snkcurr[snk]) * srccurr[src] / sumc
                                        )
                                    elif (snk, src) in ppcurrents:
                                        ppcurrents[(snk, src)][i] = (
                                            abs(snkcurr[snk]) * srccurr[src] / sumc
                                        )
                        rmsc = dict()
                        for (src, snk) in ppcurrents:
                            rmsc[f"{src},{snk}"] = math.sqrt(
                                sum(
                                    [
                                        (time[i + 1] - time[i])
                                        * (
                                            ppcurrents[(src, snk)][i]
                                            + ppcurrents[(src, snk)][i + 1]
                                        )
                                        ** 2
                                        / 4
    ...
    def _map_pins(self, pins: list):
        ...
        for pin in pins:
            if "/" in pin:
                ...
                temp = {"type": "terminal", "name": pin, "pin": None}
            blocks.append(temp)
        return blocks
