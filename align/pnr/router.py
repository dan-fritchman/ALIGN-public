...
from .. import PnR
from .manipulate_hierarchy import (
    change_concrete_names_for_routing,
    gen_abstract_verilog_d,
    connectivity_change_for_partial_routing,
)
...
def route_single_variant(
    ...
    *,
    ...
    def RouteWork(
        ...
        current_node,
        *,
        metal_l=signal_routing_metal_l,
        metal_u=signal_routing_metal_u,
        fn="",
    ):
        curr_route.RouteWork(
            mode,
            current_node,
            drcInfo,
            metal_l,
    ...
    if not noExtra:
        ...
        if current_node.isTop:
            DB.WriteGcellGlobalRoute(
                current_node, f"{current_node.name}_GcellGlobalRoute_{lidx}.json", opath
            )
        else:
            current_node_copy = PnR.hierNode(current_node)
            DB.TransformNode(
                current_node_copy,
                current_node_copy.LL,
                current_node_copy.abs_orient,
                TransformType.Backward,
            )
            DB.WriteGcellGlobalRoute(
                current_node_copy,
                f"{current_node_copy.name}_GcellGlobalRoute_{current_node_copy.n_copy}_{lidx}.json",
                opath,
            )
    ...
    if not noExtra:
        if current_node.isTop:
            ...
            DB.WriteJSON(
                ...
                False,
                False,
                f"{current_node_copy.name}_DR_{current_node_copy.n_copy}_{lidx}",
                drcInfo,
                opath,
            )
    ...
    if current_node.isTop:
        ...
        if PDN_mode:
            ...
            if dataset_generation:
                total_current = 0.036
                current_number = 20
                DB.Write_Current_Workload(
                    current_node_copy, total_current, current_number, current_file
                )
            ...
            RouteWork(
                7,
                current_node_copy,
                metal_l=power_grid_metal_l,
                metal_u=power_grid_metal_u,
                fn=power_mesh_conffile,
            )

            logger.info("Start MNA ")
            output_file_IR = "IR_drop.txt"
            output_file_EM = "EM.txt"
            Test_MNA = PnR.MNASimulationIfc(
                current_node_copy, drcInfo, current_file, output_file_IR, output_file_EM
            )
        ...
        RouteWork(
            2, current_node, metal_l=power_grid_metal_l, metal_u=power_grid_metal_u
        )
        ...
        RouteWork(
            3,
            current_node,
            metal_l=power_routing_metal_l,
            metal_u=power_routing_metal_u,
        )
    ...
    if not noGDS:
        if current_node.isTop:
            return_name = (
                f"{current_node.name}_{lidx}" if return_name is None else return_name
            )
            DB.WriteJSON(
                current_node, True, True, True, True, return_name, drcInfo, opath
            )
            ...
            return_name = (
                f"{current_node_copy.name}_{current_node_copy.n_copy}_{lidx}"
                if return_name is None
                else return_name
            )
            DB.WriteJSON(
                current_node_copy, True, True, True, True, return_name, drcInfo, opath
            )
            current_node.gdsFile = current_node_copy.gdsFile
        ...
        if current_node.isTop:
            ...
            return_name = (
                f"{current_node.name}_{current_node.n_copy}_{lidx}"
                if return_name is None
                else return_name
            )
...
def route_bottom_up(
    *, DB, idx, opath, adr_mode, PDN_mode, skipGDS, placements_to_run, nroutings
):
    ...
    for i in TraverseOrder:
        ...
        for j in subblocks_d[i]:
            ...
            logger.info(
                f"bottom up routing for {current_node.name} ({i}) placement version {j}"
            )
            ...
            for bit, blk in enumerate(current_node.Blocks):
                ...
                if child_idx >= 0:
                    assert (
                        child_idx in new_currentnode_idx_d
                    ), f"Toporder incorrect {child_idx} {i} {TraverseOrder}"
                    assert (
                        inst_idx in new_currentnode_idx_d[child_idx]
                    ), f"subblocks_d incorrect {child_idx} {inst_idx} {subblocks_d[child_idx]}"

                    DB.CheckinChildnodetoBlock(
                        current_node,
                        bit,
                        DB.hierTree[new_currentnode_idx_d[child_idx][inst_idx]],
                        blk.instance[inst_idx].orient,
                    )
                    blk.child = new_currentnode_idx_d[child_idx][inst_idx]

            return_name = f"{current_node.name}_{j}"
            result_name = route_single_variant(
                DB,
                DB.getDrc_info(),
                current_node,
                j,
            ...
            results_name_map[result_name] = (
                (f"{current_node.name}:placement_{j}",),
                new_currentnode_idx_d[i][j],
                DB,
            )
            ...
            for blk in current_node.Blocks:
                if blk.child >= 0:
                    ...
                    DB.hierTree[blk.child].parent = list(
                        set(
                            DB.hierTree[blk.child].parent
                            + [new_currentnode_idx_d[i][j]]
                        )
                    )
                    logger.debug(
                        f"Set parent of {blk.child} to {DB.hierTree[blk.child].parent}"
                    )
...
def route_top_down_aux(
    ...
    *,
    ...
    logger.debug(
        f"Start of route_top_down; placement idx {idx} lidx {lidx} nm {current_node.name} i_copy {i_copy}"
    )
    ...
    DB.TransformNode(
        current_node, current_node.LL, current_node.abs_orient, TransformType.Forward
    )
    ...
    for bit, blk in enumerate(current_node.Blocks):
        ...
        new_childnode_idx = route_top_down_aux(
            DB,
            drcInfo,
            childnode_bbox,
            childnode_orient,
            child_idx,
            lidx,
            blk.selectedInstance,
            opath,
            adr_mode,
            PDN_mode=PDN_mode,
            results_name_map=results_name_map,
            hierarchical_path=hierarchical_path + (inst.name,),
            skipGDS=skipGDS,
        )
        DB.CheckinChildnodetoBlock(
            current_node,
            bit,
            DB.hierTree[new_childnode_idx],
            DB.hierTree[new_childnode_idx].abs_orient,
        )
    ...
    for blk in current_node.Blocks:
        ...
        logger.debug(
            f"Set parent of {blk.child} to {new_currentnode_idx} => DB.hierTree[blk.child].parent[0]={DB.hierTree[blk.child].parent[0]}"
        )

    logger.debug(
        f"End of route_top_down; placement idx {idx} lidx {lidx} nm {current_node.name} i_copy {i_copy} new_currentnode_idx {new_currentnode_idx}"
    )
...
def route_top_down(
    *, DB, idx, opath, adr_mode, PDN_mode, skipGDS, placements_to_run, nroutings
    ...
    for lidx in placements_to_run:
        sel = lidx
        new_topnode_idx = route_top_down_aux(
            DB,
            DB.getDrc_info(),
            PnR.bbox(
                PnR.point(0, 0),
                PnR.point(
                    DB.hierTree[idx].PnRAS[lidx].width,
...
def route(
    *,
    ...
    logger.info(
        f"Starting {router_mode} routing on {DB.hierTree[idx].name} {idx} restricted to {placements_to_run}"
    )

    router_engines = {
        "top_down": route_top_down,
        "bottom_up": route_bottom_up,
        "no_op": route_no_op,
    }

    return router_engines[router_mode](
        DB=DB,
        idx=idx,
        opath=opath,
        adr_mode=adr_mode,
        PDN_mode=PDN_mode,
        skipGDS=skipGDS,
        placements_to_run=placements_to_run,
        nroutings=nroutings,
    )
    ...
    *,
    ...
    for concrete_top_name, scaled_placement_verilog_d in verilog_ds_to_run:
        ...
        if True:
            ...
            abstract_verilog_file = verilog_file.replace(
                ".verilog.json", ".abstract_verilog.json"
            )

            with (pathlib.Path(fpath) / abstract_verilog_file).open("wt") as fp:
                json.dump(abstract_verilog_d.dict(), fp=fp, indent=2, default=str)

            scaled_placement_verilog_file = verilog_file.replace(
                ".verilog.json", ".scaled_placement_verilog.json"
            )

            with (pathlib.Path(fpath) / scaled_placement_verilog_file).open("wt") as fp:
                json.dump(
                    scaled_placement_verilog_d.dict(), fp=fp, indent=2, default=str
                )
        ...
        cap_ctns = {
            str(pathlib.Path(gdsFile).stem): gdsFile for atn, gdsFile in cap_map
        }
        ...
        if router_mode in ["top_down", "bottom_up"]:
            DB, new_verilog_d, new_fpath, opath, _, _ = gen_DB_verilog_d(
                toplevel_args_d,
                results_dir,
                verilog_d_in=abstract_verilog_d,
                map_d_in=map_d_in,
                lef_s_in=lef_s_in,
            )
            ...
            hierarchical_place(
                ...
                numLayout=1,
                effort=effort,
                verilog_d=abstract_verilog_d,
                lambda_coeff=1,
                scale_factor=scale_factor,
                placement_verilog_d=scaled_placement_verilog_d.dict(),
                select_in_ILP=False,
                place_using_ILP=False,
                seed=0,
                use_analytical_placer=False,
                ilp_solver="symphony",
                primitives=primitives,
                placer_sa_iterations=10000,
                placer_ilp_runtime=1,
            )
