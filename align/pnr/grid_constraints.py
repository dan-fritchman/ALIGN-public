...
def check(r, x, scaling):
    return any(
        (x % r.pitch) in ored_term.offsets and scaling in ored_term.scalings
...
def simplify(pairs):
    ...
    offsets_with_both_scalings = offsets_with_pos_scaling.intersection(
        offsets_with_neg_scaling
    )
    ...
    if offsets_with_both_scalings:
        yield OffsetsScalings(
            offsets=sorted(offsets_with_both_scalings), scalings=[1, -1]
        )
...
def merge(*rs):
    ...
    res = PlaceOnGrid(
        direction=new_direction, pitch=new_pitch, ored_terms=list(simplify(pairs))
    )
...
def gen_constraints_for_module(m, modules, leaves):
    ...
    for instance in m["instances"]:
        ...
        if constraints is not None:
            new_constraints = [
                constraint.dict() if type(constraint) != dict else constraint
        ...
        for pog in place_on_grid_constraints:
            ...
            for ored_term in pog["ored_terms"]:
                for offset, scaling in product(
                    ored_term["offsets"], ored_term["scalings"]
                ):
                    new_offset = (offset - s * o * scaling) % pog["pitch"]
                    new_scaling = s * scaling
                    new_ored_terms.append(
                        OffsetsScalings(offsets=[new_offset], scalings=[new_scaling])
                    )

            pog_constraints.append(
                PlaceOnGrid(
                    direction=pog["direction"],
                    pitch=pog["pitch"],
                    ored_terms=new_ored_terms,
                )
            )

    if pog_constraints:
        m["constraints"].extend(
            cnst.dict() for cnst in split_directions_and_merge(*pog_constraints)
        )
        logger.debug(f"{m}")
