...
def cap_placer_driver(*, toplevel_args_d, results_dir):
    ...
    logger.debug(f"Running cap_placer_driver...")
    DB, verilog_d, fpath, opath, numLayout, effort = gen_DB_verilog_d(
        toplevel_args_d, results_dir
    )

    for idx in DB.TraverseHierTree():
        logger.debug(
            f"Starting bottom-up cap placement on {DB.hierTree[idx].name} {idx}"
        )
        current_node = DB.CheckoutHierNode(idx, -1)
        DB.AddingPowerPins(current_node)
        PRC = PnR.Placer_Router_Cap_Ifc(
            opath, fpath, current_node, DB.getDrc_info(), DB.checkoutSingleLEF(), 1, 6
        )
    ...
    for module in verilog_d["modules"]:
        tbl = {
            instance["instance_name"]: instance["abstract_template_name"]
    ...
    for fn in odir.glob("*.lef"):
        if fn.is_file():
            if fn.suffixes == [".lef"]:
                ...
                if m:
                    atn = m.groups()[0]
                    logger.debug(f"Found lef file for CC capacitor {atn} {ctn}")
    ...
    assert (
        required_caps == generated_caps
    ), f"Required capacitors {required_caps} different from generated_caps {generated_caps}."
