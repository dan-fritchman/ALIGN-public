...
def check_place_on_grid(instance, constraints):
    for const in constraints:
        ...
        o, s = (
            instance["transformation"][f"o{axis}"],
            instance["transformation"][f"s{axis}"],
        )
...
def place_sequence_pair(
    ...
    for name, _ in instance_map.items():
        ...
        model += (
            model.var_by_name(f"{name}_urx") - model.var_by_name(f"{name}_llx")
            == size["x"]
        )
        model += (
            model.var_by_name(f"{name}_ury") - model.var_by_name(f"{name}_lly")
            == size["y"]
        )
        ...
        if place_on_grid and name in place_on_grid:
            ...
            for constraint in place_on_grid[name]:
                ...
                grid = model.add_var(
                    name=f"{name}_grid_{axis}", var_type=mip.INTEGER, lb=0
                )
                origin = grid * pitch + mip.xsum(v[0] * v[1] for v in offset_variables)
                model += origin - size[axis] * flip == model.var_by_name(
                    f"{name}_ll{axis}"
                )
    ...
    for index0, index1 in itertools.combinations(reverse_map, 2):
        ...
        assert name0 != name1
        if (
            instance_pos[name0] < instance_pos[name1]
            and instance_neg[name0] < instance_neg[name1]
        ):  # bb = LEFT
            model += model.var_by_name(f"{name0}_urx") <= model.var_by_name(
                f"{name1}_llx"
            )
        elif (
            instance_pos[name0] > instance_pos[name1]
            ...
            model += model.var_by_name(f"{name1}_urx") <= model.var_by_name(
                f"{name0}_llx"
            )
        elif (
            instance_pos[name0] < instance_pos[name1]
            ...
            model += model.var_by_name(f"{name1}_ury") <= model.var_by_name(
                f"{name0}_lly"
            )
        elif (
            instance_pos[name0] > instance_pos[name1]
            ...
            model += model.var_by_name(f"{name0}_ury") <= model.var_by_name(
                f"{name1}_lly"
            )
        else:
            assert False
    ...
    for constraint in constraint_schema.expand_user_constraints(constraints):
        ...
        if isinstance(constraint, constraint_schema.Boundary):
            ...
            ), f"{line} not supported. Please choose among {supported_lines.keys()}"
            axis = supported_lines[line]
            i0 = instances[0]
            for i1 in instances[1:]:
                model += model.var_by_name(f"{i0}_{axis}") == model.var_by_name(
                    f"{i1}_{axis}"
                )
            ...
            for pair in pairs:
                if len(pair) == 1:
                    ...
                    model += (1 - 1 / rel_tol) * (
                        model.var_by_name(f"{pair[0]}_ll{axis}")
                        + model.var_by_name(f"{pair[0]}_ur{axis}")
                    ) <= 2 * symmetry_line
                    model += (1 + 1 / rel_tol) * (
                        model.var_by_name(f"{pair[0]}_ll{axis}")
                        + model.var_by_name(f"{pair[0]}_ur{axis}")
                    ...
                    model += (
                        model.var_by_name(f"{pair[0]}_ll{axis}")
                        + model.var_by_name(f"{pair[0]}_ur{axis}")
                        + model.var_by_name(f"{pair[1]}_ll{axis}")
                        + model.var_by_name(f"{pair[1]}_ur{axis}")
                        == 4 * symmetry_line
                    )
            ...
            for i0, i1 in itertools.combinations(instances, 2):
                ...
                model += (
                    distance
                    - model.var_by_name(f"{i1}_ll{axis}")
                    + model.var_by_name(f"{i0}_ur{axis}")
                    - upper_bound * var
                    <= 0
                )
                model += (
                    distance
                    - model.var_by_name(f"{i0}_ll{axis}")
                    + model.var_by_name(f"{i1}_ur{axis}")
                    - upper_bound * (1 - var)
                    <= 0
                )
    ...
    if wires:
        for wire_name, instance_bbox in wires.items():
            ...
            for instance, bbox in instance_bbox:
                ...
                for (tag, axis), offset in zip(
                    itertools.product(["ll", "ur"], ["x", "y"]), bbox
                ):
                    eqn = (
                        model.var_by_name(f"{instance}_ll{axis}")
                        + offset
                        + (size[axis] - 2 * offset)
                        * model.var_by_name(f"{instance}_f{axis}")
                    )
                    model += eqn <= model.var_by_name(f"{wire_name}_ur{axis}")
                    model += model.var_by_name(f"{wire_name}_ll{axis}") <= eqn

        model += mip.xsum(
            net_priority.get(wire_name, 1) * model.var_by_name(f"{wire_name}_ur{axis}")
            for wire_name in wires
            for axis in ["x", "y"]
        ) - mip.xsum(
            net_priority.get(wire_name, 1) * model.var_by_name(f"{wire_name}_ll{axis}")
            for wire_name in wires
            for axis in ["x", "y"]
        ) == model.var_by_name(
            "HPWL"
    ...
    model.objective += (
        model.var_by_name("W")
        + model.var_by_name("H")
        + scale_hpwl * model.var_by_name("HPWL")
    )
    ...
    if status == mip.OptimizationStatus.OPTIMAL:
        ...
        print(
            f"solution with cost {model.objective_value} current lower bound: {model.objective_bound}"
        )
    else:
...
def place_using_sequence_pairs(placement_data, module, top_level):
    ...
    sequence_pairs = enumerate_sequence_pairs(
        constraints, instance_map, hyper_params.max_sequence_pairs
    )
    block_variants = enumerate_block_variants(
        constraints, instance_map, variant_counts, hyper_params.max_block_variants
    )

    solutions = list()
    for block_variant, sequence_pair in itertools.islice(
        itertools.product(block_variants, sequence_pairs), hyper_params.max_candidates
    ):
        instance_sizes = dict()
        place_on_grid = dict()
        ...
        for idx, selected in enumerate(block_variant):
            ...
            place_on_grid[instance_name] = [
                c
                for c in concrete_template.get("constraints", list())
                if c["constraint"] == "PlaceOnGrid"
            ]
            ...
            for formal_actual in instances[instance_name]["fa_map"]:
                ...
                if actual not in module["global_signals"]:
                    wires[actual].append(
                        (
                            instance_name,
                            tuple(x for x in concrete_template["pin_bbox"][formal]),
                        )
                    )

        solution = place_sequence_pair(
            constraints,
            instance_map,
            instance_sizes,
            sequence_pair,
    ...
    for i in range(max_solutions):
        ...
        modules = {
            module["concrete_name"]: module for module in placement_data["modules"]
        }
