...
def _generate_json(
    *,
    ...
    cnv, d = gen_viewer_json(
        hN,
        pdkdir=pdk_dir,
        draw_grid=True,
        json_dir=str(primitive_dir),
        extract=extract,
        input_dir=input_dir,
        toplevel=toplevel,
...
def gen_constraint_files(verilog_d, input_dir):
    pnr_const_ds = {
        module["name"]: PnRConstraintWriter().map_valid_const(
    ...
    constraint_files = {
        (input_dir / f"{nm}.pnr.const.json"): constraints
        ...
        for instance in module["instances"]:
            if "abstract_template_name" in instance:
                ...
                if atn not in non_leaves:
                    leaves_called_in_an_instance[atn].append(
                        (nm, instance["instance_name"])
                    )
        ...
        for parent, instance_name in v:
            if parent in cap_constraints:
                if instance_name in cap_constraints[parent]:
                    logger.debug(
                        f"parent: {parent} instance_name: {instance_name} leaf: {leaf} cap_constraints: {cap_constraints}"
                    )
    ...
    for k, v in primitives.items():
        ...
        for suffix in [".placement_lef", ".lef", ".json", ".gds.json"]:
            ...
            if fn.is_file():
                ...
                logger.warning(
                    f"Collateral {suffix} for leaf {leaf} not found in {primitive_dir}"
                )
    ...
    *,
    ...
    if "3_pnr:prep" in steps_to_run:
        ...
        with (input_dir / lef_file).open(mode="wt") as lp:
            logger.debug(
                f"lef files: {[pathlib.Path(v['.lef']) for k,v in leaf_collateral.items()]}"
            )
        ...
        with (input_dir / placement_lef_file).open(mode="wt") as lp:
            logger.debug(
                f"placement lef files: {[pathlib.Path(v['.placement_lef']) for k,v in leaf_collateral.items()]}"
            )
            for k, v in leaf_collateral.items():
        ...
        for k, v in leaf_collateral.items():
            for suffix in [".gds.json", ".json"]:
                if suffix in v:
                    (input_dir / f"{k}{suffix}").write_text(
                        pathlib.Path(v[suffix]).read_text()
                    )
        ...
        cap_map, cap_lef_s = cap_placer_driver(
        ...
        for nm, gdsFile in cap_map:
            ...
            for pin in ["dummy_gnd_MINUS", "dummy_gnd_PLUS"]:
                entry[pin] = any(
                    term["netName"] == pin for term in layout_d["terminals"]
                )
    ...
    if "3_pnr:place" in steps_to_run:
        ...
        top_level, leaf_map, placement_verilog_alternatives, metrics = placer_driver(
            cap_map=cap_map,
            cap_lef_s=cap_lef_s,
        ...
        with open("__placer_dump__.json", "wt") as fp:
            json.dump(
                (
                    ...
                    [
                        (nm, verilog_d.dict())
                        for nm, verilog_d in placement_verilog_alternatives.items()
                    ],
                    metrics,
                ),
                fp=fp,
        ...
        with (working_dir / "__placer_dump__.json").open("rt") as fp:
            ...
            placement_verilog_alternatives = {
                nm: VerilogJsonTop.parse_obj(v)
                for nm, v in placement_verilog_alternatives
            }

    if "3_pnr:gui" in steps_to_run:
        if gui:
            placements_to_run = startup_gui(
                top_level=top_level,
                leaf_map=leaf_map,
                lambda_coeff=lambda_coeff,
    ...
    if "3_pnr:route" in steps_to_run:
        ...
        if placements_to_run is None:
            verilog_ds_to_run = [
                (f"{top_level}_{i}", placement_verilog_alternatives[f"{top_level}_{i}"])
                for i in range(min(nroutings, len(placement_verilog_alternatives)))
            ]
        else:
            verilog_ds_to_run = [
                (f"{top_level}_{i}", placement_verilog_alternatives[f"{top_level}_{i}"])
                for i in placements_to_run
            ]
        ...
        results_name_map = router_driver(
            cap_map=cap_map,
            cap_lef_s=cap_lef_s,
