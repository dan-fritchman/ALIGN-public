...
def gen_viewer_json(
    ...
    *,
    ...
    assert (
        scale_factor == 1 or scale_factor % 2 == 0
    ), f"PDK ScaleFactor should be even."
    ...
    def add_terminal(netName, layer, b, tag=None):
        ...
        terminals.append(
            {"netName": netName, "netType": "drawing", "layer": layer, "rect": r}
        )
        ...
        if layer == "cellarea":
            ...
            if center is not None:
                lyr = (
                    layer.lower() if layer.lower() in cnv.generators else layer.upper()
                )
    ...
    for cblk in hN.Blocks:
        ...
        if json_dir is not None:
            ...
            if not pth.is_file():
                logger.debug(
                    f"{pth} is not available; not importing subblock rectangles"
                )
            else:
                found = True
        ...
        if found:
            ...
            for term in d["terminals"]:
                term["rect"] = (
                    tr3.hitRect(transformation.Rect(*term["rect"])).canonical().toList()
                )
            ...
            if "subinsts" in d:
                subinsts.update(
                    {f"{blk.name}/{nm}": v for nm, v in d["subinsts"].items()}
                )
    ...
    if global_route_json is not None:
        ...
        for (k, vv) in tbl.items():
            for v in vv:
                ...
                terminals.append(
                    {"netName": k + "_gr", "netType": "drawing", "layer": ly, "rect": r}
                )
    ...
    if False:
        nets_actual = set.union(
            {net.name for net in hN.Nets}, {net.name for net in hN.PowerNets}
        )
        nets_found = {
            term["netName"] for term in d["terminals"] if term["netName"] is not None
        }
