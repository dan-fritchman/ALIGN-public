...
def check_placement(placement_verilog_d, scale_factor):
    ...
    internal_bboxes = {
        x["concrete_name"]: x["bbox"] for x in placement_verilog_d["modules"]
    }
...
def _transform_leaf(instance, leaf):
    ...
    return {
        "concrete_name": leaf["concrete_name"],
        "name": name_new,
        "transformation": tr_new.toDict(),
    }
        ...
        for term in const["ored_terms"]:
            for offset in term["offsets"]:
                if (o - offset) % const["pitch"] == 0 and s in term["scalings"]:
                    is_satisfied = True
                    logger.debug(
                        f'{flat_leaf["name"]}@{flat_leaf["concrete_name"]} satisfied {term} in {const}'
                    )
                    break
            if is_satisfied:
                break
        ...
        if filename.exists() and filename.is_file():
            with filename.open("r") as fp:
                ...
                if "metadata" in data and "constraints" in data["metadata"]:
                    place_on_grids = [
                        c
                        for c in data["metadata"]["constraints"]
