...
def gen_transformation(blk):
    tr_reflect = transformation.Transformation.genTr(
        orient_map[blk.orient], w=blk.width, h=blk.height
    )

    tr_offset = transformation.Transformation(
        oX=blk.placedBox.LL.x - blk.originBox.LL.x,
...
def gen_placement_verilog(hN, idx, sel, DB, verilog_d):
    ...
    used_internal[abstract_template_name][concrete_template_name] = (
        idx,
        sel,
        (0, 0, hN.width, hN.height),
    )
        ...
        for blk in hN.Blocks:
            ...
            if child_idx >= 0:
                ...
                if concrete_template_name not in used_internal[abstract_template_name]:
                    ...
                    assert used_internal[abstract_template_name][
                        concrete_template_name
                    ] == (child_idx, blk.selectedInstance, new_r)
            else:
                ...
                if concrete_template_name not in used_leaves[abstract_template_name]:
                    used_leaves[abstract_template_name][concrete_template_name] = (
                        new_r,
                        pinterminals,
                    )
                else:
                    assert used_leaves[abstract_template_name][
                        concrete_template_name
                    ] == (new_r, pinterminals)
    ...
    for module in d["modules"]:
        ...
        for concrete_name, (module_idx, module_sel, module_r) in used_internal[
            abstract_name
        ].items():
            ...
            instance_map = {
                inst["instance_name"]: inst for inst in new_module["instances"]
            }
            ...
            for blk in new_hN.Blocks:
                ...
                if child_idx >= 0:
                    instance_d[
                        "concrete_template_name"
                    ] = f"{inst.master}_{blk.selectedInstance}"
                else:
                    instance_d["concrete_template_name"] = pathlib.Path(
                        inst.gdsFile
                    ).stem
...
def scalar_rational_scaling(v, *, mul=1, div=1):
    if type(mul) == float:
        ...
        if r != 0:
            logger.error(
                f"Value {v} not a whole number after scaling: q={q} r={r} mul={mul} div={div}"
            )
        return q
...
def scale_placement_verilog(placement_verilog_d, scale_factor, invert=False):
    ...
    for leaf in d["leaves"]:
        ...
        leaf["terminals"] = [
            {
                "name": t["name"],
                "rect": array_rational_scaling(t["rect"], mul=mul, div=div),
            }
...
def gen_boxes_and_hovertext(placement_verilog_d, top_cell, nets_d=None):
        ...
        for instance in module["instances"]:
            ...
            if ctn in modules:
                ...
                for terminal in leaves[ctn]["terminals"]:
                    ...
                    yield gen_pintrace_xy(rr, pinname, new_prefix_path, new_tr) + (
                        isleaf,
                        lvl,
                        True,
                    )
    ...
    if top_cell in leaves:
        yield gen_trace_xy(
            leaves[top_cell]["bbox"], top_cell, (), transformation.Transformation()
        ) + (True, 0, False)
...
def standalone_overlap_checker(placement_verilog_d, top_cell):
    def rects_overlap(rA, rB):
        return max(rA[0], rB[0]) < min(rA[2], rB[2]) and max(rA[1], rB[1]) < min(
            rA[3], rB[3]
        )
    ...
    leaves = [
        (r, hovertext)
        for r, hovertext, isleaf, _, ispin in gen_boxes_and_hovertext(
            placement_verilog_d, top_cell
        )
        if isleaf and not ispin
    ]
