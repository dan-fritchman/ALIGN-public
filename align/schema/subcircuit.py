...
class SubCircuit(Model):
    ...
    parameters: Optional[
        Dict[str, str]
    ]  # Parameter Name: Value mapping (inherits & adds to base if needed)
    elements: List[Instance]
    generator: Optional[
        Dict[str, str]
    ]  # generator name from pdk, e.g., mos, cap, res, digg2inv
    ...
    def update_element(self, name, kwargs):
        i, inst = next(
            ((i, x) for i, x in enumerate(self.elements) if x.name == name.upper()),
            None,
        )
        with set_context(self.elements):
            new_inst = Instance(
                ...
                parameters=(
                    kwargs["parameters"] if "parameters" in kwargs else inst.parameters
                ),
            )
    ...
    def xyce(self):
        ...
        ret.extend(
            [
                f".PARAM {x}=" + (f"{{{y}}}" if isinstance(y, str) else f"{y}")
    ...
    def verify(self, constraint=None):
        ...
        try:
            ...
            core = [
                x.json()
                for x in itertools.chain(self.elements, self.constraints, [constraint])
    ...
    def is_identical(self, subckt):
        subckt_match = (
            subckt.pins == self.pins
            and subckt.parameters == self.parameters
            and subckt.constraints == self.constraints
        )
        if not subckt_match:
            return False
...
class Circuit(SubCircuit):
    ...
    def name_is_unique(cls, name, values):
        assert isinstance(
            cls._validator_ctx().parent, List[Instance]
        ), "subckt can only be instanitated within List[Instance]"
        assert (
            cls._validator_ctx().parent is not None
        ), "subckt can only be instantiated within a library"
        assert (
            cls._validator_ctx().parent.find(name) is None
        ), f"Existing subckt definition found {name}"
        return name
