...
def str2float(val):
    unit = next(
        (
            x
            for x in unit_multipliers
            if val.endswith(x.upper()) or val.endswith(x.lower())
        ),
        None,
    )
...
numericval = rf"[+-]?(?:0|[1-9]\d*)(?:[.]\d+)?(?:E[+\-]?\d+)?(?:{modifiers})?"
identifier = r"[^\s{}()=;*]+"
operator = r"\s*[*+-/%]\s*"
exprcontent = (
    rf"(?:{numericval}|{identifier})(?:{operator}(?:{numericval}|{identifier}))*"
)
commentchars = r"(?:[;$]|//)"

token_re_map = {
    "ANNOTATION": rf"(^|\s)*(\*|{commentchars})+\s*\@:\s*[^\n\r]*",
    "NLCOMMENT": r"(^|[\n\r])+\*[^\n\r]*",
    "COMMENT": rf"(^|\s)*{commentchars}[^\n\r]*",
    "CONTINUE": r"(^|[\n\r])+\+",
    "CONTINUEBACKSLASH": r"\\\s*[\n\r]",
    "NEWL": r"[\n\r]+",
    "EQUALS": r"\s*=\s*",
    "EXPR": rf"""(?P<quote>['"]){exprcontent}(?P=quote)|({{){exprcontent}(}})""",
    "NUMBER": numericval + rf"(?=\s|\Z|{commentchars})",
    "DECL": rf"\.{identifier}",
    "NAME": identifier,
    "WS": r"\s+",
}
spice_pat = re.compile(
    "|".join(rf"(?P<{x}>{y})" for x, y in token_re_map.items()), flags=re.IGNORECASE
)

constraint_dict = {
    x: getattr(constraint, x) for x in dir(constraint) if not x.startswith("_")
}
...
class SpiceParser:
    ...
    def _decompose(cache):
        ...
        args = [
            SpiceParser._cast(x.value.upper(), x.type)
            for i, x in enumerate(cache)
            if len(assignments.intersection({i - 1, i, i + 1})) == 0
        ]
        kwargs = {
            cache[i - 1].value.upper(): SpiceParser._cast(
                cache[i + 1].value.upper(), cache[i + 1].type
            )
    ...
    def _process_instance(self, name, args, kwargs):
        ...
        if self.library.find(model):
            ...
            logger.warning(
                f"Unknown device found {model} {kwargs}, creating a generic model for this"
            )
            with set_context(self.library):
                ...
                self.library.append(
                    Model(
                        name=model,
                        pins=pins,
                        parameters={k: "1" for k in kwargs.keys()},
                        prefix="",
                    )
                )
            model = self.library.find(model)
        ...
        with set_context(self._scope[-1].elements):
            try:
                self._scope[-1].elements.append(
                    Instance(name=name, model=model.name, pins=pins, parameters=kwargs)
                )
            except ValueError:
                assert (
                    False
                ), f"could not identify device parameters {name} {kwargs} \
    ...
    def _process_declaration(self, decl, args, kwargs):
        if decl == ".SUBCKT":
            ...
            assert not isinstance(
                self.library.find(name), SubCircuit
            ), f"User is attempting to redeclare subcircuit {name}"
            with set_context(self.library):
            ...
            assert (
                len(args) == 0
            ), f"unsupported arguments {args}, probably missing default values"
            ...
            assert (
                self.library.find(name) is None
            ), f"User is attempting to redeclare {name}"
            assert (
                self.library.find(base) is not None
            ), f"Base model {base} not found for model {name}"
            with set_context(self.library):
