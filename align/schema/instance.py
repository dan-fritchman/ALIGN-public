...
class Instance(types.BaseModel):
    ...
    def model_exists_in_library(cls, model):
        ...
        ), "Instance can only be instanitated within List[Instance]"
        assert (
            cls._validator_ctx().parent.parent is not None
        ), "List[Instance] can only be instantiated within a SubCircuit / Circuit"
        assert (
            cls._validator_ctx().parent.parent.parent is not None
        ), "Circuit / SubCircuit constaining instance must be registered to a Library"
        assert (
            cls._get_model(cls._validator_ctx().parent.parent.parent, model) is not None
    ...
    def name_complies_with_model(cls, name, values):
        ...
        model = cls._get_model(
            cls._validator_ctx().parent.parent.parent, values["model"]
        )
        name = name.upper()
    ...
    def parameters_comply_with_model(cls, parameters, values):
        ...
        if parameters:
            ...
            assert model.parameters and set(parameters.keys()).issubset(
