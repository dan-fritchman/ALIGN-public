...
class LayerMetal(Layer):
    ...
    @validator(
        "min_length", "min_end_to_end", "width", "space", "stop_pitch", "stop_point"
    )
    ...
    def _validate_width(cls, v, values):
        if isinstance(v, List):
            assert len(v) == len(
                values["width"]
            ), f"width and space length should match"
        return v
...
class PDK(BaseModel):
    ...
    def _validate_via(cls, layers):
        for key, via in layers.items():
            if isinstance(via, LayerVia):
                ...
                assert (
                    ml in layers
                ), f"Lower layer {ml} not found for {key} {layers.keys()}"
                assert mh in layers, f"Higher layer {mh} not found for {key}"
                assert (
                    layers[ml].direction != layers[mh].direction
                ), f"Lower and higher layer directions are not orthogonal"
    ...
    def generate_adr_collaterals(
        ...
        with open(write_path / "adr_design_rules.txt", "wt") as fp:
            for name, layer in self.layers.items():
                if isinstance(layer, LayerMetal):
                    fp.write(
                        f"Rule name={name}_minete type=minete value={layer.min_end_to_end} layer={name}\n"
                    )
                    fp.write(
                        f"Rule name={name}_minlength type=minlength value={layer.min_length} layer={name}\n"
                    )
                ...
                if isinstance(layer, LayerMetal):
                    ...
                    line += " stops=%s" % (
                        ",".join(
                            str(i)
                            for i in [
                                layer.stop_pitch - 2 * layer.stop_point,
                                2 * layer.stop_point,
                            ]
                        )
                    )
        ...
        def _via_string(via: LayerVia):
            ...
            via_str += (
                f'    widths value={",".join(str(i) for i in via.layer_l_width)}\n'
            )
            via_str += f"  }}\n"
            ...
            via_str += (
                f'    widths value={",".join(str(i) for i in via.layer_h_width)}\n'
            )
            via_str += f"  }}\n"
