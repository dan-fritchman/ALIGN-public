...
class Model(types.BaseModel):
    ...
    def base_check(cls, base):
        ...
        assert (
            library is not None
        ), "Could not retrieve parent scope. Please register to library."
        base_ptr = cls._get_base_model(library, base.upper())
        assert (
            base_ptr is not None
        ), f"Could not find base model {base} in libary {library}"
        return base.upper()
    ...
    def pin_check(cls, pins, values):
        if "base" not in values or not values["base"]:
            assert (
                pins
            ), "Pins must be specified for base models. Did something go wrong in base?"
            assert (
                len(pins) >= 1
            ), "Instance must have at least one terminals (e.g., dummies have one terminal)"
            pins = [p.upper() for p in pins]
        elif pins:
            logger.error(f"Inheriting from {values['base'].name}. Cannot add pins")
            raise AssertionError(
                f"Inheriting from {values['base'].name}. Cannot add pins"
            )
        else:
            pins = cls._get_base_model(
                cls._validator_ctx().parent, values["base"]
            ).pins.copy()
        return pins
    ...
    def parameter_check(cls, parameters, values):
        parameters = (
            {k.upper(): v.upper() for k, v in parameters.items()} if parameters else {}
        )
        if "base" in values and values["base"]:
            x = cls._get_base_model(
                cls._validator_ctx().parent, values["base"]
            ).parameters.copy()
            x.update(parameters)
            parameters = x
