...
class ConstraintTranslator:
    ...
    def _top_to_bottom_translation(self, node_map):
            ...
            with set_context(self.child_const):
                for const in list(self.parent_const):
                    if any(
                        ...
                        isinstance(const, constraint.ConfigureCompiler)
                        and const.propagate
                    ):
                        self.child_const.append(const)
                    elif hasattr(const, "instances") and not isinstance(
                        const, constraint.GroupBlocks
                    ):
                        ...
                        logger.debug(
                            f"transferred constraint instances {node_map} from {const} to {_child_const}"
                        )
        ...
        for const in list(self.parent_const):
            if hasattr(const, "pin_current"):
                logger.debug(
                    f"transferring charge flow constraints {const.pin_current.keys()}"
                )
                pin_map = {}
                for pin in const.pin_current.keys():
                    ...
                    if parent_inst_name in node_map.keys():
                        ...
                        assert (
                            parent_pin in self.child.get_element(child_inst_name).pins
                        )
                        pin_map[pin] = child_inst_name + "/" + parent_pin
                _child_const = {
                    x: {
                        pin_map[pin]: current
                        for pin, current in getattr(const, x).items()
                        if pin in pin_map
                    }
                    if x == "pin_current"
                    else getattr(const, x)
        ...
        if self.child_const:
            logger.debug(
                f"transferred constraints to {self.child_name} {self.child_const}"
            )
    ...
    def _update_const(self, new_inst_name, node_map):
        ...
        logger.debug(
            f"update constraints of {self.parent_name} with {new_inst_name} map {node_map}"
        )
        for const in self.parent_const:
            if hasattr(const, "instances") and not isinstance(
                const, constraint.GroupBlocks
            ):
                ...
                for pair in const.pairs:
                    if len(pair) == 2:
                        if pair[0] in node_map.keys() and pair[1] in node_map.keys():
                            ...
                            pair[0] in node_map.keys()
                            and pair[1] not in node_map.keys()
                        ):
                            pair[0] = new_inst_name
                        elif (
                            pair[1] in node_map.keys()
                            and pair[0] not in node_map.keys()
                        ):
                ...
                logger.debug(
                    f"updating charge flow constraints {const.pin_current.keys()}"
                )
                updated_pc = {}
                remove_pins = []
                for pin, current in const.pin_current.items():
                    ...
                    if parent_inst_name in node_map.keys():
                        ...
                        child_inst_name = node_map[parent_inst_name]
                        if self.child_name:
                            new_parent_pin = self.child.get_element(
                                child_inst_name
                            ).pins[pin.split("/")[1]]
                        else:
                            raise NotImplementedError(
                                "Groupcap does not have child hieararchy"
                            )
                        if new_inst_name + "/" + new_parent_pin in updated_pc:
                            for id, cur in enumerate(
                                updated_pc[new_inst_name + "/" + new_parent_pin]
                            ):
                                updated_pc[new_inst_name + "/" + new_parent_pin][id] = (
                                    cur + current[id]
                                )
                        else:
                ...
                for pin in const.pins1 + const.pins2:
                    if "/" in pin:
                        if self.child:
                            ...
                            if parent_inst_name in node_map.keys():
                                replace_pins[pin] = pin.replace(
                                    parent_inst_name, new_inst_name
                                )
    ...
    def _get_pin_map(self, pin, node_map, new_inst_name):
        ...
        if parent_inst_name in node_map.keys():
            ...
            if self.child_name:  # no child during renaming
                assert self.child.get_element(
                    child_inst_name
                ), f"child does not have instance {child_inst_name}"
