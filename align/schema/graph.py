...
class Graph(networkx.Graph):
    ...
    def remove(self, element):
        ...
        self.remove_nodes_from(
            [x for x in self.neighbors(element.name) if self.degree(x) == 1]
        )
    ...
    def default_node_match(x, y):
        ...
        if isinstance(x.get("instance"), Instance) and isinstance(
            y.get("instance"), Instance
        ):
            return y.get("instance").model in x.get("instance").mclass.bases + [
                x.get("instance").model
            ]
        else:
    ...
    def find_subgraph_matches(self, graph, skip=None, node_match=None, edge_match=None):
        ...
        matches = sorted(
            ...
            key=lambda k: [(x, y) for x, y in k.items()],
        )
        for match in matches:
            if skip and any(k in skip for k in match.keys()):
                ...
                continue
            if not any(
                self._is_element(self.nodes[node])
                and any(node in x for x in correct_matches)
                for node in match
            ):
                try:
                    ...
                    logger.debug(
                        f"skipping match {graph.subckt.name} {match.keys()} due to unsatisfied constraints"
                    )
                    pass
        ...
        with set_context(self.subckt.constraints):
            for const in subckt_const:
                if isinstance(const, constraint.SymmetricBlocks):
                    t = [
                        [self._get_key(ele, match) for ele in pair]
                    ...
                    assert (
                        x in self.subckt.constraints
                    ), f"constraint: {x} not found in {self.subckt.constraints}"
                    self.subckt.constraints.remove(x)
                elif isinstance(const, constraint.SymmetricNets):
                    pair = [
                        self._get_key(const.net1, match),
                        self._get_key(const.net2, match),
                    ]
    ...
    def all_neighbors_dist(self, pair):
        ...
        logger.debug(
            f"All neighbors of {pair[0]}: {nbrs1_type} , {pair[1]}: {nbrs2_type}"
        )
    ...
    def replace_matching_subgraph(
        self, subgraph, skip=None, node_match=None, edge_match=None
    ):
    ...
    def _replace_matches_with_subckt(self, matches, subckt, skip=None):
        ...
        for match in matches:
            ...
            assert len(subckt.pins) == len(
                set(subckt.pins)
            ), f"duplicate pins found in module {subckt.name}, {subckt.pins}"
            ...
            if not all(
                x in match for node in removal_candidates for x in self.neighbors(node)
            ):
                continue
            # Remove nodes not on subckt boundary
            if (
                skip
                and set(removal_candidates) & set(skip)
                and len(removal_candidates) > 1
            ):
                continue
            ...
            with set_context(self.subckt.constraints):
                gc = [
                    c for c in subckt.constraints if isinstance(c, constraint.Generator)
                ]
                if gc:
                ...
                if len(nodes) >= 1:
                    temp = constraint.GroupBlocks(
                        instance_name=instance_name,
                        template_name=subckt.name,
                        instances=nodes,
                        generator=generator_param,
                    )
                    logger.debug(
                        f"adding a generator parameter {temp} for {self.subckt.name}"
                    )
            ...
            logger.debug(
                f"adding instance {instance_name} of type {new_subckt.name} in subckt {self.name}"
            )
            self.add_instance(
                name=instance_name, model=new_subckt.name, pins=pin2net_map
            )
            if self.subckt.name:
                tr = ConstraintTranslator(
                    self.subckt.parent, self.subckt.name, new_subckt
                )
    ...
    def create_subckt_instance(self, subckt, match):
        ...
        with set_context(subckt_instance.elements):
            for x, y in match.items():
                ...
                subckt_instance.elements.append(
                    Instance(
                        name=element.name,
        ...
        if self.subckt.name:
            tr = ConstraintTranslator(
                self.subckt.parent, self.subckt.name, subckt_instance
            )
            tr._top_to_bottom_translation(match)
        ...
        while len(worklist) > 0:
            ...
            if len(netlist.elements) > 1:
                net2pin_map = {
                    ...
                    for x, y in enumerate(
                        (
                            ...
                            if not all(
                                neighbor in netlist.nodes
                                for neighbor in self.neighbors(net)
                            )
                        )
                    )
                }
                with set_context(self.subckt.parent):
                    subckt, index = (
                        SubCircuit(
                            name=f"XREP{index}", pins=list(net2pin_map.values())
                        ),
                        index + 1,
                    )
                    ...
                    with set_context(subckt.elements):
                        subckt.elements.append(
                            Instance(
                                ...
                                model=element.model,
                                pins={
                                    pin: net2pin_map[net] if net in net2pin_map else net
                                    for pin, net in element.pins.items()
                                },
                            )
                        )
                ...
                worklist = [
                    ...
                    if not any(element.name in match for match in matches)
                ]
                if replace:
    ...
    def _get_match_candidates(self, worklist, netlist):
        ...
        matchlist = [
            element
            for element in worklist
            if element.name not in netlist
            and any(x in netlist for x in self.neighbors(element.name))
        ]
    ...
    def flatten(self, depth=999):
        ...
        depth = depth - 1
        for subcktinst in (
            x for x in self.elements if isinstance(x.mclass, SubCircuit)
        ):
    ...
    def _replace_subckt_with_components(self, subcktinst):
        ...
        for element in subcktinst.mclass.elements:
            self.add_instance(
                ...
                model=element.model,
                pins={
                    pin: subcktinst.pins[net]
                    if net in subcktinst.pins
                    else f"{subcktinst.name}_{net}"
                    for pin, net in element.pins.items()
                },
                parameters={
                    key: eval(val, {}, subcktinst.parameters)
