...
def make_tradeoff_fig_wh(df, log=False, scale="Blugrn"):
    ...
    fig.add_trace(
        go.Scatter(x=sweep_width, y=sweep_height, mode="lines", showlegend=False)
    )
    ...
    if log:
        ...
        fig.update_xaxes(type="log", range=[log_min, log_max])
        fig.update_yaxes(
            type="log", range=[log_min, log_max], scaleanchor="x", scaleratio=1
        )
    else:
        linear_min = 0
        linear_max = max(max_width, max_height) * 1.1

        fig.update_xaxes(range=[linear_min, linear_max])
        fig.update_yaxes(range=[linear_min, linear_max], scaleanchor="x", scaleratio=1)
...
def make_tradeoff_fig_aa(df, log=False, scale="Blugrn"):
    ...
    fig.add_trace(go.Scatter(x=sweep_x, y=sweep_y, mode="lines", showlegend=False))
...
def define_axes(fig, log, max_x, max_y, *, log_one_to_one=False):
    if log:
        fig.update_xaxes(type="log")
        if log_one_to_one:
            fig.update_yaxes(type="log", scaleanchor="x", scaleratio=1)
        else:
            fig.update_yaxes(type="log")
    else:
...
def define_colorscale(fig, col):
    ...
    if min_c == max_c:
        fig.update_coloraxes(cmin=min_c, cmax=max_c, showscale=False)
    else:
        fig.update_coloraxes(cmin=min_c, cmax=max_c)
...
class AppWithCallbacksAndState:
    def gen_dataframe(self):
        data = [
            {"abstract_template_name": atn, "concrete_template_name": ctn, **m}
            for atn, v in self.tagged_bboxes.items()
            for ctn, (m, _, _) in v.items()
        ]
    ...
    def __init__(self, *, tagged_bboxes, module_name, lambda_coeff):
        ...
        self.gen_dataframe()
        self.tradeoff = make_tradeoff_fig(
            self.axes, self.df, log=True, lambda_coeff=lambda_coeff
        )
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        dcc.RadioItems(
                            id="axes-type",
                            options=[
                                {"label": i, "value": i} for i in ["linear", "loglog"]
                            ],
                            value="loglog",
                        ),
                        dcc.Dropdown(
                        ...
                        dcc.Graph(id="tradeoff-graph", figure=self.tradeoff),
                    ],
                ),
                html.Div(
                    id="placement-col",
                    children=[
                        ...
                        dcc.RadioItems(
                            ...
                            options=[
                                {"label": i, "value": i}
                                for i in ["All", "Direct", "Leaves Only"]
                            ],
                            value="Direct",
                        ),
                        dcc.RadioItems(
                            ...
                            options=[
                                {"label": i, "value": i} for i in ["No Pins", "Pins"]
                            ],
                            value="Pins",
                        ),
                        dcc.Dropdown(id="netname", options=[], multi=True, value=[]),
                        dcc.Graph(id="Placement", figure=self.placement_graph),
                    ],
                ),
                html.Div(
                    id="tree-col",
                    children=[html.Img(src=self.app.get_asset_url("align.png"))],
                ),
            ],
        )
        ...
        self.app.callback(
            ...
            [
                ...
                Input("module-name", "value"),
            ],
        )(self.change_colorscale)

    def make_placement_graph(
        self, *, display_type="Direct", display_pins_type="Pins", netname=None
    ):
        sel = self.sel
        title = self.title
        ...
        if sel is not None:
            ...
            dump_blocks(
                ...
                leaves_only,
                levels,
                netname if display_pins_type == "Pins" else None,
            )
            title_d = dict(text=sel if title is None else title)

        fig.update_layout(autosize=False, width=600, height=600, title=title_d)

        max_x = max(
            m["width"] for _, (m, _, _) in self.tagged_bboxes[self.module_name].items()
        )
        max_y = max(
            m["height"] for _, (m, _, _) in self.tagged_bboxes[self.module_name].items()
        )

        fig.update_xaxes(tickvals=[0, max_x], range=[0, max(max_x, max_y)])

        fig.update_yaxes(tickvals=[0, max_y], range=[0, max(max_x, max_y)])
    ...
    def display_hover_data(
        self, clickData, hoverData, display_type, display_pins_type, netname
    ):
        ...
        if clickData is not None:
            [idx, curve_idx, x, y] = [
                clickData["points"][0][t]
                for t in ["pointNumber", "curveNumber", "x", "y"]
            ]

        if hoverData is not None:
            [idx, curve_idx, x, y] = [
                hoverData["points"][0][t]
                for t in ["pointNumber", "curveNumber", "x", "y"]
            ]
        ...
        if display_type_change:
            self.make_placement_graph(
                ...
                display_pins_type=display_pins_type,
                netname=netname,
            )
        elif (clickData is not None or hoverData is not None) and curve_idx == 0:
...
def run_gui(*, tagged_bboxes, module_name, lambda_coeff):
    awcas = AppWithCallbacksAndState(
        tagged_bboxes=tagged_bboxes, module_name=module_name, lambda_coeff=lambda_coeff
    )
    ...
    awcas.app.run_server(
        debug=True, use_reloader=False, host=fully_qualified_domain_name, port=free_port
    )

    logger.info(
        f"final selection: {awcas.sel} We have access to any state from the GUI object here."
    )
    return awcas.sel
