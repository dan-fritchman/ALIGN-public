...
def limit_pairs(pairs):
    ...
    if len(pairs) > 12:
        ...
        for val in log10_aspect_ratios:
            best_pair = min(
                (abs(log10(newy) - log10(newx) - val), (newx, newy))
...
def gen_param(subckt, primitives, pdk_dir):
    ...
    if get_generator(generator_name.lower(), pdk_dir):
        ...
        for e in subckt.elements:
            assert (
                vt == e.model
            ), f"Primitive with different models not supported {vt} vs {e.model}"
            if values != e.parameters:
                assert (
                    values == e.parameters
                ), f"Primitive with different parameters not supported {values} vs {e.parameters}"
        ...
        if st > 1 and (nf > 1 or pl > 1):
            assert (
                False
            ), "Stacking multi-leg transistors not supported. Turn off MergeSeriesDevices"
        elif pl > 1 and nf != pl:
