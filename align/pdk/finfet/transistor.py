...
class MOS(CanvasPDK):
    ...
    def mos(self, tx: Transistor, track_pattern=None):
        ...
        assert tx.nf % 2 == 0, "Odd number of fingers are not allowed"
        ...
        if track_pattern is not None:
            ...
            if "G" in track_pattern:
                for y in track_pattern["G"]:
                    if tx.nf > 2:
                        ...
                        g_b_idx = (1, -1)
                        g_e_idx = (1 + tx.nf, 1)
                    self.addWire(self.m2, "G", y, g_b_idx, g_e_idx)
            ...
            if "S" in track_pattern:
                for y in track_pattern["S"]:
                    self.addWire(self.m2, "S", y, s_b_idx, s_e_idx)

            if "D" in track_pattern:
                for y in track_pattern["D"]:
                    self.addWire(self.m2, "D", y, d_b_idx, d_e_idx)
        ...
        return {"bbox": [0, 0, x1, y1], "instance": {}, "terminals": self.terminals}
    ...
    def tap(self, tx: Transistor):
        ...
        if self.tx.model_name.startswith("n"):
            ...
            self.addWire(self.m2, "B", 2, (1, -1), (1 + tx.nf, 1))
            self.drop_via(self.v1)
            ...
            for x in range(1, 2 + tx.nf, 2):
                self.addWire(self.m1, "B", x, (1, -1), (6, 1))
            self.addWire(self.m2, "B", 4, (1, -1), (1 + tx.nf, 1))
            self.drop_via(self.v1)
