...
class ThinFilmResistor(CanvasPDK):
    ...
    def generate(
        self, ports, netlist_parameters=None, layout_parameters=None, *args, **kwargs
    ):
        ...
        self.metadata["constraints"] = [
            PlaceOnGrid(
                direction="H",
                pitch=2 * row_height,
                ored_terms=[OffsetsScalings(offsets=[1 * row_height], scalings=[1])],
            ).dict(),
            PlaceOnGrid(
                direction="V",
                pitch=5 * poly_pitch,
                ored_terms=[OffsetsScalings(offsets=[1 * poly_pitch], scalings=[1])],
            ).dict(),
        ]
        return {
            "bbox": bbox,
