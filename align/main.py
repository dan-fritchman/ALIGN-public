        ...
        if step in sub_steps:
            for sub_step in sub_steps[step]:
                ...
                if (step, sub_step) == stop_t:
                    assert (
                        enabled
                    ), f"Stopping flow before it started: {flow_start} {flow_stop}"
                    enabled = False
        else:
            if enabled:
...
def extract_netlist_files(netlist_dir, netlist_file):
    ...
    if not netlist_dir.is_dir():
        logger.error(
            f"Netlist directory {netlist_dir} doesn't exist. Please enter a valid directory path"
        )
        raise FileNotFoundError(2, "No such netlist directory", netlist_dir)

    netlist_files = [
        x for x in netlist_dir.iterdir() if x.is_file() and x.suffix in (".sp", ".cdl")
    ]
    if not netlist_files:
        ...
        raise FileNotFoundError(
            2, "No matching netlist files", f"{netlist_dir}/*(.sp|.cdl)"
        )
...
def start_viewer(working_dir, pnr_dir, variant):
    ...
    shutil.copy(
        pnr_dir / f"{variant}.json", local_viewer_dir / "INPUT" / f"{variant}.json"
    )

    stderr = sys.stderr
    Handler = functools.partial(
        http.server.SimpleHTTPRequestHandler, directory=str(working_dir / "Viewer")
    )
    with socketserver.TCPServer(("localhost", 0), Handler) as httpd:
        logger.info(
            f"Please view layout at http://localhost:{httpd.server_address[1]}/?design={variant}"
        )
...
def schematic2layout(
    ...
    if not working_dir.is_dir():
        logger.error(
            f"Working directory {working_dir} doesn't exist. Please enter a valid directory path."
        )
    ...
    if not pdk_dir.is_dir():
        logger.error(
            f"PDK directory {pdk_dir} doesn't exist. Please enter a valid directory path"
        )
    ...
    if "1_topology" in steps_to_run:
        ...
        primitive_lib = generate_hierarchy(
            netlist, subckt, topology_dir, flatten, pdk_dir
        )
    else:
        if subckt is None:
    ...
    if "2_primitives" in steps_to_run:
        ...
        primitives = generate_primitives(
            primitive_lib, pdk_dir, primitive_dir, netlist_dir
        )
    ...
    if sub_steps:
        ...
        assert (
            gui
            or router_mode in ["collect_pins", "no_op"]
            or "3_pnr:route" not in sub_steps
            or len(variants) > 0
        ...
        for variant, filemap in variants.items():
            if filemap["errors"] > 0:
                (working_dir / filemap["errfile"].name).write_text(
                    filemap["errfile"].read_text()
                )
            ...
            if viewer:
                ...
                shutil.copy(
                    pnr_dir / f"{variant}.json",
                    pathlib.Path(os.getenv("ALIGN_HOME"))
                    / "Viewer"
                    / "INPUT"
                    / f"{variant}.json",
                )
            ...
            if "gdsjson" in filemap:
                ...
                print(
                    "Use KLayout to visualize the generated GDS:",
                    working_dir / f"{variant}.gds",
                )

            assert skipGDS or "python_gds_json" in filemap
            if "python_gds_json" in filemap:
                convert_GDSjson_GDS(
                    filemap["python_gds_json"], working_dir / f"{variant}.python.gds"
                )
                print(
                    "Use KLayout to visualize the python generated GDS:",
                    working_dir / f"{variant}.python.gds",
                )

            assert skipGDS or "lef" in filemap
            if "lef" in filemap:
                (working_dir / filemap["lef"].name).write_text(
                    filemap["lef"].read_text()
                )

            if extract:
                (working_dir / filemap["cir"].name).write_text(
                    filemap["cir"].read_text()
                )
            ...
            if regression:
                (regression_dir / filemap["gdsjson"].name).write_text(
                    filemap["gdsjson"].read_text()
                )
                (regression_dir / filemap["python_gds_json"].name).write_text(
                    filemap["python_gds_json"].read_text()
                )
                convert_GDSjson_GDS(
                    filemap["python_gds_json"], regression_dir / f"{variant}.python.gds"
                )
                convert_GDSjson_GDS(
                    filemap["gdsjson"], regression_dir / f"{variant}.gds"
                )
                (regression_dir / filemap["lef"].name).write_text(
                    filemap["lef"].read_text()
                )
                (regression_dir / f"{subckt}.v").write_text(
                    (topology_dir / f"{subckt}.v").read_text()
                )
