...
class GEN_PRIMITIVE_FROM_GDS:
    def __init__(self, gdsdir: str, layers: str, primdir: str, topodir: str):
        ...
        if not self.update_primitives_json(primdir) or not self.add_primitive_files(
            gdsdir, layers, primdir
        ):
            return
            ...
            if vjson.endswith(".verilog.json"):
                ...
                if "modules" in vjdata:
                    for m in vjdata["modules"]:
                        if (
                            "instances" in m
                            and len(m["instances"]) == 0
                            and "constraints" in m
                        ):
        ...
        for primlib in os.listdir(topodir):
            if primlib == "__primitives_library__.json":
                ...
                for m in self._removedmodules:
                    if len(m) == 2:
                        pdata.append({"name": m[0], "pins": m[1]})
        ...
        if os.path.isfile(primjson):
            with open(primjson) as fp:
                ...
                for m in self._removedmodules:
                    if len(m) == 2:
                        pdata[m[0]] = {
                            "abstract_template_name": m[0],
                            "concrete_template_name": m[0],
                        }
                if len(self._removedmodules):
