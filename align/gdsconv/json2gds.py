            ...
            if "elements" in cell:
                for elem in cell["elements"]:
                    ...
                    if t == "boundary":
                        ...
                        if "layer" in elem:
                            store_data("LAYER", str(elem["layer"]))
                        if "datatype" in elem:
                            store_data("DATATYPE", str(elem["datatype"]))
                        if "xy" in elem:
                            store_data("XY", unbracket(elem["xy"]))
                        if "propattr" in elem:
                            store_data("PROPATTR", str(elem["propattr"]))
                        if "propvalue" in elem:
                            store_data("PROPVALUE", quote(elem["propvalue"]))
                    elif t == "path":
                        ...
                        if "datatype" in elem:
                            store_data("DATATYPE", str(elem["datatype"]))
                        if "pathtype" in elem:
                            store_data("PATHTYPE", str(elem["pathtype"]))
                        if "width" in elem:
                            store_data("WIDTH", str(elem["width"]))
                        if "bgnextn" in elem:
                            store_data("BGNEXTN", str(elem["bgnextn"]))
                        if "endextn" in elem:
                            store_data("ENDEXTN", str(elem["endextn"]))
                        if "xy" in elem:
                            store_data("XY", unbracket(elem["xy"]))
                    elif t == "text":
                        ...
                        if "layer" in elem:
                            store_data("LAYER", str(elem["layer"]))
                        if "texttype" in elem:
                            store_data("TEXTTYPE", str(elem["texttype"]))
                        if "presentation" in elem:
                            store_data("PRESENTATION", str(elem["presentation"]))
                        if "strans" in elem:
                            store_data("STRANS", str(elem["strans"]))
                        if "mag" in elem:
                            store_data("MAG", str(elem["mag"]))
                        if "angle" in elem:
                            store_data("ANGLE", str(elem["angle"]))
                        if "xy" in elem:
                            store_data("XY", unbracket(elem["xy"]))
                        if "string" in elem:
                            store_data("STRING", quote(elem["string"]))
                    elif t == "sref":
                        store_data("SREF", None)
                        if "sname" in elem:
                            store_data("SNAME", quote(elem["sname"]))
                        if "strans" in elem:
                            store_data("STRANS", str(elem["strans"]))
                        if "angle" in elem:
                            store_data("ANGLE", str(elem["angle"]))
...
def convert_GDSjson_GDS(name, oname):
    with open(name, "rt") as ifile, open(oname, "wb") as ofile:
