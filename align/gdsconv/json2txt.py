                ...
                if "elements" in cell:
                    for elem in cell["elements"]:
                        ...
                        if t == "boundary":
                            oprint("BOUNDARY")
                            if "layer" in elem:
                                oprint("LAYER:", bracket(elem["layer"]))
                            if "datatype" in elem:
                                oprint("DATATYPE:", bracket(elem["datatype"]))
                            if "xy" in elem:
                                oprint("XY:", unbracket(elem["xy"]))
                            if "propattr" in elem:
                                oprint("PROPATTR:", bracket(elem["propattr"]))
                            if "propvalue" in elem:
                                oprint("PROPVALUE:", quote(elem["propvalue"]))
                        elif t == "text":
                            oprint("TEXT")
                            if "layer" in elem:
                                oprint("LAYER:", bracket(elem["layer"]))
                            if "texttype" in elem:
                                oprint("TEXTTYPE:", bracket(elem["texttype"]))
                            if "presentation" in elem:
                                oprint("PRESENTATION:", elem["presentation"])
                            if "strans" in elem:
                                oprint("STRANS:", elem["strans"])
                            if "mag" in elem:
                                oprint("MAG:", bracket(elem["mag"]))
                            if "angle" in elem:
                                oprint("ANGLE:", bracket(elem["angle"]))
                            if "xy" in elem:
                                oprint("XY:", unbracket(elem["xy"]))
                            if "string" in elem:
                                oprint("STRING:", quote(elem["string"]))
                        elif t == "sref":
                            oprint("SREF")
                            if "sname" in elem:
                                oprint("SNAME:", quote(elem["sname"]))
                            if "strans" in elem:
                                oprint("STRANS:", elem["strans"])
                            if "angle" in elem:
                                oprint("ANGLE:", bracket(elem["angle"]))
