...
class GDS2_LEF_JSON:
    def __init__(self, layerfile, gdsfile, name):
        ...
        self._cellname = (
            ...
            if name
                ...
                self._cell.name
                if self._cell
                else gdsfile[(gdsfile.find("/") + 1) : gdsfile.find(".gds")]
            )
        )
    ...
    def writeLEFJSON(self, outdir):
        ...
        with open(outdir + leffile, "wt") as ofs:
                        ...
                        if key in polygons:
                            for poly in polygons[key]:
                                if len(poly) < 2:
                                    continue
                                box = [
                                    round(min(r[0] for r in poly) * 1e9),
                                    round(min(r[1] for r in poly) * 1e9),
                                    round(max(r[0] for r in poly) * 1e9),
                                    round(max(r[1] for r in poly) * 1e9),
                                ]
                                if (
                                    box[0] <= pos[0]
                                    and box[1] <= pos[1]
                                    ...
                                    ofs.write(
                                        f"        RECT {box[0]} {box[1]} {box[2]} {box[3]} ;\n"
                                    )
                                    ofs.write(f"    END\n  END {lbl.text}\n")
                                    pindict = {
                                        "layer": lname,
            ...
            ofs.write("  OBS\n")
            for k in polygons:
                if k[0] not in self._layernames:
                    continue
                lname = self._layernames[k[0]]
                if (
                    lname not in self._layers
                    or k[1] not in self._layers[lname]
                    or lname.lower() == "bbox"
                ):
                    continue
                for poly in polygons[k]:
                    ...
                    if (
                        "M" in lname
                        or "V" in lname
                        and (self._layers[lname][k[1]].lower() not in ("label"))
                    ):
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "-o", "--outdir", type=str, default="", help="<file output directory>"
    )
    ap.add_argument(
        "-n", "--name", type=str, default="", help="<name to use for output module>"
    )
    args = ap.parse_args()
