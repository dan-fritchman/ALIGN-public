        ...
        for line in ifile:
            ...
            if tag_type == types.NODATA:
                if isElement(tag_name):
                    ...
                    first = True
                    indentPrint(
                        level, first, quote("type"), ": ", quote(tag_name.lower())
                    )
                    first = False
                elif (
                    tag_name == "ENDEL" or tag_name == "ENDSTR" or tag_name == "ENDLIB"
                ):
                    if levelTag[level] != "":
                        s = levelTag[level]
                        levelTag[level] = ""
                ...
                if tag_name == "BGNLIB" or tag_name == "BGNSTR":
                    ...
                    first = True
                    indentPrint(
                        level, first, quote("time"), ": ", show_data(tag_type, rest)
                    )
                else:
                    indentPrint(
                        level,
                        first,
                        quote(tag_name.lower()),
                        ": ",
                        show_data(tag_type, rest),
                    )
                first = False
        if levelTag[level] != "":
            levelTag[level] = ""
