...
def reconfigure_loglevels(file_level=None, console_level=None):
    if file_level and console_level:
        logging.getLogger().setLevel(
            min(logging.getLevelName(file_level), logging.getLevelName(console_level))
        )
    elif file_level:
...
def get_loglevels():
    ...
    consolehandler = next(
        x for x in logging.getLogger().handlers if isinstance(x, logging.StreamHandler)
    )
    return logging.getLevelName(filehandler.level), logging.getLevelName(
