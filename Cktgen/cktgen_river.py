...
if __name__ == "__main__":
    ...
    for i in range(n - 2):
        adnetl.addInstance(
            ADI(ndev, ("un%d" % i), ADITransform.translate(xg(0), yg(n - 1 - i)))
        )
        adnetl.addInstance(
            ADI(
                pdev,
                ("up%d" % i),
                mirrorAcrossYAxis(pdev).preMult(
                    ADITransform.translate(xg(n - 1), yg(n - 3 - i))
                ),
            )
        )
    ...
    for i in range(n - 2):
        ...
        netl.newGR(
            ("i%d" % i), Rect(n - 2 - i, n - 1 - i, n - 2 - i, n - 3 - i), vly, vWidth
        )
        netl.newGR(
            ("i%d" % i), Rect(n - 2 - i, n - 3 - i, n - 1, n - 3 - i), hly, hWidth
        )

        netl.newGR(("o%d" % i), Rect(0, n - 1 - i, n - 2 - i, n - 1 - i), hly, hWidth)
        netl.newGR(
            ("o%d" % i), Rect(n - 2 - i, n - 1 - i, n - 2 - i, n - 3 - i), vly, vWidth
        )
        netl.newGR(
            ("o%d" % i), Rect(n - 2 - i, n - 3 - i, n - 1, n - 3 - i), hly, hWidth
        )

        netl.newGR(("z%d" % i), Rect(0, n - 1 - i, n - 2 - i, n - 1 - i), hly, hWidth)
        netl.newGR(
            ("z%d" % i), Rect(n - 2 - i, n - 1 - i, n - 2 - i, n - 3 - i), vly, vWidth
        )
        netl.newGR(
            ("z%d" % i), Rect(n - 2 - i, n - 3 - i, n - 1, n - 3 - i), hly, hWidth
        )
