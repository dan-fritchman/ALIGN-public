...
if __name__ == "__main__":
    ...
    for i in range(n - 3 - k):
        ...
        fy = n - 3 - k - i
        adnetl.addInstance(
            ADI(ndev, ("un%d" % i), ADITransform.translate(xg(sx), yg(sy)))
        )
        adnetl.addInstance(
            ADI(
                pdev,
                ("vn%d" % i),
                mirrorAcrossYAxis(pdev).preMult(ADITransform.translate(xg(fx), yg(fy))),
            )
        )
    ...
    for i in range(n - 3 - k):
        ...
        adnetl.addInstance(
            ADI(ndev, ("up%d" % i), ADITransform.translate(xg(sx), yg(sy)))
        )
        adnetl.addInstance(
            ADI(
                pdev,
