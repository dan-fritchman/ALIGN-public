...
class MetalTemplate:
    ...
    def __eq__(self, that):
        return (
            ...
            and self.widths == that.widths
            and self.spaces == that.spaces
            and self.colors == that.colors
        )
    ...
    def __str__(self):
        result = "MetalTemplate layer=%s name=%s widths=%s spaces=%s" % (
            self.layer,
            self.name,
            (",".join(str(i) for i in self.widths)),
            (",".join(str(i) for i in self.spaces)),
        )
        if self.colors:
    ...
    def __init__(self, fp):
        ...
        self._metalTemplates = [
            MetalTemplate(
                ...
                colors=d["colors"],
                stops=d["stops"],
                stop_offset=d["stop_offset"],
            )
            for d in self.json["metalTemplates"]
        ]
    ...
    def write_options_file(self, fn, bbox):
        with open(fn, "w") as fp:
            for mt in self.metalTemplates:
                ogd = (
                    "" if mt.stop_offset is None else f" ogdoffset_abs={mt.stop_offset}"
                )
                fp.write(
