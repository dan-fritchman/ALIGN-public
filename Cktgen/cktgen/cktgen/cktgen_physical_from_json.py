...
def extend(u, rng):
    if rng[0] is None or u < rng[0]:
        rng[0] = u
    if rng[1] is None or u > rng[1]:
        rng[1] = u
                ...
                wires = [
                    {
                        ...
                        "net_name": actual,
                        "width": 320,
                        "rect": [xc0, y_rng[0], xc0, y_rng[1]],
                    },
                    {
                        "layer": "metal3",
                        "net_name": actual,
                        "width": 320,
                        "rect": [xc1, y_rng[0], xc1, y_rng[1]],
                    },
                ]
            else:
                wires = [
...
def gr_hints(parser_results):
    ...
    for inst in placer_results["instances"]:
        ...
        for (formal, actual) in inst["formal_actual_map"].items():
            ...
            lst = [
                trans.hitRect(tRect(*terminal["rect"])).canonical()
                for terminal in pin_map[formal]
            ]
...
def hack_gr(results, bbox):
    ...
    for wire in wires:
        ...
        new_wire = {"layer": ly, "net_name": wire["net_name"], "width": 320, "rect": nr}
...
def main(args, tech):
    ...
    for inst in placer_results["instances"]:
        ...
        adnetl.addInstance(
            ADI(adts[tN], iN, ADITransform(tr["oX"], tr["oY"], tr["sX"], tr["sY"]))
        )
