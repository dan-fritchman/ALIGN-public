...
class Instance:
    ...
    def prnt(self):
        print(
            f"{self.template_name} {self.instance_name} ( {','.join(map(str,self.fa_list))} )"
        )
    ...
    def whole(self):
        ...
        while True:
            if self.nexttok is None:
                ...
                self._expect("NAME")
                m.nm = self.tok.value
                m.parameters = self.parse_possibly_empty_comma_separated_list(
                    self.parse_name
                )
                self._expect("SEMI")
                ...
                while True:
                    if self._accept_keyword("input") or self._accept_keyword("output"):
                        ...
                        inst.fa_list = self.parse_possibly_empty_comma_separated_list(
                            self.parse_fa_pair
                        )
                        self._expect("SEMI")
