...
class ADT:
    def __init__(self, tech, nm, npp=10, nr=1, *, physical_bbox=None):
        ...
        if physical_bbox is None:
            self.bbox = Rect(
                0,
                0,
                npp * self.tech.pitchPoly,
                nr * self.tech.dgPerRow * self.tech.pitchDG,
            )
        else:
    ...
    def parse_lgf(tech, fn):
        ...
        p_wire = re.compile(
            r"^Wire\s+net=(\S+)\s+(gid=(\S+)\s+|)layer=(\S+)\s+rect=(\S+):(\S+):(\S+):(\S+)\s*$"
        )
...
class ADI:
    ...
    def __repr__(self):
        return "template: %s instance: %s trans: %s" % (
            self.template,
            self.instanceName,
            self.trans,
        )
        ...
        for (_, v) in self.instances.items():
            for w in v.template.terminals:
                ...
                if fN in v.formalActualMap:
                    ...
                    if aN not in self.ces:
                        self.ces[aN] = {}
                    pN = (v.instanceName, w.netName)
                    if pN not in self.ces[aN]:
                        self.ces[aN][pN] = []
...
class Rect:
    ...
    def add(self, x, y):
        return Rect(
            min(x, self.llx), min(y, self.lly), max(x, self.urx), max(y, self.ury)
        )
...
class Wire:
    ...
    def __str__(self):
        return "Wire  net=%s%s layer=%s rect=%s" % (
            self.netName,
            ("" if self.gid is None else (" gid=%d" % self.gid)),
            self.layer,
            self.rect,
        )
...
def encode_GR(tech, obj):
    if isinstance(obj, GR):
        ...
        if obj.rect.llx == obj.rect.urx:  # vertical wire
            ...
            urx = xc + obj.width // 2
            lly = tech.pitchDG * (
                tech.halfYGRGrid * 2 * obj.rect.lly + tech.halfYGRGrid
            )
            ury = tech.pitchDG * (
                tech.halfYGRGrid * 2 * obj.rect.ury + tech.halfYGRGrid
            )
            ...
            urx = tech.pitchPoly * (
                tech.halfXGRGrid * 2 * obj.rect.urx + tech.halfXGRGrid
            )
        else:
        ...
        return {
            ...
            "layer": obj.layer,
            "gid": obj.gid,
            "rect": [obj.rect.llx, obj.rect.lly, obj.rect.urx, obj.rect.ury],
            "color": obj.color,
        }
    else:
...
class Netlist:
    ...
    def write_ctrl_file(
        ...
        *,
        ...
        with open(fn, "w") as fp:
            fp.write(
                f"""# circuit-independent technology collateral
...
"""
                ...
                for gr in v.grs:
                    if gr.connected_pins is not None:
                        ...
                        for cp in gr.connected_pins:
                            ...
                            if pin_gr_pitches_long > 0.5 and gr.layer in [
                                "metal2",
                                "metal4",
                            ]:
                                print(
                                    f"Long ({round(pin_gr_pitches_long,2)} pitches) horizonal pin found",
                                    cand,
                                    gr,
                                )
                            ...
                            if cand in self.wire_cache:
                                ...
                                print(
                                    "connected pin not found for:", k, hier_name, cand
                                )
                                assert hier_name[0] in ["C1", "C2", "R1", "R2"]
                            else:
                                print(
                                    "connected top-level pin not found for:",
                                    k,
                                    hier_name,
                                    cand,
                                )
    ...
    def write_files(self, tech, dirname, args):
        ...
        self.write_global_routing_file(
            tech, dirname + "/" + self.nm + "_dr_globalrouting.txt"
        )
        self.dumpGR(
            tech, dirname + "/" + self.nm + "_dr_globalrouting.json", no_grid=True
        )
...
def parse_lgf(fp):
    ...
    p_wire2 = re.compile(
        r"^Wire\s+net=(\S+)\s+layer=(\S+)\s+rect=(\S+):(\S+):(\S+):(\S+)(\s+gid=(\S+)|)\s*$"
    )

    p_wire_in_obj = re.compile(
        r"^\s+Wire\s+net=(\S+)\s+layer=(\S+)\s+rect=(\S+):(\S+):(\S+):(\S+)\s*$"
    )

    p_obj = re.compile(r"^Obj\s+net=(\S+)\s+gen=(\S+)\s+x=(\S+)\s+y=(\S+)\s*$")

    p_obj_lbrace = re.compile(
        r"^Obj\s+net=(\S+)\s+gen=(\S+)\s+x=(\S+)\s+y=(\S+)\s*{\s*$"
    )
...
def removeDuplicates(data):
    ...
    for d in data:
        ...
        if layer in viaLayers:
            ...
            twice_center = sum(
                rect[index] for index in indicesTbl[layersDict[layer]][0]
            )
            if layer not in tbl:
                tbl[layer] = {}
    ...
    for (layer, dirname) in layers:
        ...
        for (twice_center, v) in tbl[layer].items():
            ...
            for (rect, netName) in sl.rects:
                terminals.append({"layer": layer, "net_name": netName, "rect": rect})
...
def parse_args(command_line_args=None):
    parser = argparse.ArgumentParser(
        description="Generates input files for amsr (Analog router)"
    )
    ...
    parser.add_argument(
        "-tf", "--technology_file", type=str, default="DR_COLLATERAL/Process.json"
    )
...
if __name__ == "__main__":
    ...
    def mirrorAcrossYAxis(adt):
        return ADITransform.mirrorAcrossYAxis().preMult(
            ADITransform.translate(adt.bbox.urx, 0)
        )
    ...
    adnetl.addInstance(
        ADI(
            pdev,
            "up0",
            mirrorAcrossYAxis(pdev).preMult(ADITransform.translate(xg(8), yg(8))),
        )
    )
