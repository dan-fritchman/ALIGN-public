...
def run_router_in_container(args):
    ...
    run_sh(
        f'(cd INPUT && tar cvf - .) | docker run --rm {M_INPUT} -i ubuntu bash -c "cd /Cktgen/INPUT && tar xvf -"',
        "create INPUT",
    )

    run_sh(f"docker volume rm -f {args.routervol}", f"remove volume {args.routervol}")
    run_sh(
        f'(cd {args.techdir} && tar cvf - .) | docker run --rm {M_DR_COLLATERAL} -i ubuntu bash -c "cd /Cktgen/DR_COLLATERAL && tar xvf -"',
        "create DR_COLLATERAL",
    )
    ...
    run_sh(
        f'docker run --name sam {M_out} {M_INPUT} {M_DR_COLLATERAL} {ROUTER_IMAGE} bash -c "cd /Cktgen && amsr.exe -file INPUT/ctrl.txt"',
        "run detailed_router",
    )
...
def cmdline(argv):
    ...
    parser.add_argument(
        ...
        "--techdir",
        type=str,
        default="../DetailedRouter/DR_COLLATERAL_Generator/strawman1",
    )
    ...
    if args.viewer_input_dir and args.source:
        run_sh(
            f'cp INPUT/mydesign_dr_globalrouting.json {args.viewer_input_dir + "/" + args.source + ".json"}'
        )
